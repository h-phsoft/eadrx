...
//
// Table class for phs_keyvalues
//
class cphs_keyvalues extends cTable {
	var $kval_id;
	var $key_id;
	var $lang_id;
	var $key_value;
	...
	function __construct() {
		...
		$this->TableVar = 'phs_keyvalues';
		$this->TableName = 'phs_keyvalues';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`phs_keyvalues`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// kval_id
		$this->kval_id = new cField('phs_keyvalues', 'phs_keyvalues', 'x_kval_id', 'kval_id', '`kval_id`', '`kval_id`', 3, -1, FALSE, '`kval_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// key_id
		$this->key_id = new cField('phs_keyvalues', 'phs_keyvalues', 'x_key_id', 'key_id', '`key_id`', '`key_id`', 3, -1, FALSE, '`key_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->key_id->Sortable = TRUE; // Allow sort
		$this->key_id->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->key_id->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->key_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['key_id'] = &$this->key_id;

		// lang_id
		$this->lang_id = new cField('phs_keyvalues', 'phs_keyvalues', 'x_lang_id', 'lang_id', '`lang_id`', '`lang_id`', 3, -1, FALSE, '`lang_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// key_value
		$this->key_value = new cField('phs_keyvalues', 'phs_keyvalues', 'x_key_value', 'key_value', '`key_value`', '`key_value`', 200, -1, FALSE, '`key_value`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->key_value->Sortable = TRUE; // Allow sort
		$this->fields['key_value'] = &$this->key_value;

		// key_rvalue
		$this->key_rvalue = new cField('phs_keyvalues', 'phs_keyvalues', 'x_key_rvalue', 'key_rvalue', '`key_rvalue`', '`key_rvalue`', 200, -1, FALSE, '`key_rvalue`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->key_rvalue->Sortable = TRUE; // Allow sort
		$this->fields['key_rvalue'] = &$this->key_rvalue;

		// key_text
		$this->key_text = new cField('phs_keyvalues', 'phs_keyvalues', 'x_key_text', 'key_text', '`key_text`', '`key_text`', 201, -1, FALSE, '`key_text`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	function GetMasterFilter() {
		...
		if ($this->getCurrentMasterTable() == "phs_keys") {
			if ($this->key_id->getSessionValue() <> "")
				$sMasterFilter .= "`key_id`=" . ew_QuotedValue($this->key_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "phs_keys") {
			if ($this->key_id->getSessionValue() <> "")
				$sDetailFilter .= "`key_id`=" . ew_QuotedValue($this->key_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_phs_keys() {
		return "`key_id`=@key_id@";
	}

	// Detail filter
	function SqlDetailFilter_phs_keys() {
		return "`key_id`=@key_id@";
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`phs_keyvalues`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "phs_keyvaluesedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "phs_keyvaluesadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("phs_keyvaluesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("phs_keyvaluesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "phs_keyvaluesadd.php?" . $this->UrlParm($parm);
		else
			$url = "phs_keyvaluesadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("phs_keyvaluesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("phs_keyvaluesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("phs_keyvaluesdelete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "phs_keys" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
			$url .= (strpos($url, "?") !== FALSE ? "&" : "?") . EW_TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_key_id=" . urlencode($this->key_id->CurrentValue);
		}
		return $url;
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->key_id->Exportable) $Doc->ExportCaption($this->key_id);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->key_id->Exportable) $Doc->ExportField($this->key_id);
