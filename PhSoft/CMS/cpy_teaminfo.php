...
//
// Table class for cpy_team
//
class ccpy_team extends cTable {
	var $team_id;
	var $team_iname;
	var $team_order;
	var $team_photo;
	...
	function __construct() {
		...
		$this->TableVar = 'cpy_team';
		$this->TableName = 'cpy_team';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`cpy_team`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// team_id
		$this->team_id = new cField('cpy_team', 'cpy_team', 'x_team_id', 'team_id', '`team_id`', '`team_id`', 3, -1, FALSE, '`team_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->team_id->Sortable = FALSE; // Allow sort
		$this->team_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['team_id'] = &$this->team_id;

		// team_iname
		$this->team_iname = new cField('cpy_team', 'cpy_team', 'x_team_iname', 'team_iname', '`team_iname`', '`team_iname`', 200, -1, FALSE, '`team_iname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->team_iname->Sortable = TRUE; // Allow sort
		$this->fields['team_iname'] = &$this->team_iname;

		// team_order
		$this->team_order = new cField('cpy_team', 'cpy_team', 'x_team_order', 'team_order', '`team_order`', '`team_order`', 2, -1, FALSE, '`team_order`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// team_photo
		$this->team_photo = new cField('cpy_team', 'cpy_team', 'x_team_photo', 'team_photo', '`team_photo`', '`team_photo`', 200, -1, TRUE, '`team_photo`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "cpy_team_names") {
			$sDetailUrl = $GLOBALS["cpy_team_names"]->GetListUrl() . "?" . EW_TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$sDetailUrl .= "&fk_team_id=" . urlencode($this->team_id->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "cpy_teamlist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`cpy_team`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->team_id->setDbValue($conn->Insert_ID());
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('team_id', $rs))
				ew_AddFilter($where, ew_QuotedName('team_id', $this->DBID) . '=' . ew_QuotedValue($rs['team_id'], $this->team_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->team_id->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->team_id->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@team_id@", ew_AdjustSql($this->team_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "cpy_teamedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "cpy_teamadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("cpy_teamview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("cpy_teamview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "cpy_teamadd.php?" . $this->UrlParm($parm);
		else
			$url = "cpy_teamadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("cpy_teamedit.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("cpy_teamedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("cpy_teamadd.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("cpy_teamadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("cpy_teamdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "team_id:" . ew_VarToJson($this->team_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->team_id->CurrentValue)) {
			$sUrl .= "team_id=" . urlencode($this->team_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["team_id"]))
				$arKeys[] = $_POST["team_id"];
