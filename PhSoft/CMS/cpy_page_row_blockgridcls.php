...
$cpy_page_row_block_grid = NULL; // Initialize page object first

class ccpy_page_row_block_grid extends ccpy_page_row_block {
	...
	// Page object name
	var $PageObjName = 'cpy_page_row_block_grid';

	// Grid form hidden field names
	var $FormName = 'fcpy_page_row_blockgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["cpy_page_row_block"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadDefaultValues() {
		...
		$this->blk_image->Upload->Index = $this->RowIndex;
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->blk_id->CurrentValue = strval($arKeys[0]); // blk_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "cpy_page_row") {
			...
			if ($GLOBALS["cpy_page_row"]->EventCancelled) $this->EventCancelled = TRUE;
		}
