...
$phs_keyvalues_grid = NULL; // Initialize page object first

class cphs_keyvalues_grid extends cphs_keyvalues {
	...
	// Page object name
	var $PageObjName = 'phs_keyvalues_grid';

	// Grid form hidden field names
	var $FormName = 'fphs_keyvaluesgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["phs_keyvalues"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->kval_id->CurrentValue = strval($arKeys[0]); // kval_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "phs_keys") {
				$this->key_id->CurrentValue = $this->key_id->getSessionValue();
			}
		$conn = &$this->Connection();
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "phs_keys") {
			$this->key_id->Visible = FALSE;
			if ($GLOBALS["phs_keys"]->EventCancelled) $this->EventCancelled = TRUE;
		}
