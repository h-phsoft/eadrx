...
class cphs_users extends cTable {
	...
	function __construct() {
		...
		// user_password
		$this->user_password = new cField('phs_users', 'phs_users', 'x_user_password', 'user_password', '`user_password`', '`user_password`', 200, -1, FALSE, '`user_password`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function UserIDFilter($userid) {
		...
		$sParentUserIDFilter = '`user_id` IN (SELECT `user_id` FROM ' . "`phs_users`" . ' WHERE `pgrp_id` = ' . ew_QuotedValue($userid, EW_DATATYPE_NUMBER, EW_USER_TABLE_DBID) . ')';
	...
	function GetUserIDSubquery(&$fld, &$masterfld) {
		...
		$sWrk = "";
		$sSql = "SELECT " . $masterfld->FldExpression . " FROM `phs_users`";
