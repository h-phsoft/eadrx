...
$app_book_view = NULL; // Initialize page object first

class capp_book_view extends capp_book {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "app_booklist.php"; // No matching record, return to list
					} else {
						$this->LoadRowValues($this->Recordset); // Load row values
					}
			}
		} else {
			$sReturnUrl = "app_booklist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		$item = &$option->Add("detail_app_book_page");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("app_book_page", "TblCaption");
		...
		$links = "";
		if ($GLOBALS["app_book_page_grid"] && $GLOBALS["app_book_page_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'app_book_page')) {
			$links .= "<li><a class=\"ewRowLink ewDetailView\" data-action=\"view\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailViewLink")) . "\" href=\"" . ew_HtmlEncode($this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=app_book_page")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailViewLink")) . "</a></li>";
			if ($DetailViewTblVar <> "") $DetailViewTblVar .= ",";
			$DetailViewTblVar .= "app_book_page";
		}
		if ($GLOBALS["app_book_page_grid"] && $GLOBALS["app_book_page_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'app_book_page')) {
			$links .= "<li><a class=\"ewRowLink ewDetailEdit\" data-action=\"edit\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailEditLink")) . "\" href=\"" . ew_HtmlEncode($this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=app_book_page")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailEditLink")) . "</a></li>";
			if ($DetailEditTblVar <> "") $DetailEditTblVar .= ",";
			$DetailEditTblVar .= "app_book_page";
		}
		if ($GLOBALS["app_book_page_grid"] && $GLOBALS["app_book_page_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'app_book_page')) {
			$links .= "<li><a class=\"ewRowLink ewDetailCopy\" data-action=\"add\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailCopyLink")) . "\" href=\"" . ew_HtmlEncode($this->GetCopyUrl(EW_TABLE_SHOW_DETAIL . "=app_book_page")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailCopyLink")) . "</a></li>";
		...
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'app_book_page');
		if ($item->Visible) {
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("app_book_page", $DetailTblVar)) {
				...
				if ($GLOBALS["app_book_page_grid"]->DetailView) {
					$GLOBALS["app_book_page_grid"]->CurrentMode = "view";
...
// Create page object
if (!isset($app_book_view)) $app_book_view = new capp_book_view();

// Page init
$app_book_view->Page_Init();

// Page main
$app_book_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_book_view->Page_Render();
?>
...
var CurrentForm = fapp_bookview = new ew_Form("fapp_bookview", "view");

// Form_CustomValidate event
fapp_bookview.Form_CustomValidate = 
...
fapp_bookview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_bookview.Lists["x_lang_id"] = {"LinkField":"x_lang_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_lang_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_language"};
fapp_bookview.Lists["x_lang_id"].Data = "<?php echo $app_book_view->lang_id->LookupFilterQuery(FALSE, "view") ?>";
fapp_bookview.Lists["x_status_id"] = {"LinkField":"x_status_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_status_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_status"};
fapp_bookview.Lists["x_status_id"].Data = "<?php echo $app_book_view->status_id->LookupFilterQuery(FALSE, "view") ?>";
...
<div class="ewToolbar">
<?php $app_book_view->ExportOptions->Render("body") ?>
<?php
	foreach ($app_book_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php $app_book_view->ShowPageHeader(); ?>
<?php
$app_book_view->ShowMessage();
?>
<?php if (!$app_book_view->IsModal) { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($app_book_view->Pager)) $app_book_view->Pager = new cPrevNextPager($app_book_view->StartRec, $app_book_view->DisplayRecs, $app_book_view->TotalRecs, $app_book_view->AutoHidePager) ?>
<?php if ($app_book_view->Pager->RecordCount > 0 && $app_book_view->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($app_book_view->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $app_book_view->PageUrl() ?>start=<?php echo $app_book_view->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($app_book_view->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $app_book_view->PageUrl() ?>start=<?php echo $app_book_view->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $app_book_view->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($app_book_view->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $app_book_view->PageUrl() ?>start=<?php echo $app_book_view->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($app_book_view->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $app_book_view->PageUrl() ?>start=<?php echo $app_book_view->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $app_book_view->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<form name="fapp_bookview" id="fapp_bookview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_book_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_book_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_book">
<input type="hidden" name="modal" value="<?php echo intval($app_book_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($app_book->lang_id->Visible) { // lang_id ?>
	<tr id="r_lang_id">
		<td class="col-sm-2"><span id="elh_app_book_lang_id"><?php echo $app_book->lang_id->FldCaption() ?></span></td>
...
<?php echo $app_book->lang_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_book->status_id->Visible) { // status_id ?>
	<tr id="r_status_id">
		<td class="col-sm-2"><span id="elh_app_book_status_id"><?php echo $app_book->status_id->FldCaption() ?></span></td>
...
<?php echo $app_book->status_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_book->book_title->Visible) { // book_title ?>
	<tr id="r_book_title">
		<td class="col-sm-2"><span id="elh_app_book_book_title"><?php echo $app_book->book_title->FldCaption() ?></span></td>
...
<?php echo $app_book->book_title->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_book->book_price->Visible) { // book_price ?>
	<tr id="r_book_price">
		<td class="col-sm-2"><span id="elh_app_book_book_price"><?php echo $app_book->book_price->FldCaption() ?></span></td>
...
<?php echo $app_book->book_price->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_book->book_image->Visible) { // book_image ?>
	<tr id="r_book_image">
		<td class="col-sm-2"><span id="elh_app_book_book_image"><?php echo $app_book->book_image->FldCaption() ?></span></td>
		<td data-name="book_image"<?php echo $app_book->book_image->CellAttributes() ?>>
<span id="el_app_book_book_image">
<span>
<?php echo ew_GetFileViewTag($app_book->book_image, $app_book->book_image->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php if ($app_book->book_desc->Visible) { // book_desc ?>
	<tr id="r_book_desc">
		<td class="col-sm-2"><span id="elh_app_book_book_desc"><?php echo $app_book->book_desc->FldCaption() ?></span></td>
		<td data-name="book_desc"<?php echo $app_book->book_desc->CellAttributes() ?>>
<span id="el_app_book_book_desc">
<span<?php echo $app_book->book_desc->ViewAttributes() ?>>
<?php echo $app_book->book_desc->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_book->ins_datetime->Visible) { // ins_datetime ?>
	<tr id="r_ins_datetime">
		<td class="col-sm-2"><span id="elh_app_book_ins_datetime"><?php echo $app_book->ins_datetime->FldCaption() ?></span></td>
...
<?php echo $app_book->ins_datetime->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php } ?>
<?php
	if (in_array("app_book_page", explode(",", $app_book->getCurrentDetailTable())) && $app_book_page->DetailView) {
?>
