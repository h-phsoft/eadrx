...
$phs_users_update = NULL; // Initialize page object first

class cphs_users_update extends cphs_users {
	...
	// Page object name
	var $PageObjName = 'phs_users_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("phs_userslist.php"); // No records selected, return to list
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					...
					$this->user_logon->setDbValue($this->Recordset->fields('user_logon'));
					...
					if (!ew_CompareValue($this->user_logon->DbValue, $this->Recordset->fields('user_logon')))
	...
	function LoadFormValues() {
		...
		$this->user_logon->MultiUpdate = $objForm->GetValue("u_user_logon");
	...
	function RenderRow() {
		...
		$this->user_email->ViewCustomAttributes = "";
			...
			if (!$Security->IsAdmin() && $Security->IsLoggedIn()) { // Non system admin
				if (strval($this->user_id->CurrentValue) == strval(CurrentUserID())) {
			if ($Security->CanAdmin()) { // System admin
			if (strval($this->pgrp_id->CurrentValue) <> "") {
				$this->pgrp_id->EditValue = $this->pgrp_id->OptionCaption($this->pgrp_id->CurrentValue);
			} else {
				$this->pgrp_id->EditValue = NULL;
			}
			} else {
				$this->pgrp_id->EditValue = $Language->Phrase("PasswordMask");
			}
			$this->pgrp_id->ViewCustomAttributes = "";
				} else {
				}
			...
			$this->pgrp_id->EditValue = $this->pgrp_id->Options(TRUE);
			}
	...
	function ValidateForm() {
		...
		if ($this->user_logon->MultiUpdate == "1") $lUpdateCnt++;
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if ($this->user_logon->MultiUpdate <> "" && !$this->user_logon->FldIsDetailKey && !is_null($this->user_logon->FormValue) && $this->user_logon->FormValue == "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// pgrp_id
			if ($Security->CanAdmin()) { // System admin
			$this->pgrp_id->SetDbValueDef($rsnew, $this->pgrp_id->CurrentValue, NULL, $this->pgrp_id->ReadOnly || $this->pgrp_id->MultiUpdate <> "1");
			}

			// user_logon
			$this->user_logon->SetDbValueDef($rsnew, $this->user_logon->CurrentValue, "", $this->user_logon->ReadOnly || $this->user_logon->MultiUpdate <> "1");

			// user_password
			$this->user_password->SetDbValueDef($rsnew, $this->user_password->CurrentValue, "", $this->user_password->ReadOnly || $this->user_password->MultiUpdate <> "1" || (EW_ENCRYPTED_PASSWORD && $rs->fields('user_password') == $this->user_password->CurrentValue));

			// user_email
			$this->user_email->SetDbValueDef($rsnew, $this->user_email->CurrentValue, "", $this->user_email->ReadOnly || $this->user_email->MultiUpdate <> "1");
...
// Create page object
if (!isset($phs_users_update)) $phs_users_update = new cphs_users_update();

// Page init
$phs_users_update->Page_Init();

// Page main
$phs_users_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$phs_users_update->Page_Render();
?>
...
var CurrentForm = fphs_usersupdate = new ew_Form("fphs_usersupdate", "update");

// Validate form
fphs_usersupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_user_logon");
...
fphs_usersupdate.Form_CustomValidate = 
...
fphs_usersupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fphs_usersupdate.Lists["x_pgrp_id"] = {"LinkField":"x_user_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_user_id","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_users"};
fphs_usersupdate.Lists["x_pgrp_id"].Options = <?php echo json_encode($phs_users_update->pgrp_id->Options()) ?>;
...
<?php $phs_users_update->ShowPageHeader(); ?>
<?php
$phs_users_update->ShowMessage();
?>
<form name="fphs_usersupdate" id="fphs_usersupdate" class="<?php echo $phs_users_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($phs_users_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $phs_users_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="phs_users">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($phs_users_update->IsModal) ?>">
<!-- Fields to prevent google autofill -->
<input class="hidden" type="text" name="<?php echo ew_Encrypt(ew_Random()) ?>">
<input class="hidden" type="password" name="<?php echo ew_Encrypt(ew_Random()) ?>">
<?php foreach ($phs_users_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_phs_usersupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($phs_users->pgrp_id->Visible) { // pgrp_id ?>
	<div id="r_pgrp_id" class="form-group">
		<label for="x_pgrp_id" class="<?php echo $phs_users_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_pgrp_id" id="u_pgrp_id" class="ewMultiSelect" value="1"<?php echo ($phs_users->pgrp_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_users->pgrp_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_users_update->RightColumnClass ?>"><div<?php echo $phs_users->pgrp_id->CellAttributes() ?>>
<?php if (!$Security->IsAdmin() && $Security->IsLoggedIn()) { // Non system admin ?>
<?php if (strval($phs_users->user_id->CurrentValue) == strval(CurrentUserID())) { ?>
<span id="el_phs_users_pgrp_id">
<span<?php echo $phs_users->pgrp_id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $phs_users->pgrp_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="phs_users" data-field="x_pgrp_id" name="x_pgrp_id" id="x_pgrp_id" value="<?php echo ew_HtmlEncode($phs_users->pgrp_id->CurrentValue) ?>">
<?php } else { ?>
<span id="el_phs_users_pgrp_id">
...
<span id="el_phs_users_pgrp_id">
<p class="form-control-static"><?php echo $phs_users->pgrp_id->EditValue ?></p>
</span>
<?php } else { ?>
<span id="el_phs_users_pgrp_id">
<select data-table="phs_users" data-field="x_pgrp_id" data-value-separator="<?php echo $phs_users->pgrp_id->DisplayValueSeparatorAttribute() ?>" id="x_pgrp_id" name="x_pgrp_id"<?php echo $phs_users->pgrp_id->EditAttributes() ?>>
...
<?php if ($phs_users->user_logon->Visible) { // user_logon ?>
	<div id="r_user_logon" class="form-group">
		<label for="x_user_logon" class="<?php echo $phs_users_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_user_logon" id="u_user_logon" class="ewMultiSelect" value="1"<?php echo ($phs_users->user_logon->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_users->user_logon->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_users_update->RightColumnClass ?>"><div<?php echo $phs_users->user_logon->CellAttributes() ?>>
...
<?php if ($phs_users->user_password->Visible) { // user_password ?>
	<div id="r_user_password" class="form-group">
		<label for="x_user_password" class="<?php echo $phs_users_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_user_password" id="u_user_password" class="ewMultiSelect" value="1"<?php echo ($phs_users->user_password->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_users->user_password->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_users_update->RightColumnClass ?>"><div<?php echo $phs_users->user_password->CellAttributes() ?>>
...
<?php if ($phs_users->user_email->Visible) { // user_email ?>
	<div id="r_user_email" class="form-group">
		<label for="x_user_email" class="<?php echo $phs_users_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_user_email" id="u_user_email" class="ewMultiSelect" value="1"<?php echo ($phs_users->user_email->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_users->user_email->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_users_update->RightColumnClass ?>"><div<?php echo $phs_users->user_email->CellAttributes() ?>>
...
<?php if (!$phs_users_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $phs_users_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $phs_users_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
	</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fphs_usersupdate.Init();
</script>
<?php
$phs_users_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$phs_users_update->Page_Terminate();
?>
