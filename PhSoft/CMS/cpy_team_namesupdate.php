...
$cpy_team_names_update = NULL; // Initialize page object first

class ccpy_team_names_update extends ccpy_team_names {
	...
	// Page object name
	var $PageObjName = 'cpy_team_names_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("cpy_team_nameslist.php"); // No records selected, return to list
	...
	function LoadMultiUpdateValues() {
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					$this->team_id->setDbValue($this->Recordset->fields('team_id'));
					$this->lang_id->setDbValue($this->Recordset->fields('lang_id'));
					$this->team_name->setDbValue($this->Recordset->fields('team_name'));
					$this->team_title->setDbValue($this->Recordset->fields('team_title'));
					$this->team_position->setDbValue($this->Recordset->fields('team_position'));
					$this->team_text->setDbValue($this->Recordset->fields('team_text'));
				} else {
					if (!ew_CompareValue($this->team_id->DbValue, $this->Recordset->fields('team_id')))
					...
					if (!ew_CompareValue($this->team_name->DbValue, $this->Recordset->fields('team_name')))
						$this->team_name->CurrentValue = NULL;
					if (!ew_CompareValue($this->team_title->DbValue, $this->Recordset->fields('team_title')))
						$this->team_title->CurrentValue = NULL;
					if (!ew_CompareValue($this->team_position->DbValue, $this->Recordset->fields('team_position')))
						$this->team_position->CurrentValue = NULL;
					if (!ew_CompareValue($this->team_text->DbValue, $this->Recordset->fields('team_text')))
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->teamn_id->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->team_id->MultiUpdate = $objForm->GetValue("u_team_id");
	...
	function ValidateForm() {
		...
		$gsFormError = "";
		$lUpdateCnt = 0;
		if ($this->team_id->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->lang_id->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->team_name->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->team_title->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->team_position->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->team_text->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if ($this->team_id->MultiUpdate <> "" && !$this->team_id->FldIsDetailKey && !is_null($this->team_id->FormValue) && $this->team_id->FormValue == "") {
		...
		if ($this->team_name->MultiUpdate <> "" && !$this->team_name->FldIsDetailKey && !is_null($this->team_name->FormValue) && $this->team_name->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->team_name->FldCaption(), $this->team_name->ReqErrMsg));
		}
		if ($this->team_title->MultiUpdate <> "" && !$this->team_title->FldIsDetailKey && !is_null($this->team_title->FormValue) && $this->team_title->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->team_title->FldCaption(), $this->team_title->ReqErrMsg));
		}
		if ($this->team_position->MultiUpdate <> "" && !$this->team_position->FldIsDetailKey && !is_null($this->team_position->FormValue) && $this->team_position->FormValue == "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// team_id
			$this->team_id->SetDbValueDef($rsnew, $this->team_id->CurrentValue, 0, $this->team_id->ReadOnly || $this->team_id->MultiUpdate <> "1");

			// lang_id
			$this->lang_id->SetDbValueDef($rsnew, $this->lang_id->CurrentValue, 0, $this->lang_id->ReadOnly || $this->lang_id->MultiUpdate <> "1");

			// team_name
			$this->team_name->SetDbValueDef($rsnew, $this->team_name->CurrentValue, "", $this->team_name->ReadOnly || $this->team_name->MultiUpdate <> "1");

			// team_title
			$this->team_title->SetDbValueDef($rsnew, $this->team_title->CurrentValue, "", $this->team_title->ReadOnly || $this->team_title->MultiUpdate <> "1");

			// team_position
			$this->team_position->SetDbValueDef($rsnew, $this->team_position->CurrentValue, "", $this->team_position->ReadOnly || $this->team_position->MultiUpdate <> "1");

			// team_text
			$this->team_text->SetDbValueDef($rsnew, $this->team_text->CurrentValue, NULL, $this->team_text->ReadOnly || $this->team_text->MultiUpdate <> "1");
...
// Create page object
if (!isset($cpy_team_names_update)) $cpy_team_names_update = new ccpy_team_names_update();

// Page init
$cpy_team_names_update->Page_Init();

// Page main
$cpy_team_names_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cpy_team_names_update->Page_Render();
?>
...
var CurrentForm = fcpy_team_namesupdate = new ew_Form("fcpy_team_namesupdate", "update");

// Validate form
fcpy_team_namesupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_team_id");
			...
			uelm = this.GetElements("u" + infix + "_team_name");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
			...
			uelm = this.GetElements("u" + infix + "_team_title");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
			...
			uelm = this.GetElements("u" + infix + "_team_position");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
...
fcpy_team_namesupdate.Form_CustomValidate = 
...
fcpy_team_namesupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fcpy_team_namesupdate.Lists["x_team_id"] = {"LinkField":"x_team_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_team_iname","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cpy_team"};
fcpy_team_namesupdate.Lists["x_team_id"].Data = "<?php echo $cpy_team_names_update->team_id->LookupFilterQuery(FALSE, "update") ?>";
fcpy_team_namesupdate.Lists["x_lang_id"] = {"LinkField":"x_lang_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_lang_id","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_language"};
fcpy_team_namesupdate.Lists["x_lang_id"].Data = "<?php echo $cpy_team_names_update->lang_id->LookupFilterQuery(FALSE, "update") ?>";
...
<?php $cpy_team_names_update->ShowPageHeader(); ?>
<?php
$cpy_team_names_update->ShowMessage();
?>
<form name="fcpy_team_namesupdate" id="fcpy_team_namesupdate" class="<?php echo $cpy_team_names_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cpy_team_names_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cpy_team_names_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="cpy_team_names">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($cpy_team_names_update->IsModal) ?>">
<?php foreach ($cpy_team_names_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_cpy_team_namesupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($cpy_team_names->team_id->Visible) { // team_id ?>
	<div id="r_team_id" class="form-group">
		<label for="x_team_id" class="<?php echo $cpy_team_names_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_team_id" id="u_team_id" class="ewMultiSelect" value="1"<?php echo ($cpy_team_names->team_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $cpy_team_names->team_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $cpy_team_names_update->RightColumnClass ?>"><div<?php echo $cpy_team_names->team_id->CellAttributes() ?>>
...
<?php if ($cpy_team_names->lang_id->Visible) { // lang_id ?>
	<div id="r_lang_id" class="form-group">
		<label for="x_lang_id" class="<?php echo $cpy_team_names_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_lang_id" id="u_lang_id" class="ewMultiSelect" value="1"<?php echo ($cpy_team_names->lang_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $cpy_team_names->lang_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $cpy_team_names_update->RightColumnClass ?>"><div<?php echo $cpy_team_names->lang_id->CellAttributes() ?>>
...
<?php if ($cpy_team_names->team_name->Visible) { // team_name ?>
	<div id="r_team_name" class="form-group">
		<label for="x_team_name" class="<?php echo $cpy_team_names_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_team_name" id="u_team_name" class="ewMultiSelect" value="1"<?php echo ($cpy_team_names->team_name->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $cpy_team_names->team_name->FldCaption() ?></label></div></label>
		<div class="<?php echo $cpy_team_names_update->RightColumnClass ?>"><div<?php echo $cpy_team_names->team_name->CellAttributes() ?>>
...
<?php if ($cpy_team_names->team_title->Visible) { // team_title ?>
	<div id="r_team_title" class="form-group">
		<label for="x_team_title" class="<?php echo $cpy_team_names_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_team_title" id="u_team_title" class="ewMultiSelect" value="1"<?php echo ($cpy_team_names->team_title->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $cpy_team_names->team_title->FldCaption() ?></label></div></label>
		<div class="<?php echo $cpy_team_names_update->RightColumnClass ?>"><div<?php echo $cpy_team_names->team_title->CellAttributes() ?>>
...
<?php if ($cpy_team_names->team_position->Visible) { // team_position ?>
	<div id="r_team_position" class="form-group">
		<label for="x_team_position" class="<?php echo $cpy_team_names_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_team_position" id="u_team_position" class="ewMultiSelect" value="1"<?php echo ($cpy_team_names->team_position->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $cpy_team_names->team_position->FldCaption() ?></label></div></label>
		<div class="<?php echo $cpy_team_names_update->RightColumnClass ?>"><div<?php echo $cpy_team_names->team_position->CellAttributes() ?>>
...
<?php if ($cpy_team_names->team_text->Visible) { // team_text ?>
	<div id="r_team_text" class="form-group">
		<label class="<?php echo $cpy_team_names_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_team_text" id="u_team_text" class="ewMultiSelect" value="1"<?php echo ($cpy_team_names->team_text->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $cpy_team_names->team_text->FldCaption() ?></label></div></label>
		<div class="<?php echo $cpy_team_names_update->RightColumnClass ?>"><div<?php echo $cpy_team_names->team_text->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateEditor("fcpy_team_namesupdate", "x_team_text", 35, 8, <?php echo ($cpy_team_names->team_text->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<?php if (!$cpy_team_names_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $cpy_team_names_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cpy_team_names_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
	</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fcpy_team_namesupdate.Init();
</script>
<?php
$cpy_team_names_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cpy_team_names_update->Page_Terminate();
?>
