...
$app_subscribe_view = NULL; // Initialize page object first

class capp_subscribe_view extends capp_subscribe {
	...
	// Page object name
	var $PageObjName = 'app_subscribe_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["subs_id"] <> "") {
			$this->RecKey["subs_id"] = $_GET["subs_id"];
			$KeyUrl .= "&amp;subs_id=" . urlencode($this->RecKey["subs_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["subs_id"] <> "") {
				...
				$this->RecKey["subs_id"] = $this->subs_id->QueryStringValue;
			} elseif (@$_POST["subs_id"] <> "") {
				$this->subs_id->setFormValue($_POST["subs_id"]);
				$this->RecKey["subs_id"] = $this->subs_id->FormValue;
			} else {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "app_subscribelist.php"; // No matching record, return to list
					} else {
						$this->LoadRowValues($this->Recordset); // Load row values
					}
			}
		} else {
			$sReturnUrl = "app_subscribelist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		// "detail_app_subscribe_cats"
		$item = &$option->Add("detail_app_subscribe_cats");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("app_subscribe_cats", "TblCaption");
		$body .= str_replace("%c", $this->app_subscribe_cats_Count, $Language->Phrase("DetailCount"));
		$body = "<a class=\"btn btn-default btn-sm ewRowLink ewDetail\" data-action=\"list\" href=\"" . ew_HtmlEncode("app_subscribe_catslist.php?" . EW_TABLE_SHOW_MASTER . "=app_subscribe&fk_subs_id=" . urlencode(strval($this->subs_id->CurrentValue)) . "") . "\">" . $body . "</a>";
		$links = "";
		if ($GLOBALS["app_subscribe_cats_grid"] && $GLOBALS["app_subscribe_cats_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'app_subscribe_cats')) {
			$links .= "<li><a class=\"ewRowLink ewDetailView\" data-action=\"view\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailViewLink")) . "\" href=\"" . ew_HtmlEncode($this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=app_subscribe_cats")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailViewLink")) . "</a></li>";
			if ($DetailViewTblVar <> "") $DetailViewTblVar .= ",";
			$DetailViewTblVar .= "app_subscribe_cats";
		}
		if ($GLOBALS["app_subscribe_cats_grid"] && $GLOBALS["app_subscribe_cats_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'app_subscribe_cats')) {
			$links .= "<li><a class=\"ewRowLink ewDetailEdit\" data-action=\"edit\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailEditLink")) . "\" href=\"" . ew_HtmlEncode($this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=app_subscribe_cats")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailEditLink")) . "</a></li>";
			if ($DetailEditTblVar <> "") $DetailEditTblVar .= ",";
			$DetailEditTblVar .= "app_subscribe_cats";
		}
		if ($GLOBALS["app_subscribe_cats_grid"] && $GLOBALS["app_subscribe_cats_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'app_subscribe_cats')) {
			$links .= "<li><a class=\"ewRowLink ewDetailCopy\" data-action=\"add\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailCopyLink")) . "\" href=\"" . ew_HtmlEncode($this->GetCopyUrl(EW_TABLE_SHOW_DETAIL . "=app_subscribe_cats")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailCopyLink")) . "</a></li>";
			if ($DetailCopyTblVar <> "") $DetailCopyTblVar .= ",";
			$DetailCopyTblVar .= "app_subscribe_cats";
		}
		if ($links <> "") {
		...
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'app_subscribe_cats');
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "app_subscribe_cats";
		}
	...
	function LoadRowValues($rs = NULL) {
		...
		$sDetailFilter = $GLOBALS["app_subscribe_cats"]->SqlDetailFilter_app_subscribe();
		$sDetailFilter = str_replace("@sub_id@", ew_AdjustSql($this->subs_id->DbValue, "DB"), $sDetailFilter);
		$GLOBALS["app_subscribe_cats"]->setCurrentMasterTable("app_subscribe");
		$sDetailFilter = $GLOBALS["app_subscribe_cats"]->ApplyUserIDFilters($sDetailFilter);
		$this->app_subscribe_cats_Count = $GLOBALS["app_subscribe_cats"]->LoadRecordCount($sDetailFilter);
	}
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("app_subscribe_cats", $DetailTblVar)) {
				...
				if ($GLOBALS["app_subscribe_cats_grid"]->DetailView) {
					$GLOBALS["app_subscribe_cats_grid"]->CurrentMode = "view";
...
// Create page object
if (!isset($app_subscribe_view)) $app_subscribe_view = new capp_subscribe_view();

// Page init
$app_subscribe_view->Page_Init();

// Page main
$app_subscribe_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_subscribe_view->Page_Render();
?>
...
var CurrentForm = fapp_subscribeview = new ew_Form("fapp_subscribeview", "view");

// Form_CustomValidate event
fapp_subscribeview.Form_CustomValidate = 
...
fapp_subscribeview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_subscribeview.Lists["x_serv_id"] = {"LinkField":"x_serv_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_serv_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_service"};
fapp_subscribeview.Lists["x_serv_id"].Data = "<?php echo $app_subscribe_view->serv_id->LookupFilterQuery(FALSE, "view") ?>";
fapp_subscribeview.Lists["x_user_id"] = {"LinkField":"x_user_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_user_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cpy_user"};
fapp_subscribeview.Lists["x_user_id"].Data = "<?php echo $app_subscribe_view->user_id->LookupFilterQuery(FALSE, "view") ?>";
fapp_subscribeview.Lists["x_cycle_id"] = {"LinkField":"x_cycle_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_cycle_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_bill_cycle"};
fapp_subscribeview.Lists["x_cycle_id"].Data = "<?php echo $app_subscribe_view->cycle_id->LookupFilterQuery(FALSE, "view") ?>";
fapp_subscribeview.Lists["x_book_id"] = {"LinkField":"x_book_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_book_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_book"};
fapp_subscribeview.Lists["x_book_id"].Data = "<?php echo $app_subscribe_view->book_id->LookupFilterQuery(FALSE, "view") ?>";
fapp_subscribeview.Lists["x_pkg_id"] = {"LinkField":"x_pkg_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_pkg_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_tips_package"};
fapp_subscribeview.Lists["x_pkg_id"].Data = "<?php echo $app_subscribe_view->pkg_id->LookupFilterQuery(FALSE, "view") ?>";
fapp_subscribeview.Lists["x_tcat_id"] = {"LinkField":"x_tcat_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_tcat_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_tips_category"};
fapp_subscribeview.Lists["x_tcat_id"].Data = "<?php echo $app_subscribe_view->tcat_id->LookupFilterQuery(FALSE, "view") ?>";
fapp_subscribeview.Lists["x_test_id"] = {"LinkField":"x_test_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_test_iname","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_test"};
fapp_subscribeview.Lists["x_test_id"].Data = "<?php echo $app_subscribe_view->test_id->LookupFilterQuery(FALSE, "view") ?>";
fapp_subscribeview.Lists["x_cat_id"] = {"LinkField":"x_cat_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_cat_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_consultation_category"};
fapp_subscribeview.Lists["x_cat_id"].Data = "<?php echo $app_subscribe_view->cat_id->LookupFilterQuery(FALSE, "view") ?>";
fapp_subscribeview.Lists["x_cons_id"] = {"LinkField":"x_cons_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_cons_amount","x_cons_message","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_consultation"};
fapp_subscribeview.Lists["x_cons_id"].Data = "<?php echo $app_subscribe_view->cons_id->LookupFilterQuery(FALSE, "view") ?>";
...
<div class="ewToolbar">
<?php $app_subscribe_view->ExportOptions->Render("body") ?>
<?php
	foreach ($app_subscribe_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php $app_subscribe_view->ShowPageHeader(); ?>
<?php
$app_subscribe_view->ShowMessage();
?>
<?php if (!$app_subscribe_view->IsModal) { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($app_subscribe_view->Pager)) $app_subscribe_view->Pager = new cPrevNextPager($app_subscribe_view->StartRec, $app_subscribe_view->DisplayRecs, $app_subscribe_view->TotalRecs, $app_subscribe_view->AutoHidePager) ?>
<?php if ($app_subscribe_view->Pager->RecordCount > 0 && $app_subscribe_view->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($app_subscribe_view->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $app_subscribe_view->PageUrl() ?>start=<?php echo $app_subscribe_view->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($app_subscribe_view->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $app_subscribe_view->PageUrl() ?>start=<?php echo $app_subscribe_view->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $app_subscribe_view->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($app_subscribe_view->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $app_subscribe_view->PageUrl() ?>start=<?php echo $app_subscribe_view->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($app_subscribe_view->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $app_subscribe_view->PageUrl() ?>start=<?php echo $app_subscribe_view->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $app_subscribe_view->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<form name="fapp_subscribeview" id="fapp_subscribeview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_subscribe_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_subscribe_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_subscribe">
<input type="hidden" name="modal" value="<?php echo intval($app_subscribe_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($app_subscribe->serv_id->Visible) { // serv_id ?>
	<tr id="r_serv_id">
		<td class="col-sm-2"><span id="elh_app_subscribe_serv_id"><?php echo $app_subscribe->serv_id->FldCaption() ?></span></td>
...
<?php echo $app_subscribe->serv_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_subscribe->user_id->Visible) { // user_id ?>
	<tr id="r_user_id">
		<td class="col-sm-2"><span id="elh_app_subscribe_user_id"><?php echo $app_subscribe->user_id->FldCaption() ?></span></td>
...
<?php echo $app_subscribe->user_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_subscribe->cycle_id->Visible) { // cycle_id ?>
	<tr id="r_cycle_id">
		<td class="col-sm-2"><span id="elh_app_subscribe_cycle_id"><?php echo $app_subscribe->cycle_id->FldCaption() ?></span></td>
...
<?php echo $app_subscribe->cycle_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_subscribe->book_id->Visible) { // book_id ?>
	<tr id="r_book_id">
		<td class="col-sm-2"><span id="elh_app_subscribe_book_id"><?php echo $app_subscribe->book_id->FldCaption() ?></span></td>
...
<?php echo $app_subscribe->book_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_subscribe->pkg_id->Visible) { // pkg_id ?>
	<tr id="r_pkg_id">
		<td class="col-sm-2"><span id="elh_app_subscribe_pkg_id"><?php echo $app_subscribe->pkg_id->FldCaption() ?></span></td>
		<td data-name="pkg_id"<?php echo $app_subscribe->pkg_id->CellAttributes() ?>>
<span id="el_app_subscribe_pkg_id">
<span<?php echo $app_subscribe->pkg_id->ViewAttributes() ?>>
<?php echo $app_subscribe->pkg_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_subscribe->tcat_id->Visible) { // tcat_id ?>
	<tr id="r_tcat_id">
		<td class="col-sm-2"><span id="elh_app_subscribe_tcat_id"><?php echo $app_subscribe->tcat_id->FldCaption() ?></span></td>
...
<?php echo $app_subscribe->tcat_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_subscribe->test_id->Visible) { // test_id ?>
	<tr id="r_test_id">
		<td class="col-sm-2"><span id="elh_app_subscribe_test_id"><?php echo $app_subscribe->test_id->FldCaption() ?></span></td>
...
<?php echo $app_subscribe->test_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_subscribe->cat_id->Visible) { // cat_id ?>
	<tr id="r_cat_id">
		<td class="col-sm-2"><span id="elh_app_subscribe_cat_id"><?php echo $app_subscribe->cat_id->FldCaption() ?></span></td>
...
<?php echo $app_subscribe->cat_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_subscribe->cons_id->Visible) { // cons_id ?>
	<tr id="r_cons_id">
		<td class="col-sm-2"><span id="elh_app_subscribe_cons_id"><?php echo $app_subscribe->cons_id->FldCaption() ?></span></td>
...
<?php echo $app_subscribe->cons_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_subscribe->subs_start->Visible) { // subs_start ?>
	<tr id="r_subs_start">
		<td class="col-sm-2"><span id="elh_app_subscribe_subs_start"><?php echo $app_subscribe->subs_start->FldCaption() ?></span></td>
...
<?php echo $app_subscribe->subs_start->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_subscribe->subs_end->Visible) { // subs_end ?>
	<tr id="r_subs_end">
		<td class="col-sm-2"><span id="elh_app_subscribe_subs_end"><?php echo $app_subscribe->subs_end->FldCaption() ?></span></td>
...
<?php echo $app_subscribe->subs_end->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_subscribe->subs_qnt->Visible) { // subs_qnt ?>
	<tr id="r_subs_qnt">
		<td class="col-sm-2"><span id="elh_app_subscribe_subs_qnt"><?php echo $app_subscribe->subs_qnt->FldCaption() ?></span></td>
...
<?php echo $app_subscribe->subs_qnt->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_subscribe->subs_price->Visible) { // subs_price ?>
	<tr id="r_subs_price">
		<td class="col-sm-2"><span id="elh_app_subscribe_subs_price"><?php echo $app_subscribe->subs_price->FldCaption() ?></span></td>
...
<?php echo $app_subscribe->subs_price->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_subscribe->subs_amt->Visible) { // subs_amt ?>
	<tr id="r_subs_amt">
		<td class="col-sm-2"><span id="elh_app_subscribe_subs_amt"><?php echo $app_subscribe->subs_amt->FldCaption() ?></span></td>
...
<?php echo $app_subscribe->subs_amt->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_subscribe->ins_datetime->Visible) { // ins_datetime ?>
	<tr id="r_ins_datetime">
		<td class="col-sm-2"><span id="elh_app_subscribe_ins_datetime"><?php echo $app_subscribe->ins_datetime->FldCaption() ?></span></td>
...
<?php echo $app_subscribe->ins_datetime->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php } ?>
<?php
	if (in_array("app_subscribe_cats", explode(",", $app_subscribe->getCurrentDetailTable())) && $app_subscribe_cats->DetailView) {
?>
...
<script type="text/javascript">
fapp_subscribeview.Init();
</script>
<?php
$app_subscribe_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$app_subscribe_view->Page_Terminate();
?>
