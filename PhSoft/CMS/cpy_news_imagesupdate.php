...
$cpy_news_images_update = NULL; // Initialize page object first

class ccpy_news_images_update extends ccpy_news_images {
	...
	// Page object name
	var $PageObjName = 'cpy_news_images_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("cpy_news_imageslist.php"); // No records selected, return to list
	...
	function LoadMultiUpdateValues() {
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					$this->news_id->setDbValue($this->Recordset->fields('news_id'));
					$this->nimg_order->setDbValue($this->Recordset->fields('nimg_order'));
				} else {
					if (!ew_CompareValue($this->news_id->DbValue, $this->Recordset->fields('news_id')))
						$this->news_id->CurrentValue = NULL;
					if (!ew_CompareValue($this->nimg_order->DbValue, $this->Recordset->fields('nimg_order')))
						$this->nimg_order->CurrentValue = NULL;
				}
				$i++;
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->nimg_id->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function GetUploadFiles() {
		...
		$this->nimg_photo->MultiUpdate = $objForm->GetValue("u_nimg_photo");
	}
	...
	function LoadFormValues() {
		...
		$this->news_id->MultiUpdate = $objForm->GetValue("u_news_id");
		if (!$this->nimg_order->FldIsDetailKey) {
			$this->nimg_order->setFormValue($objForm->GetValue("x_nimg_order"));
		}
		$this->nimg_order->MultiUpdate = $objForm->GetValue("u_nimg_order");
	...
	function ValidateForm() {
		...
		$gsFormError = "";
		$lUpdateCnt = 0;
		if ($this->news_id->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->nimg_order->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->nimg_photo->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if ($this->news_id->MultiUpdate <> "" && !$this->news_id->FldIsDetailKey && !is_null($this->news_id->FormValue) && $this->news_id->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->news_id->FldCaption(), $this->news_id->ReqErrMsg));
		}
		if ($this->nimg_order->MultiUpdate <> "" && !$this->nimg_order->FldIsDetailKey && !is_null($this->nimg_order->FormValue) && $this->nimg_order->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->nimg_order->FldCaption(), $this->nimg_order->ReqErrMsg));
		}
		if ($this->nimg_order->MultiUpdate <> "") {
			if (!ew_CheckInteger($this->nimg_order->FormValue)) {
				ew_AddMessage($gsFormError, $this->nimg_order->FldErrMsg());
			}
		}
		if ($this->nimg_photo->MultiUpdate <> "" && $this->nimg_photo->Upload->FileName == "" && !$this->nimg_photo->Upload->KeepFile) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// news_id
			$this->news_id->SetDbValueDef($rsnew, $this->news_id->CurrentValue, 0, $this->news_id->ReadOnly || $this->news_id->MultiUpdate <> "1");

			// nimg_order
			$this->nimg_order->SetDbValueDef($rsnew, $this->nimg_order->CurrentValue, 0, $this->nimg_order->ReadOnly || $this->nimg_order->MultiUpdate <> "1");

			// nimg_photo
			if ($this->nimg_photo->Visible && !$this->nimg_photo->ReadOnly && strval($this->nimg_photo->MultiUpdate) == "1" && !$this->nimg_photo->Upload->KeepFile) {
			...
			if ($this->nimg_photo->Visible && !$this->nimg_photo->Upload->KeepFile) {
				...
				if (!ew_Empty($this->nimg_photo->Upload->FileName) && $this->UpdateCount == 1) {
					...
					$this->nimg_photo->SetDbValueDef($rsnew, $this->nimg_photo->Upload->FileName, "", $this->nimg_photo->ReadOnly || $this->nimg_photo->MultiUpdate <> "1");
				}
			}
...
// Create page object
if (!isset($cpy_news_images_update)) $cpy_news_images_update = new ccpy_news_images_update();

// Page init
$cpy_news_images_update->Page_Init();

// Page main
$cpy_news_images_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cpy_news_images_update->Page_Render();
?>
...
var CurrentForm = fcpy_news_imagesupdate = new ew_Form("fcpy_news_imagesupdate", "update");

// Validate form
fcpy_news_imagesupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_news_id");
			...
			uelm = this.GetElements("u" + infix + "_nimg_order");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
			...
			uelm = this.GetElements("u" + infix + "_nimg_photo");
			if (uelm && uelm.checked) {
...
fcpy_news_imagesupdate.Form_CustomValidate = 
...
fcpy_news_imagesupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fcpy_news_imagesupdate.Lists["x_news_id"] = {"LinkField":"x_news_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_news_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cpy_news"};
fcpy_news_imagesupdate.Lists["x_news_id"].Data = "<?php echo $cpy_news_images_update->news_id->LookupFilterQuery(FALSE, "update") ?>";
...
<?php $cpy_news_images_update->ShowPageHeader(); ?>
<?php
$cpy_news_images_update->ShowMessage();
?>
<form name="fcpy_news_imagesupdate" id="fcpy_news_imagesupdate" class="<?php echo $cpy_news_images_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cpy_news_images_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cpy_news_images_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="cpy_news_images">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($cpy_news_images_update->IsModal) ?>">
<?php foreach ($cpy_news_images_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_cpy_news_imagesupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($cpy_news_images->news_id->Visible) { // news_id ?>
	<div id="r_news_id" class="form-group">
		<label for="x_news_id" class="<?php echo $cpy_news_images_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_news_id" id="u_news_id" class="ewMultiSelect" value="1"<?php echo ($cpy_news_images->news_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $cpy_news_images->news_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $cpy_news_images_update->RightColumnClass ?>"><div<?php echo $cpy_news_images->news_id->CellAttributes() ?>>
...
<?php if ($cpy_news_images->nimg_order->Visible) { // nimg_order ?>
	<div id="r_nimg_order" class="form-group">
		<label for="x_nimg_order" class="<?php echo $cpy_news_images_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_nimg_order" id="u_nimg_order" class="ewMultiSelect" value="1"<?php echo ($cpy_news_images->nimg_order->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $cpy_news_images->nimg_order->FldCaption() ?></label></div></label>
		<div class="<?php echo $cpy_news_images_update->RightColumnClass ?>"><div<?php echo $cpy_news_images->nimg_order->CellAttributes() ?>>
...
<?php if ($cpy_news_images->nimg_photo->Visible) { // nimg_photo ?>
	<div id="r_nimg_photo" class="form-group">
		<label class="<?php echo $cpy_news_images_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_nimg_photo" id="u_nimg_photo" class="ewMultiSelect" value="1"<?php echo ($cpy_news_images->nimg_photo->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $cpy_news_images->nimg_photo->FldCaption() ?></label></div></label>
		<div class="<?php echo $cpy_news_images_update->RightColumnClass ?>"><div<?php echo $cpy_news_images->nimg_photo->CellAttributes() ?>>
...
<?php if (!$cpy_news_images_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $cpy_news_images_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cpy_news_images_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
	</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fcpy_news_imagesupdate.Init();
</script>
<?php
$cpy_news_images_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cpy_news_images_update->Page_Terminate();
?>
