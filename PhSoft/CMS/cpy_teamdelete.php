...
$cpy_team_delete = NULL; // Initialize page object first

class ccpy_team_delete extends ccpy_team {
	...
	// Page object name
	var $PageObjName = 'cpy_team_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("cpy_teamlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in cpy_team class, cpy_teaminfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("cpy_teamlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($cpy_team_delete)) $cpy_team_delete = new ccpy_team_delete();

// Page init
$cpy_team_delete->Page_Init();

// Page main
$cpy_team_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cpy_team_delete->Page_Render();
?>
...
var CurrentForm = fcpy_teamdelete = new ew_Form("fcpy_teamdelete", "delete");

// Form_CustomValidate event
fcpy_teamdelete.Form_CustomValidate = 
...
fcpy_teamdelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $cpy_team_delete->ShowPageHeader(); ?>
<?php
$cpy_team_delete->ShowMessage();
?>
<form name="fcpy_teamdelete" id="fcpy_teamdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cpy_team_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cpy_team_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="cpy_team">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($cpy_team_delete->RecKeys as $key) { ?>
...
<?php if ($cpy_team->team_iname->Visible) { // team_iname ?>
		<th class="<?php echo $cpy_team->team_iname->HeaderCellClass() ?>"><span id="elh_cpy_team_team_iname" class="cpy_team_team_iname"><?php echo $cpy_team->team_iname->FldCaption() ?></span></th>
<?php } ?>
<?php if ($cpy_team->team_order->Visible) { // team_order ?>
		<th class="<?php echo $cpy_team->team_order->HeaderCellClass() ?>"><span id="elh_cpy_team_team_order" class="cpy_team_team_order"><?php echo $cpy_team->team_order->FldCaption() ?></span></th>
<?php } ?>
<?php if ($cpy_team->team_photo->Visible) { // team_photo ?>
		<th class="<?php echo $cpy_team->team_photo->HeaderCellClass() ?>"><span id="elh_cpy_team_team_photo" class="cpy_team_team_photo"><?php echo $cpy_team->team_photo->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$cpy_team_delete->RecCnt = 0;
$i = 0;
while (!$cpy_team_delete->Recordset->EOF) {
	$cpy_team_delete->RecCnt++;
	$cpy_team_delete->RowCnt++;

	// Set row properties
	$cpy_team->ResetAttrs();
	$cpy_team->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$cpy_team_delete->LoadRowValues($cpy_team_delete->Recordset);

	// Render row
	$cpy_team_delete->RenderRow();
?>
	<tr<?php echo $cpy_team->RowAttributes() ?>>
<?php if ($cpy_team->team_iname->Visible) { // team_iname ?>
		<td<?php echo $cpy_team->team_iname->CellAttributes() ?>>
<span id="el<?php echo $cpy_team_delete->RowCnt ?>_cpy_team_team_iname" class="cpy_team_team_iname">
...
<span id="el<?php echo $cpy_team_delete->RowCnt ?>_cpy_team_team_order" class="cpy_team_team_order">
...
<span id="el<?php echo $cpy_team_delete->RowCnt ?>_cpy_team_team_photo" class="cpy_team_team_photo">
<span>
...
<?php
	$cpy_team_delete->Recordset->MoveNext();
}
$cpy_team_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cpy_team_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fcpy_teamdelete.Init();
</script>
<?php
$cpy_team_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cpy_team_delete->Page_Terminate();
?>
