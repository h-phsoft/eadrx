...
$cpy_faq_grid = NULL; // Initialize page object first

class ccpy_faq_grid extends ccpy_faq {
	...
	// Page object name
	var $PageObjName = 'cpy_faq_grid';

	// Grid form hidden field names
	var $FormName = 'fcpy_faqgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["cpy_faq"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->faq_id->CurrentValue = strval($arKeys[0]); // faq_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "cpy_faq_category") {
				$this->fcat_id->CurrentValue = $this->fcat_id->getSessionValue();
			}
		$conn = &$this->Connection();
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "cpy_faq_category") {
			$this->fcat_id->Visible = FALSE;
			if ($GLOBALS["cpy_faq_category"]->EventCancelled) $this->EventCancelled = TRUE;
		}
