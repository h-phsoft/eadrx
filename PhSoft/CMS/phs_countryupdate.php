...
$phs_country_update = NULL; // Initialize page object first

class cphs_country_update extends cphs_country {
	...
	// Page object name
	var $PageObjName = 'phs_country_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("phs_countrylist.php"); // No records selected, return to list
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					...
					$this->cntry_code->setDbValue($this->Recordset->fields('cntry_code'));
					$this->cntry_name->setDbValue($this->Recordset->fields('cntry_name'));
				} else {
					if (!ew_CompareValue($this->status_id->DbValue, $this->Recordset->fields('status_id')))
						$this->status_id->CurrentValue = NULL;
					if (!ew_CompareValue($this->cntry_code->DbValue, $this->Recordset->fields('cntry_code')))
						$this->cntry_code->CurrentValue = NULL;
					if (!ew_CompareValue($this->cntry_name->DbValue, $this->Recordset->fields('cntry_name')))
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->cntry_id->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->cntry_code->MultiUpdate = $objForm->GetValue("u_cntry_code");
		if (!$this->cntry_name->FldIsDetailKey) {
			$this->cntry_name->setFormValue($objForm->GetValue("x_cntry_name"));
		}
		$this->cntry_name->MultiUpdate = $objForm->GetValue("u_cntry_name");
	...
	function ValidateForm() {
		...
		if ($this->cntry_code->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->cntry_name->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
		...
		if ($this->status_id->MultiUpdate <> "") {
			if (!ew_CheckInteger($this->status_id->FormValue)) {
				ew_AddMessage($gsFormError, $this->status_id->FldErrMsg());
			}
		}
		if ($this->cntry_code->MultiUpdate <> "" && !$this->cntry_code->FldIsDetailKey && !is_null($this->cntry_code->FormValue) && $this->cntry_code->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->cntry_code->FldCaption(), $this->cntry_code->ReqErrMsg));
		}
		if ($this->cntry_name->MultiUpdate <> "" && !$this->cntry_name->FldIsDetailKey && !is_null($this->cntry_name->FormValue) && $this->cntry_name->FormValue == "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// cntry_code
			$this->cntry_code->SetDbValueDef($rsnew, $this->cntry_code->CurrentValue, "", $this->cntry_code->ReadOnly || $this->cntry_code->MultiUpdate <> "1");

			// cntry_name
			$this->cntry_name->SetDbValueDef($rsnew, $this->cntry_name->CurrentValue, "", $this->cntry_name->ReadOnly || $this->cntry_name->MultiUpdate <> "1");
...
// Create page object
if (!isset($phs_country_update)) $phs_country_update = new cphs_country_update();

// Page init
$phs_country_update->Page_Init();

// Page main
$phs_country_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$phs_country_update->Page_Render();
?>
...
var CurrentForm = fphs_countryupdate = new ew_Form("fphs_countryupdate", "update");

// Validate form
fphs_countryupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_cntry_code");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
			...
			uelm = this.GetElements("u" + infix + "_cntry_name");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
...
fphs_countryupdate.Form_CustomValidate = 
...
fphs_countryupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fphs_countryupdate.Lists["x_status_id"] = {"LinkField":"x_status_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_status_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_status"};
fphs_countryupdate.Lists["x_status_id"].Data = "<?php echo $phs_country_update->status_id->LookupFilterQuery(FALSE, "update") ?>";
fphs_countryupdate.AutoSuggests["x_status_id"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $phs_country_update->status_id->LookupFilterQuery(TRUE, "update"))) ?>;
...
<?php $phs_country_update->ShowPageHeader(); ?>
<?php
$phs_country_update->ShowMessage();
?>
<form name="fphs_countryupdate" id="fphs_countryupdate" class="<?php echo $phs_country_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($phs_country_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $phs_country_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="phs_country">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($phs_country_update->IsModal) ?>">
<?php foreach ($phs_country_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_phs_countryupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($phs_country->status_id->Visible) { // status_id ?>
	<div id="r_status_id" class="form-group">
		<label class="<?php echo $phs_country_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_status_id" id="u_status_id" class="ewMultiSelect" value="1"<?php echo ($phs_country->status_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_country->status_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_country_update->RightColumnClass ?>"><div<?php echo $phs_country->status_id->CellAttributes() ?>>
...
<script type="text/javascript">
fphs_countryupdate.CreateAutoSuggest({"id":"x_status_id","forceSelect":false});
</script>
</span>
...
<?php if ($phs_country->cntry_code->Visible) { // cntry_code ?>
	<div id="r_cntry_code" class="form-group">
		<label for="x_cntry_code" class="<?php echo $phs_country_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_cntry_code" id="u_cntry_code" class="ewMultiSelect" value="1"<?php echo ($phs_country->cntry_code->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_country->cntry_code->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_country_update->RightColumnClass ?>"><div<?php echo $phs_country->cntry_code->CellAttributes() ?>>
...
<?php if ($phs_country->cntry_name->Visible) { // cntry_name ?>
	<div id="r_cntry_name" class="form-group">
		<label for="x_cntry_name" class="<?php echo $phs_country_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_cntry_name" id="u_cntry_name" class="ewMultiSelect" value="1"<?php echo ($phs_country->cntry_name->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_country->cntry_name->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_country_update->RightColumnClass ?>"><div<?php echo $phs_country->cntry_name->CellAttributes() ?>>
...
<?php if (!$phs_country_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $phs_country_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $phs_country_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
	</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fphs_countryupdate.Init();
</script>
<?php
$phs_country_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$phs_country_update->Page_Terminate();
?>
