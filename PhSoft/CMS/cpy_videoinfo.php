...
class ccpy_video extends cTable {
	...
	function __construct() {
		...
		// video_name
		$this->video_name = new cField('cpy_video', 'cpy_video', 'x_video_name', 'video_name', '`video_name`', '`video_name`', 200, -1, FALSE, '`video_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->video_name->Sortable = TRUE; // Allow sort
		$this->fields['video_name'] = &$this->video_name;
	...
	function LoadListRowValues(&$rs) {
		...
		$this->video_name->setDbValue($rs->fields('video_name'));
	...
	function RenderEditRow() {
		...
		$this->video_name->EditValue = $this->video_name->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->video_name->Exportable) $Doc->ExportCaption($this->video_name);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->video_name->Exportable) $Doc->ExportField($this->video_name);
