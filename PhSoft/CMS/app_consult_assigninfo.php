...
$app_consult_assign = NULL;

//
// Table class for app_consult_assign
//
class capp_consult_assign extends cTable {
	var $assign_id;
	var $user_id;
	var $cons_id;
	var $status_id;
	...
	function __construct() {
		...
		$this->TableVar = 'app_consult_assign';
		$this->TableName = 'app_consult_assign';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`app_consult_assign`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// assign_id
		$this->assign_id = new cField('app_consult_assign', 'app_consult_assign', 'x_assign_id', 'assign_id', '`assign_id`', '`assign_id`', 3, -1, FALSE, '`assign_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->assign_id->Sortable = FALSE; // Allow sort
		$this->assign_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['assign_id'] = &$this->assign_id;

		// user_id
		$this->user_id = new cField('app_consult_assign', 'app_consult_assign', 'x_user_id', 'user_id', '`user_id`', '`user_id`', 3, -1, FALSE, '`user_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// cons_id
		$this->cons_id = new cField('app_consult_assign', 'app_consult_assign', 'x_cons_id', 'cons_id', '`cons_id`', '`cons_id`', 3, -1, FALSE, '`cons_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// status_id
		$this->status_id = new cField('app_consult_assign', 'app_consult_assign', 'x_status_id', 'status_id', '`status_id`', '`status_id`', 16, -1, FALSE, '`status_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// ins_datetime
		$this->ins_datetime = new cField('app_consult_assign', 'app_consult_assign', 'x_ins_datetime', 'ins_datetime', '`ins_datetime`', ew_CastDateFieldForLike('`ins_datetime`', 11, "DB"), 135, 11, FALSE, '`ins_datetime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`app_consult_assign`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->assign_id->setDbValue($conn->Insert_ID());
			$rs['assign_id'] = $this->assign_id->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('assign_id', $rs))
				ew_AddFilter($where, ew_QuotedName('assign_id', $this->DBID) . '=' . ew_QuotedValue($rs['assign_id'], $this->assign_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->assign_id->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->assign_id->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@assign_id@", ew_AdjustSql($this->assign_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "app_consult_assignlist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "app_consult_assignedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "app_consult_assignadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("app_consult_assignview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("app_consult_assignview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "app_consult_assignadd.php?" . $this->UrlParm($parm);
		else
			$url = "app_consult_assignadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("app_consult_assignedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("app_consult_assignadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("app_consult_assigndelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "assign_id:" . ew_VarToJson($this->assign_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->assign_id->CurrentValue)) {
			$sUrl .= "assign_id=" . urlencode($this->assign_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["assign_id"]))
				$arKeys[] = $_POST["assign_id"];
			elseif (isset($_GET["assign_id"]))
				$arKeys[] = $_GET["assign_id"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->assign_id->CurrentValue = $key;
	...
	function LoadListRowValues(&$rs) {
		$this->assign_id->setDbValue($rs->fields('assign_id'));
	...
	// Common render codes
		...
		$this->assign_id->ViewValue = $this->assign_id->CurrentValue;
		$this->assign_id->ViewCustomAttributes = "";
		...
		// assign_id
		$this->assign_id->LinkCustomAttributes = "";
		$this->assign_id->HrefValue = "";
		$this->assign_id->TooltipValue = "";
	...
	function RenderEditRow() {
		...
		// assign_id
		$this->assign_id->EditAttrs["class"] = "form-control";
		$this->assign_id->EditCustomAttributes = "";
		$this->assign_id->EditValue = $this->assign_id->CurrentValue;
		$this->assign_id->ViewCustomAttributes = "";
