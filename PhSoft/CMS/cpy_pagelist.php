...
class ccpy_page_list extends ccpy_page {
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_page_stext") && $objForm->HasValue("o_page_stext") && $this->page_stext->CurrentValue <> $this->page_stext->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_page_desc") && $objForm->HasValue("o_page_desc") && $this->page_desc->CurrentValue <> $this->page_desc->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function SetupSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->page_desc); // page_desc
	...
	function SetupListOptions() {
		...
		// "detail_cpy_page_row"
		$item = &$this->ListOptions->Add("detail_cpy_page_row");
		$item->CssClass = "text-nowrap";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'cpy_page_row') && !$this->ShowMultipleDetails;
		$item->OnLeft = TRUE;
	...
	function RenderListOptions() {
		...
		// "detail_cpy_page_row"
		$oListOpt = &$this->ListOptions->Items["detail_cpy_page_row"];
		if ($Security->AllowList(CurrentProjectID() . 'cpy_page_row')) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("cpy_page_row", "TblCaption");
			$body .= "&nbsp;" . str_replace("%c", $this->cpy_page_row_Count, $Language->Phrase("DetailCount"));
			$body = "<a class=\"btn btn-default btn-sm ewRowLink ewDetail\" data-action=\"list\" href=\"" . ew_HtmlEncode("cpy_page_rowlist.php?" . EW_TABLE_SHOW_MASTER . "=cpy_page&fk_page_id=" . urlencode(strval($this->page_id->CurrentValue)) . "") . "\">" . $body . "</a>";
			$links = "";
			if ($GLOBALS["cpy_page_row_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'cpy_page_row')) {
				$caption = $Language->Phrase("MasterDetailViewLink");
				$url = $this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=cpy_page_row");
				...
				$DetailViewTblVar .= "cpy_page_row";
			}
			if ($GLOBALS["cpy_page_row_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'cpy_page_row')) {
				$caption = $Language->Phrase("MasterDetailEditLink");
				$url = $this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=cpy_page_row");
				...
				$DetailEditTblVar .= "cpy_page_row";
			}
			if ($GLOBALS["cpy_page_row_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'cpy_page_row')) {
				$caption = $Language->Phrase("MasterDetailCopyLink");
				$url = $this->GetCopyUrl(EW_TABLE_SHOW_DETAIL . "=cpy_page_row");
	...
	function SetupOtherOptions() {
		...
		$DetailTableLink = "";
		$item = &$option->Add("detailadd_cpy_page_row");
		$url = $this->GetAddUrl(EW_TABLE_SHOW_DETAIL . "=cpy_page_row");
		$caption = $Language->Phrase("Add") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["cpy_page_row"]->TableCaption();
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" title=\"" . ew_HtmlTitle($caption) . "\" data-caption=\"" . ew_HtmlTitle($caption) . "\" href=\"" . ew_HtmlEncode($url) . "\">" . $caption . "</a>";
		$item->Visible = ($GLOBALS["cpy_page_row"]->DetailAdd && $Security->AllowAdd(CurrentProjectID() . 'cpy_page_row') && $Security->CanAdd());
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "cpy_page_row";
		}
		...
		$item->Body = "<a class=\"ewAction ewMultiUpdate\" title=\"" . ew_HtmlTitle($Language->Phrase("UpdateSelectedLink")) . "\" data-table=\"cpy_page\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("UpdateSelectedLink")) . "\" href=\"\" onclick=\"ew_SubmitAction(event,{f:document.fcpy_pagelist,url:'" . $this->MultiUpdateUrl . "'});return false;\">" . $Language->Phrase("UpdateSelectedLink") . "</a>";
		$item->Visible = ($Security->CanEdit());
	...
	function LoadFormValues() {
		...
		$this->page_stext->setOldValue($objForm->GetValue("o_page_stext"));
		if (!$this->page_desc->FldIsDetailKey) {
			$this->page_desc->setFormValue($objForm->GetValue("x_page_desc"));
		}
		$this->page_desc->setOldValue($objForm->GetValue("o_page_desc"));
	...
	function LoadRowValues($rs = NULL) {
		...
		if (!isset($GLOBALS["cpy_page_row_grid"])) $GLOBALS["cpy_page_row_grid"] = new ccpy_page_row_grid;
		$sDetailFilter = $GLOBALS["cpy_page_row"]->SqlDetailFilter_cpy_page();
		$sDetailFilter = str_replace("@page_id@", ew_AdjustSql($this->page_id->DbValue, "DB"), $sDetailFilter);
		$GLOBALS["cpy_page_row"]->setCurrentMasterTable("cpy_page");
		$sDetailFilter = $GLOBALS["cpy_page_row"]->ApplyUserIDFilters($sDetailFilter);
		$this->cpy_page_row_Count = $GLOBALS["cpy_page_row"]->LoadRecordCount($sDetailFilter);
	}
	...
	function RenderRow() {
		...
		$this->page_desc->ViewCustomAttributes = "";
			...
			if ($this->Export == "")
				$this->page_desc->ViewValue = $this->HighlightValue($this->page_desc);
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_cpy_page\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_cpy_page',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fcpy_pagelist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Check empty row
fcpy_pagelist.EmptyRow = function(infix) {
	var fobj = this.Form;
	...
	if (ew_ValueChanged(fobj, infix, "page_stext", false)) return false;
	if (ew_ValueChanged(fobj, infix, "page_desc", false)) return false;
	return true;
}
...
fcpy_pagelist.Lists["x_status_id"] = {"LinkField":"x_status_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_status_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_status"};
fcpy_pagelist.Lists["x_status_id"].Data = "<?php echo $cpy_page_list->status_id->LookupFilterQuery(FALSE, "list") ?>";
...
if ($cpy_page->CurrentAction == "gridadd") {
	$cpy_page->CurrentFilter = "0=1";
	$cpy_page_list->StartRec = 1;
	$cpy_page_list->DisplayRecs = $cpy_page->GridAddRowCount;
	$cpy_page_list->TotalRecs = $cpy_page_list->DisplayRecs;
	$cpy_page_list->StopRec = $cpy_page_list->DisplayRecs;
} else {
...
<?php } ?>
<?php if ($cpy_page_list->Pager->RecordCount > 0) { ?>
...
<?php if ($cpy_page->status_id->Visible) { // status_id ?>
	<?php if ($cpy_page->SortUrl($cpy_page->status_id) == "") { ?>
		<th data-name="status_id" class="<?php echo $cpy_page->status_id->HeaderCellClass() ?>"><div id="elh_cpy_page_status_id" class="cpy_page_status_id"><div class="ewTableHeaderCaption"><?php echo $cpy_page->status_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="status_id" class="<?php echo $cpy_page->status_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $cpy_page->SortUrl($cpy_page->status_id) ?>',1);"><div id="elh_cpy_page_status_id" class="cpy_page_status_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $cpy_page->status_id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($cpy_page->status_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($cpy_page->status_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
...
<?php if ($cpy_page->page_desc->Visible) { // page_desc ?>
	<?php if ($cpy_page->SortUrl($cpy_page->page_desc) == "") { ?>
		<th data-name="page_desc" class="<?php echo $cpy_page->page_desc->HeaderCellClass() ?>"><div id="elh_cpy_page_page_desc" class="cpy_page_page_desc"><div class="ewTableHeaderCaption"><?php echo $cpy_page->page_desc->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="page_desc" class="<?php echo $cpy_page->page_desc->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $cpy_page->SortUrl($cpy_page->page_desc) ?>',1);"><div id="elh_cpy_page_page_desc" class="cpy_page_page_desc">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $cpy_page->page_desc->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($cpy_page->page_desc->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($cpy_page->page_desc->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
<?php
	if ($cpy_page->CurrentAction == "add" || $cpy_page->CurrentAction == "copy") {
		...
		if ($cpy_page->CurrentAction == "copy" && !$cpy_page_list->LoadRow())
			$cpy_page->CurrentAction = "add";
...
<span id="el<?php echo $cpy_page_list->RowCnt ?>_cpy_page_status_id" class="form-group cpy_page_status_id">
<select data-table="cpy_page" data-field="x_status_id" data-value-separator="<?php echo $cpy_page->status_id->DisplayValueSeparatorAttribute() ?>" id="x<?php echo $cpy_page_list->RowIndex ?>_status_id" name="x<?php echo $cpy_page_list->RowIndex ?>_status_id"<?php echo $cpy_page->status_id->EditAttributes() ?>>
<?php echo $cpy_page->status_id->SelectOptionListHtml("x<?php echo $cpy_page_list->RowIndex ?>_status_id") ?>
</select>
</span>
<input type="hidden" data-table="cpy_page" data-field="x_status_id" name="o<?php echo $cpy_page_list->RowIndex ?>_status_id" id="o<?php echo $cpy_page_list->RowIndex ?>_status_id" value="<?php echo ew_HtmlEncode($cpy_page->status_id->OldValue) ?>">
</td>
	<?php } ?>
	<?php if ($cpy_page->slid_id->Visible) { // slid_id ?>
...
<input type="text" data-table="cpy_page" data-field="x_page_name" name="x<?php echo $cpy_page_list->RowIndex ?>_page_name" id="x<?php echo $cpy_page_list->RowIndex ?>_page_name" size="30" maxlength="200" placeholder="<?php echo ew_HtmlEncode($cpy_page->page_name->getPlaceHolder()) ?>" value="<?php echo $cpy_page->page_name->EditValue ?>"<?php echo $cpy_page->page_name->EditAttributes() ?>>
</span>
...
<input type="hidden" data-table="cpy_page" data-field="x_page_stext" name="o<?php echo $cpy_page_list->RowIndex ?>_page_stext" id="o<?php echo $cpy_page_list->RowIndex ?>_page_stext" value="<?php echo ew_HtmlEncode($cpy_page->page_stext->OldValue) ?>">
	...
	<?php } ?>
	<?php if ($cpy_page->page_desc->Visible) { // page_desc ?>
		<td data-name="page_desc">
<span id="el<?php echo $cpy_page_list->RowCnt ?>_cpy_page_page_desc" class="form-group cpy_page_page_desc">
<textarea data-table="cpy_page" data-field="x_page_desc" name="x<?php echo $cpy_page_list->RowIndex ?>_page_desc" id="x<?php echo $cpy_page_list->RowIndex ?>_page_desc" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($cpy_page->page_desc->getPlaceHolder()) ?>"<?php echo $cpy_page->page_desc->EditAttributes() ?>><?php echo $cpy_page->page_desc->EditValue ?></textarea>
</span>
<input type="hidden" data-table="cpy_page" data-field="x_page_desc" name="o<?php echo $cpy_page_list->RowIndex ?>_page_desc" id="o<?php echo $cpy_page_list->RowIndex ?>_page_desc" value="<?php echo ew_HtmlEncode($cpy_page->page_desc->OldValue) ?>">
</td>
	<?php } ?>
<?php
...
while ($cpy_page_list->RecCnt < $cpy_page_list->StopRec) {
	...
	if (intval($cpy_page_list->RecCnt) >= intval($cpy_page_list->StartRec)) {
		...
		if ($cpy_page->CurrentAction == "gridadd") // Grid add
			$cpy_page->RowType = EW_ROWTYPE_ADD; // Render add
		if ($cpy_page->CurrentAction == "gridadd" && $cpy_page->EventCancelled && !$objForm->HasValue("k_blankrow")) // Insert failed
...
?>
	<?php if ($cpy_page->status_id->Visible) { // status_id ?>
		<td data-name="status_id"<?php echo $cpy_page->status_id->CellAttributes() ?>>
...
<span id="el<?php echo $cpy_page_list->RowCnt ?>_cpy_page_status_id" class="cpy_page_status_id">
...
<?php } ?>
<?php if ($cpy_page->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $cpy_page_list->RowCnt ?>_cpy_page_page_desc" class="cpy_page_page_desc">
...
?>
	<?php if ($cpy_page->status_id->Visible) { // status_id ?>
		<td data-name="status_id">
<span id="el$rowindex$_cpy_page_status_id" class="form-group cpy_page_status_id">
<select data-table="cpy_page" data-field="x_status_id" data-value-separator="<?php echo $cpy_page->status_id->DisplayValueSeparatorAttribute() ?>" id="x<?php echo $cpy_page_list->RowIndex ?>_status_id" name="x<?php echo $cpy_page_list->RowIndex ?>_status_id"<?php echo $cpy_page->status_id->EditAttributes() ?>>
<?php echo $cpy_page->status_id->SelectOptionListHtml("x<?php echo $cpy_page_list->RowIndex ?>_status_id") ?>
</select>
</span>
...
<input type="hidden" data-table="cpy_page" data-field="x_page_stext" name="o<?php echo $cpy_page_list->RowIndex ?>_page_stext" id="o<?php echo $cpy_page_list->RowIndex ?>_page_stext" value="<?php echo ew_HtmlEncode($cpy_page->page_stext->OldValue) ?>">
	...
	<?php if ($cpy_page->page_desc->Visible) { // page_desc ?>
		<td data-name="page_desc">
<span id="el$rowindex$_cpy_page_page_desc" class="form-group cpy_page_page_desc">
<textarea data-table="cpy_page" data-field="x_page_desc" name="x<?php echo $cpy_page_list->RowIndex ?>_page_desc" id="x<?php echo $cpy_page_list->RowIndex ?>_page_desc" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($cpy_page->page_desc->getPlaceHolder()) ?>"<?php echo $cpy_page->page_desc->EditAttributes() ?>><?php echo $cpy_page->page_desc->EditValue ?></textarea>
</span>
<input type="hidden" data-table="cpy_page" data-field="x_page_desc" name="o<?php echo $cpy_page_list->RowIndex ?>_page_desc" id="o<?php echo $cpy_page_list->RowIndex ?>_page_desc" value="<?php echo ew_HtmlEncode($cpy_page->page_desc->OldValue) ?>">
</td>
...
// Render list options (body, right)
$cpy_page_list->ListOptions->Render("body", "right", $cpy_page_list->RowIndex);
?>
<script type="text/javascript">
fcpy_pagelist.UpdateOpts(<?php echo $cpy_page_list->RowIndex ?>);
</script>
...
<?php } ?>
<?php if ($cpy_page->CurrentAction == "gridadd") { ?>
