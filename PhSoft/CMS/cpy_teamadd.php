...
$cpy_team_add = NULL; // Initialize page object first

class ccpy_team_add extends ccpy_team {
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$loaded) { // Record not loaded
					...
					$this->Page_Terminate("cpy_teamlist.php"); // No matching record, return to list
				}
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "cpy_teamview.php")
	...
	function LoadDefaultValues() {
		...
		$this->team_photo->CurrentValue = NULL; // Clear file related field
	}
	...
	function RenderRow() {
		...
		$this->team_photo->ViewCustomAttributes = "";
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->team_photo);
	...
	function ValidateForm() {
		...
		if (in_array("cpy_team_names", $DetailTblVar) && $GLOBALS["cpy_team_names"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("cpy_team_names", $DetailTblVar) && $GLOBALS["cpy_team_names"]->DetailAdd) {
				$GLOBALS["cpy_team_names"]->team_id->setSessionValue($this->team_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["cpy_team_names_grid"])) $GLOBALS["cpy_team_names_grid"] = new ccpy_team_names_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "cpy_team_names"); // Load user level of detail table
				$AddRow = $GLOBALS["cpy_team_names_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["cpy_team_names"]->team_id->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("cpy_team_names", $DetailTblVar)) {
				...
				if ($GLOBALS["cpy_team_names_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["cpy_team_names_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["cpy_team_names_grid"]->CurrentMode = "add";
					$GLOBALS["cpy_team_names_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($cpy_team_add)) $cpy_team_add = new ccpy_team_add();

// Page init
$cpy_team_add->Page_Init();

// Page main
$cpy_team_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cpy_team_add->Page_Render();
?>
...
var CurrentForm = fcpy_teamadd = new ew_Form("fcpy_teamadd", "add");

// Validate form
fcpy_teamadd.Validate = function() {
...
fcpy_teamadd.Form_CustomValidate = 
...
fcpy_teamadd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $cpy_team_add->ShowPageHeader(); ?>
<?php
$cpy_team_add->ShowMessage();
?>
<form name="fcpy_teamadd" id="fcpy_teamadd" class="<?php echo $cpy_team_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cpy_team_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cpy_team_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="cpy_team">
<input type="hidden" name="a_add" id="a_add" value="A">
<input type="hidden" name="modal" value="<?php echo intval($cpy_team_add->IsModal) ?>">
<div class="ewAddDiv"><!-- page* -->
<?php if ($cpy_team->team_iname->Visible) { // team_iname ?>
	<div id="r_team_iname" class="form-group">
		<label id="elh_cpy_team_team_iname" for="x_team_iname" class="<?php echo $cpy_team_add->LeftColumnClass ?>"><?php echo $cpy_team->team_iname->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $cpy_team_add->RightColumnClass ?>"><div<?php echo $cpy_team->team_iname->CellAttributes() ?>>
...
<?php if ($cpy_team->team_order->Visible) { // team_order ?>
	<div id="r_team_order" class="form-group">
		<label id="elh_cpy_team_team_order" for="x_team_order" class="<?php echo $cpy_team_add->LeftColumnClass ?>"><?php echo $cpy_team->team_order->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $cpy_team_add->RightColumnClass ?>"><div<?php echo $cpy_team->team_order->CellAttributes() ?>>
...
<?php if ($cpy_team->team_photo->Visible) { // team_photo ?>
	<div id="r_team_photo" class="form-group">
		<label id="elh_cpy_team_team_photo" class="<?php echo $cpy_team_add->LeftColumnClass ?>"><?php echo $cpy_team->team_photo->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $cpy_team_add->RightColumnClass ?>"><div<?php echo $cpy_team->team_photo->CellAttributes() ?>>
...
<?php
	if (in_array("cpy_team_names", explode(",", $cpy_team->getCurrentDetailTable())) && $cpy_team_names->DetailAdd) {
?>
...
<?php } ?>
<?php if (!$cpy_team_add->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $cpy_team_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cpy_team_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
