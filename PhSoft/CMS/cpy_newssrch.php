...
class ccpy_news_search extends ccpy_news {
	...
	function RenderRow() {
		...
		$this->news_text->ViewCustomAttributes = "";
			...
			$this->news_date->EditValue = ew_HtmlEncode(ew_FormatDateTime(ew_UnFormatDateTime($this->news_date->AdvancedSearch->SearchValue, 0), 8));
	...
	function ValidateSearch() {
		...
		if (!EW_SERVER_VALIDATE)
			return TRUE;
		if (!ew_CheckDateDef($this->news_date->AdvancedSearch->SearchValue)) {
...
fcpy_newssearch.Lists["x_status_id"] = {"LinkField":"x_status_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_status_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_status"};
fcpy_newssearch.Lists["x_status_id"].Data = "<?php echo $cpy_news_search->status_id->LookupFilterQuery(FALSE, "search") ?>";
...
<?php if ($cpy_news->status_id->Visible) { // status_id ?>
	<div id="r_status_id" class="form-group">
		<label for="x_status_id" class="<?php echo $cpy_news_search->LeftColumnClass ?>"><span id="elh_cpy_news_status_id"><?php echo $cpy_news->status_id->FldCaption() ?></span>
		...
		<div class="<?php echo $cpy_news_search->RightColumnClass ?>"><div<?php echo $cpy_news->status_id->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("fcpy_newssearch", "x_news_date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php } ?>
</span>
		</div></div>
...
<input type="text" data-table="cpy_news" data-field="x_news_image" name="x_news_image" id="x_news_image" size="30" maxlength="200" placeholder="<?php echo ew_HtmlEncode($cpy_news->news_image->getPlaceHolder()) ?>" value="<?php echo $cpy_news->news_image->EditValue ?>"<?php echo $cpy_news->news_image->EditAttributes() ?>>
</span>
		</div></div>
	</div>
<?php } ?>
