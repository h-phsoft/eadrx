...
$phs_menu_update = NULL; // Initialize page object first

class cphs_menu_update extends cphs_menu {
	...
	// Page object name
	var $PageObjName = 'phs_menu_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("phs_menulist.php"); // No records selected, return to list
	...
	function LoadMultiUpdateValues() {
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					$this->menu_id->setDbValue($this->Recordset->fields('menu_id'));
					$this->menu_pid->setDbValue($this->Recordset->fields('menu_pid'));
					...
					$this->menu_order->setDbValue($this->Recordset->fields('menu_order'));
					$this->menu_name->setDbValue($this->Recordset->fields('menu_name'));
					$this->menu_icon->setDbValue($this->Recordset->fields('menu_icon'));
					$this->menu_href->setDbValue($this->Recordset->fields('menu_href'));
					$this->menu_target->setDbValue($this->Recordset->fields('menu_target'));
					$this->menu_page->setDbValue($this->Recordset->fields('menu_page'));
				} else {
					if (!ew_CompareValue($this->menu_id->DbValue, $this->Recordset->fields('menu_id')))
						$this->menu_id->CurrentValue = NULL;
					if (!ew_CompareValue($this->menu_pid->DbValue, $this->Recordset->fields('menu_pid')))
					...
					if (!ew_CompareValue($this->menu_order->DbValue, $this->Recordset->fields('menu_order')))
						$this->menu_order->CurrentValue = NULL;
					if (!ew_CompareValue($this->menu_name->DbValue, $this->Recordset->fields('menu_name')))
						$this->menu_name->CurrentValue = NULL;
					if (!ew_CompareValue($this->menu_icon->DbValue, $this->Recordset->fields('menu_icon')))
						$this->menu_icon->CurrentValue = NULL;
					if (!ew_CompareValue($this->menu_href->DbValue, $this->Recordset->fields('menu_href')))
						$this->menu_href->CurrentValue = NULL;
					if (!ew_CompareValue($this->menu_target->DbValue, $this->Recordset->fields('menu_target')))
						$this->menu_target->CurrentValue = NULL;
					if (!ew_CompareValue($this->menu_page->DbValue, $this->Recordset->fields('menu_page')))
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->menu_id->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->menu_id->MultiUpdate = $objForm->GetValue("u_menu_id");
		if (!$this->menu_pid->FldIsDetailKey) {
			$this->menu_pid->setFormValue($objForm->GetValue("x_menu_pid"));
		}
		$this->menu_pid->MultiUpdate = $objForm->GetValue("u_menu_pid");
	...
	function ValidateForm() {
		...
		$gsFormError = "";
		$lUpdateCnt = 0;
		if ($this->menu_id->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->menu_pid->MultiUpdate == "1") $lUpdateCnt++;
		...
		if ($this->menu_order->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->menu_name->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->menu_icon->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->menu_href->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->menu_target->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->menu_page->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if ($this->menu_id->MultiUpdate <> "") {
			if (!ew_CheckInteger($this->menu_id->FormValue)) {
				ew_AddMessage($gsFormError, $this->menu_id->FldErrMsg());
			}
		}
		if ($this->menu_pid->MultiUpdate <> "" && !$this->menu_pid->FldIsDetailKey && !is_null($this->menu_pid->FormValue) && $this->menu_pid->FormValue == "") {
		...
		if ($this->menu_order->MultiUpdate <> "" && !$this->menu_order->FldIsDetailKey && !is_null($this->menu_order->FormValue) && $this->menu_order->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->menu_order->FldCaption(), $this->menu_order->ReqErrMsg));
		}
		if ($this->menu_order->MultiUpdate <> "") {
			if (!ew_CheckInteger($this->menu_order->FormValue)) {
				ew_AddMessage($gsFormError, $this->menu_order->FldErrMsg());
			}
		}
		if ($this->menu_name->MultiUpdate <> "" && !$this->menu_name->FldIsDetailKey && !is_null($this->menu_name->FormValue) && $this->menu_name->FormValue == "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->menu_pid->SetDbValueDef($rsnew, $this->menu_pid->CurrentValue, 0, $this->menu_pid->ReadOnly || $this->menu_pid->MultiUpdate <> "1");
			...
			// menu_order
			$this->menu_order->SetDbValueDef($rsnew, $this->menu_order->CurrentValue, 0, $this->menu_order->ReadOnly || $this->menu_order->MultiUpdate <> "1");

			// menu_name
			$this->menu_name->SetDbValueDef($rsnew, $this->menu_name->CurrentValue, "", $this->menu_name->ReadOnly || $this->menu_name->MultiUpdate <> "1");

			// menu_icon
			$this->menu_icon->SetDbValueDef($rsnew, $this->menu_icon->CurrentValue, NULL, $this->menu_icon->ReadOnly || $this->menu_icon->MultiUpdate <> "1");

			// menu_href
			$this->menu_href->SetDbValueDef($rsnew, $this->menu_href->CurrentValue, NULL, $this->menu_href->ReadOnly || $this->menu_href->MultiUpdate <> "1");

			// menu_target
			$this->menu_target->SetDbValueDef($rsnew, $this->menu_target->CurrentValue, NULL, $this->menu_target->ReadOnly || $this->menu_target->MultiUpdate <> "1");

			// menu_page
			$this->menu_page->SetDbValueDef($rsnew, $this->menu_page->CurrentValue, NULL, $this->menu_page->ReadOnly || $this->menu_page->MultiUpdate <> "1");
...
// Create page object
if (!isset($phs_menu_update)) $phs_menu_update = new cphs_menu_update();

// Page init
$phs_menu_update->Page_Init();

// Page main
$phs_menu_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$phs_menu_update->Page_Render();
?>
...
var CurrentForm = fphs_menuupdate = new ew_Form("fphs_menuupdate", "update");

// Validate form
fphs_menuupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_menu_id");
			...
			uelm = this.GetElements("u" + infix + "_menu_pid");
			...
			uelm = this.GetElements("u" + infix + "_menu_order");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
			...
			uelm = this.GetElements("u" + infix + "_menu_name");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
...
fphs_menuupdate.Form_CustomValidate = 
...
fphs_menuupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fphs_menuupdate.Lists["x_menu_pid"] = {"LinkField":"x_menu_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_menu_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_menu"};
fphs_menuupdate.Lists["x_menu_pid"].Data = "<?php echo $phs_menu_update->menu_pid->LookupFilterQuery(FALSE, "update") ?>";
fphs_menuupdate.Lists["x_page_id"] = {"LinkField":"x_page_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_page_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cpy_page"};
fphs_menuupdate.Lists["x_page_id"].Data = "<?php echo $phs_menu_update->page_id->LookupFilterQuery(FALSE, "update") ?>";
fphs_menuupdate.Lists["x_test_id"] = {"LinkField":"x_test_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_test_iname","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_test"};
fphs_menuupdate.Lists["x_test_id"].Data = "<?php echo $phs_menu_update->test_id->LookupFilterQuery(FALSE, "update") ?>";
fphs_menuupdate.Lists["x_type_id"] = {"LinkField":"x_type_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_type_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_menu_type"};
fphs_menuupdate.Lists["x_type_id"].Data = "<?php echo $phs_menu_update->type_id->LookupFilterQuery(FALSE, "update") ?>";
fphs_menuupdate.Lists["x_status_id"] = {"LinkField":"x_status_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_status_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_status"};
fphs_menuupdate.Lists["x_status_id"].Data = "<?php echo $phs_menu_update->status_id->LookupFilterQuery(FALSE, "update") ?>";
...
<?php $phs_menu_update->ShowPageHeader(); ?>
<?php
$phs_menu_update->ShowMessage();
?>
<form name="fphs_menuupdate" id="fphs_menuupdate" class="<?php echo $phs_menu_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($phs_menu_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $phs_menu_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="phs_menu">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($phs_menu_update->IsModal) ?>">
<?php foreach ($phs_menu_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_phs_menuupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($phs_menu->menu_pid->Visible) { // menu_pid ?>
	<div id="r_menu_pid" class="form-group">
		<label for="x_menu_pid" class="<?php echo $phs_menu_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_menu_pid" id="u_menu_pid" class="ewMultiSelect" value="1"<?php echo ($phs_menu->menu_pid->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_menu->menu_pid->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_menu_update->RightColumnClass ?>"><div<?php echo $phs_menu->menu_pid->CellAttributes() ?>>
...
<?php if ($phs_menu->page_id->Visible) { // page_id ?>
	<div id="r_page_id" class="form-group">
		<label for="x_page_id" class="<?php echo $phs_menu_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_page_id" id="u_page_id" class="ewMultiSelect" value="1"<?php echo ($phs_menu->page_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_menu->page_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_menu_update->RightColumnClass ?>"><div<?php echo $phs_menu->page_id->CellAttributes() ?>>
...
<?php if ($phs_menu->test_id->Visible) { // test_id ?>
	<div id="r_test_id" class="form-group">
		<label for="x_test_id" class="<?php echo $phs_menu_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_test_id" id="u_test_id" class="ewMultiSelect" value="1"<?php echo ($phs_menu->test_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_menu->test_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_menu_update->RightColumnClass ?>"><div<?php echo $phs_menu->test_id->CellAttributes() ?>>
...
<?php if ($phs_menu->type_id->Visible) { // type_id ?>
	<div id="r_type_id" class="form-group">
		<label for="x_type_id" class="<?php echo $phs_menu_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_type_id" id="u_type_id" class="ewMultiSelect" value="1"<?php echo ($phs_menu->type_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_menu->type_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_menu_update->RightColumnClass ?>"><div<?php echo $phs_menu->type_id->CellAttributes() ?>>
...
<?php if ($phs_menu->status_id->Visible) { // status_id ?>
	<div id="r_status_id" class="form-group">
		<label for="x_status_id" class="<?php echo $phs_menu_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_status_id" id="u_status_id" class="ewMultiSelect" value="1"<?php echo ($phs_menu->status_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_menu->status_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_menu_update->RightColumnClass ?>"><div<?php echo $phs_menu->status_id->CellAttributes() ?>>
...
<?php if ($phs_menu->menu_order->Visible) { // menu_order ?>
	<div id="r_menu_order" class="form-group">
		<label for="x_menu_order" class="<?php echo $phs_menu_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_menu_order" id="u_menu_order" class="ewMultiSelect" value="1"<?php echo ($phs_menu->menu_order->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_menu->menu_order->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_menu_update->RightColumnClass ?>"><div<?php echo $phs_menu->menu_order->CellAttributes() ?>>
...
<?php if ($phs_menu->menu_name->Visible) { // menu_name ?>
	<div id="r_menu_name" class="form-group">
		<label for="x_menu_name" class="<?php echo $phs_menu_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_menu_name" id="u_menu_name" class="ewMultiSelect" value="1"<?php echo ($phs_menu->menu_name->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_menu->menu_name->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_menu_update->RightColumnClass ?>"><div<?php echo $phs_menu->menu_name->CellAttributes() ?>>
...
<?php if ($phs_menu->menu_icon->Visible) { // menu_icon ?>
	<div id="r_menu_icon" class="form-group">
		<label for="x_menu_icon" class="<?php echo $phs_menu_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_menu_icon" id="u_menu_icon" class="ewMultiSelect" value="1"<?php echo ($phs_menu->menu_icon->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_menu->menu_icon->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_menu_update->RightColumnClass ?>"><div<?php echo $phs_menu->menu_icon->CellAttributes() ?>>
...
<?php if ($phs_menu->menu_href->Visible) { // menu_href ?>
	<div id="r_menu_href" class="form-group">
		<label for="x_menu_href" class="<?php echo $phs_menu_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_menu_href" id="u_menu_href" class="ewMultiSelect" value="1"<?php echo ($phs_menu->menu_href->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_menu->menu_href->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_menu_update->RightColumnClass ?>"><div<?php echo $phs_menu->menu_href->CellAttributes() ?>>
...
<?php if ($phs_menu->menu_target->Visible) { // menu_target ?>
	<div id="r_menu_target" class="form-group">
		<label for="x_menu_target" class="<?php echo $phs_menu_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_menu_target" id="u_menu_target" class="ewMultiSelect" value="1"<?php echo ($phs_menu->menu_target->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_menu->menu_target->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_menu_update->RightColumnClass ?>"><div<?php echo $phs_menu->menu_target->CellAttributes() ?>>
...
<?php if ($phs_menu->menu_page->Visible) { // menu_page ?>
	<div id="r_menu_page" class="form-group">
		<label for="x_menu_page" class="<?php echo $phs_menu_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_menu_page" id="u_menu_page" class="ewMultiSelect" value="1"<?php echo ($phs_menu->menu_page->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_menu->menu_page->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_menu_update->RightColumnClass ?>"><div<?php echo $phs_menu->menu_page->CellAttributes() ?>>
...
<?php if (!$phs_menu_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $phs_menu_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $phs_menu_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
	</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fphs_menuupdate.Init();
</script>
<?php
$phs_menu_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$phs_menu_update->Page_Terminate();
?>
