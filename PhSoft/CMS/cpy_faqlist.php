...
class ccpy_faq_list extends ccpy_faq {
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "cpy_faq_category") {
			global $cpy_faq_category;
			$rsmaster = $cpy_faq_category->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("cpy_faq_categorylist.php"); // Return to master page
			} else {
				$cpy_faq_category->LoadListRowValues($rsmaster);
				$cpy_faq_category->RowType = EW_ROWTYPE_MASTER; // Master row
				$cpy_faq_category->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function AddRow($rsold = NULL) {
		...
		// fcat_id
		$this->fcat_id->SetDbValueDef($rsnew, $this->fcat_id->CurrentValue, 0, strval($this->fcat_id->CurrentValue) == "");
	...
	function ExportData() {
		...
		if (EW_EXPORT_MASTER_RECORD && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "cpy_faq_category") {
			global $cpy_faq_category;
			if (!isset($cpy_faq_category)) $cpy_faq_category = new ccpy_faq_category;
			$rsmaster = $cpy_faq_category->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("v"); // Change to vertical
				if ($this->Export <> "csv" || EW_EXPORT_MASTER_RECORD_FOR_CSV) {
					$Doc->Table = &$cpy_faq_category;
					$cpy_faq_category->ExportDocument($Doc, $rsmaster, 1, 1);
					$Doc->ExportEmptyRow();
					$Doc->Table = &$this;
				}
...
<?php } ?>
<?php if (($cpy_faq->Export == "") || (EW_EXPORT_MASTER_RECORD && $cpy_faq->Export == "print")) { ?>
<?php
if ($cpy_faq_list->DbMasterFilter <> "" && $cpy_faq->getCurrentMasterTable() == "cpy_faq_category") {
	if ($cpy_faq_list->MasterRecordExists) {
?>
<?php include_once "cpy_faq_categorymaster.php" ?>
<?php
	}
}
?>
...
<?php } ?>
<input type="hidden" name="t" value="cpy_faq">
<?php if ($cpy_faq->getCurrentMasterTable() == "cpy_faq_category" && $cpy_faq->CurrentAction <> "") { ?>
<input type="hidden" name="<?php echo EW_TABLE_SHOW_MASTER ?>" value="cpy_faq_category">
<input type="hidden" name="fk_fcat_id" value="<?php echo $cpy_faq->fcat_id->getSessionValue() ?>">
<?php } ?>
...
<input type="hidden" id="x<?php echo $cpy_faq_list->RowIndex ?>_fcat_id" name="x<?php echo $cpy_faq_list->RowIndex ?>_fcat_id" value="<?php echo ew_HtmlEncode($cpy_faq->fcat_id->CurrentValue) ?>">
<?php } else { ?>
<span id="el<?php echo $cpy_faq_list->RowCnt ?>_cpy_faq_fcat_id" class="form-group cpy_faq_fcat_id">
