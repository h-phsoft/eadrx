...
$app_tips_update = NULL; // Initialize page object first

class capp_tips_update extends capp_tips {
	...
	// Page object name
	var $PageObjName = 'app_tips_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("app_tipslist.php"); // No records selected, return to list
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					...
					$this->tips_text->setDbValue($this->Recordset->fields('tips_text'));
					...
					if (!ew_CompareValue($this->tips_text->DbValue, $this->Recordset->fields('tips_text')))
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->tips_id->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->tips_text->MultiUpdate = $objForm->GetValue("u_tips_text");
	...
	function ValidateForm() {
		...
		if ($this->tips_text->MultiUpdate == "1") $lUpdateCnt++;
		...
		if ($this->tips_text->MultiUpdate <> "" && !$this->tips_text->FldIsDetailKey && !is_null($this->tips_text->FormValue) && $this->tips_text->FormValue == "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// tips_text
			$this->tips_text->SetDbValueDef($rsnew, $this->tips_text->CurrentValue, "", $this->tips_text->ReadOnly || $this->tips_text->MultiUpdate <> "1");
...
// Create page object
if (!isset($app_tips_update)) $app_tips_update = new capp_tips_update();

// Page init
$app_tips_update->Page_Init();

// Page main
$app_tips_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_tips_update->Page_Render();
?>
...
var CurrentForm = fapp_tipsupdate = new ew_Form("fapp_tipsupdate", "update");

// Validate form
fapp_tipsupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_tips_text");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
...
fapp_tipsupdate.Form_CustomValidate = 
...
fapp_tipsupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_tipsupdate.Lists["x_tcat_id"] = {"LinkField":"x_tcat_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_tcat_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_tips_category"};
fapp_tipsupdate.Lists["x_tcat_id"].Data = "<?php echo $app_tips_update->tcat_id->LookupFilterQuery(FALSE, "update") ?>";
fapp_tipsupdate.Lists["x_lang_id"] = {"LinkField":"x_lang_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_lang_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_language"};
fapp_tipsupdate.Lists["x_lang_id"].Data = "<?php echo $app_tips_update->lang_id->LookupFilterQuery(FALSE, "update") ?>";
fapp_tipsupdate.Lists["x_status_id"] = {"LinkField":"x_status_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_status_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_status"};
fapp_tipsupdate.Lists["x_status_id"].Data = "<?php echo $app_tips_update->status_id->LookupFilterQuery(FALSE, "update") ?>";
...
<?php $app_tips_update->ShowPageHeader(); ?>
<?php
$app_tips_update->ShowMessage();
?>
<form name="fapp_tipsupdate" id="fapp_tipsupdate" class="<?php echo $app_tips_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_tips_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_tips_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_tips">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($app_tips_update->IsModal) ?>">
<?php foreach ($app_tips_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_app_tipsupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($app_tips->tcat_id->Visible) { // tcat_id ?>
	<div id="r_tcat_id" class="form-group">
		<label for="x_tcat_id" class="<?php echo $app_tips_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_tcat_id" id="u_tcat_id" class="ewMultiSelect" value="1"<?php echo ($app_tips->tcat_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_tips->tcat_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_tips_update->RightColumnClass ?>"><div<?php echo $app_tips->tcat_id->CellAttributes() ?>>
...
<?php if ($app_tips->lang_id->Visible) { // lang_id ?>
	<div id="r_lang_id" class="form-group">
		<label for="x_lang_id" class="<?php echo $app_tips_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_lang_id" id="u_lang_id" class="ewMultiSelect" value="1"<?php echo ($app_tips->lang_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_tips->lang_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_tips_update->RightColumnClass ?>"><div<?php echo $app_tips->lang_id->CellAttributes() ?>>
...
<?php if ($app_tips->status_id->Visible) { // status_id ?>
	<div id="r_status_id" class="form-group">
		<label for="x_status_id" class="<?php echo $app_tips_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_status_id" id="u_status_id" class="ewMultiSelect" value="1"<?php echo ($app_tips->status_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_tips->status_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_tips_update->RightColumnClass ?>"><div<?php echo $app_tips->status_id->CellAttributes() ?>>
...
<?php if ($app_tips->tips_text->Visible) { // tips_text ?>
	<div id="r_tips_text" class="form-group">
		<label class="<?php echo $app_tips_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_tips_text" id="u_tips_text" class="ewMultiSelect" value="1"<?php echo ($app_tips->tips_text->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_tips->tips_text->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_tips_update->RightColumnClass ?>"><div<?php echo $app_tips->tips_text->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateEditor("fapp_tipsupdate", "x_tips_text", 35, 4, <?php echo ($app_tips->tips_text->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<?php if ($app_tips->ins_datetime->Visible) { // ins_datetime ?>
	<div id="r_ins_datetime" class="form-group">
		<label for="x_ins_datetime" class="<?php echo $app_tips_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_ins_datetime" id="u_ins_datetime" class="ewMultiSelect" value="1"<?php echo ($app_tips->ins_datetime->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_tips->ins_datetime->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_tips_update->RightColumnClass ?>"><div<?php echo $app_tips->ins_datetime->CellAttributes() ?>>
...
<?php if (!$app_tips_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $app_tips_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $app_tips_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
	</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fapp_tipsupdate.Init();
</script>
<?php
$app_tips_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$app_tips_update->Page_Terminate();
?>
