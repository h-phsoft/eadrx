...
$app_tips_package = NULL;

//
// Table class for app_tips_package
//
class capp_tips_package extends cTable {
	var $pkg_id;
	var $cycle_id;
	var $status_id;
	var $pkg_order;
	...
	function __construct() {
		...
		$this->TableVar = 'app_tips_package';
		$this->TableName = 'app_tips_package';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`app_tips_package`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// pkg_id
		$this->pkg_id = new cField('app_tips_package', 'app_tips_package', 'x_pkg_id', 'pkg_id', '`pkg_id`', '`pkg_id`', 3, -1, FALSE, '`pkg_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// cycle_id
		$this->cycle_id = new cField('app_tips_package', 'app_tips_package', 'x_cycle_id', 'cycle_id', '`cycle_id`', '`cycle_id`', 2, -1, FALSE, '`cycle_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// status_id
		$this->status_id = new cField('app_tips_package', 'app_tips_package', 'x_status_id', 'status_id', '`status_id`', '`status_id`', 16, -1, FALSE, '`status_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// pkg_order
		$this->pkg_order = new cField('app_tips_package', 'app_tips_package', 'x_pkg_order', 'pkg_order', '`pkg_order`', '`pkg_order`', 3, -1, FALSE, '`pkg_order`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->pkg_order->Sortable = TRUE; // Allow sort
		$this->pkg_order->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['pkg_order'] = &$this->pkg_order;

		// pkg_name
		$this->pkg_name = new cField('app_tips_package', 'app_tips_package', 'x_pkg_name', 'pkg_name', '`pkg_name`', '`pkg_name`', 200, -1, FALSE, '`pkg_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->pkg_name->Sortable = TRUE; // Allow sort
		$this->fields['pkg_name'] = &$this->pkg_name;

		// pkg_price
		$this->pkg_price = new cField('app_tips_package', 'app_tips_package', 'x_pkg_price', 'pkg_price', '`pkg_price`', '`pkg_price`', 131, -1, FALSE, '`pkg_price`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "app_tips_pkg_cat") {
			$sDetailUrl = $GLOBALS["app_tips_pkg_cat"]->GetListUrl() . "?" . EW_TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$sDetailUrl .= "&fk_pkg_id=" . urlencode($this->pkg_id->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "app_tips_packagelist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`app_tips_package`";
	}
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "`pkg_order` ASC,`cycle_id` ASC,`status_id` ASC";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "app_tips_packagelist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "app_tips_packageedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "app_tips_packageadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("app_tips_packageview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("app_tips_packageview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "app_tips_packageadd.php?" . $this->UrlParm($parm);
		else
			$url = "app_tips_packageadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("app_tips_packageedit.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("app_tips_packageedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("app_tips_packageadd.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("app_tips_packageadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("app_tips_packagedelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->pkg_order->setDbValue($rs->fields('pkg_order'));
	...
	function RenderEditRow() {
		...
		$this->pkg_order->EditValue = $this->pkg_order->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->pkg_order->Exportable) $Doc->ExportCaption($this->pkg_order);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->pkg_order->Exportable) $Doc->ExportField($this->pkg_order);
