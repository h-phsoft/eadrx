...
$app_book_page_view = NULL; // Initialize page object first

class capp_book_page_view extends capp_book_page {
	...
	// Page object name
	var $PageObjName = 'app_book_page_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["bpage_id"] <> "") {
			$this->RecKey["bpage_id"] = $_GET["bpage_id"];
			$KeyUrl .= "&amp;bpage_id=" . urlencode($this->RecKey["bpage_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["bpage_id"] <> "") {
				...
				$this->RecKey["bpage_id"] = $this->bpage_id->QueryStringValue;
			} elseif (@$_POST["bpage_id"] <> "") {
				$this->bpage_id->setFormValue($_POST["bpage_id"]);
				$this->RecKey["bpage_id"] = $this->bpage_id->FormValue;
			} else {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "app_book_pagelist.php"; // No matching record, return to list
					} else {
						$this->LoadRowValues($this->Recordset); // Load row values
					}
			}
		} else {
			$sReturnUrl = "app_book_pagelist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($app_book_page_view)) $app_book_page_view = new capp_book_page_view();

// Page init
$app_book_page_view->Page_Init();

// Page main
$app_book_page_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_book_page_view->Page_Render();
?>
...
var CurrentForm = fapp_book_pageview = new ew_Form("fapp_book_pageview", "view");

// Form_CustomValidate event
fapp_book_pageview.Form_CustomValidate = 
...
fapp_book_pageview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_book_pageview.Lists["x_book_id"] = {"LinkField":"x_book_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_book_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_book"};
fapp_book_pageview.Lists["x_book_id"].Data = "<?php echo $app_book_page_view->book_id->LookupFilterQuery(FALSE, "view") ?>";
...
<div class="ewToolbar">
<?php $app_book_page_view->ExportOptions->Render("body") ?>
<?php
	foreach ($app_book_page_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php $app_book_page_view->ShowPageHeader(); ?>
<?php
$app_book_page_view->ShowMessage();
?>
<?php if (!$app_book_page_view->IsModal) { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($app_book_page_view->Pager)) $app_book_page_view->Pager = new cPrevNextPager($app_book_page_view->StartRec, $app_book_page_view->DisplayRecs, $app_book_page_view->TotalRecs, $app_book_page_view->AutoHidePager) ?>
<?php if ($app_book_page_view->Pager->RecordCount > 0 && $app_book_page_view->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($app_book_page_view->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $app_book_page_view->PageUrl() ?>start=<?php echo $app_book_page_view->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($app_book_page_view->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $app_book_page_view->PageUrl() ?>start=<?php echo $app_book_page_view->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $app_book_page_view->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($app_book_page_view->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $app_book_page_view->PageUrl() ?>start=<?php echo $app_book_page_view->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($app_book_page_view->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $app_book_page_view->PageUrl() ?>start=<?php echo $app_book_page_view->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $app_book_page_view->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<form name="fapp_book_pageview" id="fapp_book_pageview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_book_page_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_book_page_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_book_page">
<input type="hidden" name="modal" value="<?php echo intval($app_book_page_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($app_book_page->book_id->Visible) { // book_id ?>
	<tr id="r_book_id">
		<td class="col-sm-2"><span id="elh_app_book_page_book_id"><?php echo $app_book_page->book_id->FldCaption() ?></span></td>
...
<?php echo $app_book_page->book_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_book_page->page_num->Visible) { // page_num ?>
	<tr id="r_page_num">
		<td class="col-sm-2"><span id="elh_app_book_page_page_num"><?php echo $app_book_page->page_num->FldCaption() ?></span></td>
...
<?php echo $app_book_page->page_num->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($app_book_page->page_image->Visible) { // page_image ?>
	<tr id="r_page_image">
		<td class="col-sm-2"><span id="elh_app_book_page_page_image"><?php echo $app_book_page->page_image->FldCaption() ?></span></td>
		<td data-name="page_image"<?php echo $app_book_page->page_image->CellAttributes() ?>>
<span id="el_app_book_page_page_image">
<span>
<?php echo ew_GetFileViewTag($app_book_page->page_image, $app_book_page->page_image->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php if ($app_book_page->page_text->Visible) { // page_text ?>
	<tr id="r_page_text">
		<td class="col-sm-2"><span id="elh_app_book_page_page_text"><?php echo $app_book_page->page_text->FldCaption() ?></span></td>
		<td data-name="page_text"<?php echo $app_book_page->page_text->CellAttributes() ?>>
<span id="el_app_book_page_page_text">
<span<?php echo $app_book_page->page_text->ViewAttributes() ?>>
<?php echo $app_book_page->page_text->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
fapp_book_pageview.Init();
</script>
<?php
$app_book_page_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$app_book_page_view->Page_Terminate();
?>
