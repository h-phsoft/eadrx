...
$app_test_name_update = NULL; // Initialize page object first

class capp_test_name_update extends capp_test_name {
	...
	// Page object name
	var $PageObjName = 'app_test_name_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("app_test_namelist.php"); // No records selected, return to list
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					...
					$this->test_name->setDbValue($this->Recordset->fields('test_name'));
					$this->test_desc->setDbValue($this->Recordset->fields('test_desc'));
				} else {
					...
					if (!ew_CompareValue($this->test_name->DbValue, $this->Recordset->fields('test_name')))
						$this->test_name->CurrentValue = NULL;
					if (!ew_CompareValue($this->test_desc->DbValue, $this->Recordset->fields('test_desc')))
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->ntest_id->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->test_name->MultiUpdate = $objForm->GetValue("u_test_name");
		if (!$this->test_desc->FldIsDetailKey) {
			$this->test_desc->setFormValue($objForm->GetValue("x_test_desc"));
		}
		$this->test_desc->MultiUpdate = $objForm->GetValue("u_test_desc");
	...
	function ValidateForm() {
		...
		if ($this->test_name->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->test_desc->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
		...
		if ($this->test_name->MultiUpdate <> "" && !$this->test_name->FldIsDetailKey && !is_null($this->test_name->FormValue) && $this->test_name->FormValue == "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// test_name
			$this->test_name->SetDbValueDef($rsnew, $this->test_name->CurrentValue, "", $this->test_name->ReadOnly || $this->test_name->MultiUpdate <> "1");

			// test_desc
			$this->test_desc->SetDbValueDef($rsnew, $this->test_desc->CurrentValue, NULL, $this->test_desc->ReadOnly || $this->test_desc->MultiUpdate <> "1");
...
// Create page object
if (!isset($app_test_name_update)) $app_test_name_update = new capp_test_name_update();

// Page init
$app_test_name_update->Page_Init();

// Page main
$app_test_name_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_test_name_update->Page_Render();
?>
...
var CurrentForm = fapp_test_nameupdate = new ew_Form("fapp_test_nameupdate", "update");

// Validate form
fapp_test_nameupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_test_name");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
...
fapp_test_nameupdate.Form_CustomValidate = 
...
fapp_test_nameupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_test_nameupdate.Lists["x_test_id"] = {"LinkField":"x_test_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_test_iname","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_test"};
fapp_test_nameupdate.Lists["x_test_id"].Data = "<?php echo $app_test_name_update->test_id->LookupFilterQuery(FALSE, "update") ?>";
fapp_test_nameupdate.Lists["x_lang_id"] = {"LinkField":"x_lang_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_lang_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_language"};
fapp_test_nameupdate.Lists["x_lang_id"].Data = "<?php echo $app_test_name_update->lang_id->LookupFilterQuery(FALSE, "update") ?>";
...
<?php $app_test_name_update->ShowPageHeader(); ?>
<?php
$app_test_name_update->ShowMessage();
?>
<form name="fapp_test_nameupdate" id="fapp_test_nameupdate" class="<?php echo $app_test_name_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_test_name_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_test_name_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_test_name">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($app_test_name_update->IsModal) ?>">
<?php foreach ($app_test_name_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_app_test_nameupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($app_test_name->test_id->Visible) { // test_id ?>
	<div id="r_test_id" class="form-group">
		<label for="x_test_id" class="<?php echo $app_test_name_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_test_id" id="u_test_id" class="ewMultiSelect" value="1"<?php echo ($app_test_name->test_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test_name->test_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_name_update->RightColumnClass ?>"><div<?php echo $app_test_name->test_id->CellAttributes() ?>>
...
<?php if ($app_test_name->lang_id->Visible) { // lang_id ?>
	<div id="r_lang_id" class="form-group">
		<label for="x_lang_id" class="<?php echo $app_test_name_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_lang_id" id="u_lang_id" class="ewMultiSelect" value="1"<?php echo ($app_test_name->lang_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test_name->lang_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_name_update->RightColumnClass ?>"><div<?php echo $app_test_name->lang_id->CellAttributes() ?>>
...
<?php if ($app_test_name->test_name->Visible) { // test_name ?>
	<div id="r_test_name" class="form-group">
		<label for="x_test_name" class="<?php echo $app_test_name_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_test_name" id="u_test_name" class="ewMultiSelect" value="1"<?php echo ($app_test_name->test_name->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test_name->test_name->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_name_update->RightColumnClass ?>"><div<?php echo $app_test_name->test_name->CellAttributes() ?>>
...
<?php if ($app_test_name->test_desc->Visible) { // test_desc ?>
	<div id="r_test_desc" class="form-group">
		<label class="<?php echo $app_test_name_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_test_desc" id="u_test_desc" class="ewMultiSelect" value="1"<?php echo ($app_test_name->test_desc->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test_name->test_desc->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_name_update->RightColumnClass ?>"><div<?php echo $app_test_name->test_desc->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateEditor("fapp_test_nameupdate", "x_test_desc", 35, 8, <?php echo ($app_test_name->test_desc->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<?php if (!$app_test_name_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $app_test_name_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $app_test_name_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
	</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fapp_test_nameupdate.Init();
</script>
<?php
$app_test_name_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$app_test_name_update->Page_Terminate();
?>
