...
$app_tips_pkg_cat_delete = NULL; // Initialize page object first

class capp_tips_pkg_cat_delete extends capp_tips_pkg_cat {
	...
	// Page object name
	var $PageObjName = 'app_tips_pkg_cat_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("app_tips_pkg_catlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in app_tips_pkg_cat class, app_tips_pkg_catinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("app_tips_pkg_catlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($app_tips_pkg_cat_delete)) $app_tips_pkg_cat_delete = new capp_tips_pkg_cat_delete();

// Page init
$app_tips_pkg_cat_delete->Page_Init();

// Page main
$app_tips_pkg_cat_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_tips_pkg_cat_delete->Page_Render();
?>
...
var CurrentForm = fapp_tips_pkg_catdelete = new ew_Form("fapp_tips_pkg_catdelete", "delete");

// Form_CustomValidate event
fapp_tips_pkg_catdelete.Form_CustomValidate = 
...
fapp_tips_pkg_catdelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_tips_pkg_catdelete.Lists["x_pkg_id"] = {"LinkField":"x_pkg_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_pkg_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_tips_package"};
fapp_tips_pkg_catdelete.Lists["x_pkg_id"].Data = "<?php echo $app_tips_pkg_cat_delete->pkg_id->LookupFilterQuery(FALSE, "delete") ?>";
fapp_tips_pkg_catdelete.Lists["x_tcat_id"] = {"LinkField":"x_tcat_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_tcat_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_tips_category"};
fapp_tips_pkg_catdelete.Lists["x_tcat_id"].Data = "<?php echo $app_tips_pkg_cat_delete->tcat_id->LookupFilterQuery(FALSE, "delete") ?>";
...
<?php $app_tips_pkg_cat_delete->ShowPageHeader(); ?>
<?php
$app_tips_pkg_cat_delete->ShowMessage();
?>
<form name="fapp_tips_pkg_catdelete" id="fapp_tips_pkg_catdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_tips_pkg_cat_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_tips_pkg_cat_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_tips_pkg_cat">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($app_tips_pkg_cat_delete->RecKeys as $key) { ?>
...
<?php if ($app_tips_pkg_cat->pkg_id->Visible) { // pkg_id ?>
		<th class="<?php echo $app_tips_pkg_cat->pkg_id->HeaderCellClass() ?>"><span id="elh_app_tips_pkg_cat_pkg_id" class="app_tips_pkg_cat_pkg_id"><?php echo $app_tips_pkg_cat->pkg_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($app_tips_pkg_cat->tcat_id->Visible) { // tcat_id ?>
		<th class="<?php echo $app_tips_pkg_cat->tcat_id->HeaderCellClass() ?>"><span id="elh_app_tips_pkg_cat_tcat_id" class="app_tips_pkg_cat_tcat_id"><?php echo $app_tips_pkg_cat->tcat_id->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$app_tips_pkg_cat_delete->RecCnt = 0;
$i = 0;
while (!$app_tips_pkg_cat_delete->Recordset->EOF) {
	$app_tips_pkg_cat_delete->RecCnt++;
	$app_tips_pkg_cat_delete->RowCnt++;

	// Set row properties
	$app_tips_pkg_cat->ResetAttrs();
	$app_tips_pkg_cat->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$app_tips_pkg_cat_delete->LoadRowValues($app_tips_pkg_cat_delete->Recordset);

	// Render row
	$app_tips_pkg_cat_delete->RenderRow();
?>
	<tr<?php echo $app_tips_pkg_cat->RowAttributes() ?>>
<?php if ($app_tips_pkg_cat->pkg_id->Visible) { // pkg_id ?>
		<td<?php echo $app_tips_pkg_cat->pkg_id->CellAttributes() ?>>
<span id="el<?php echo $app_tips_pkg_cat_delete->RowCnt ?>_app_tips_pkg_cat_pkg_id" class="app_tips_pkg_cat_pkg_id">
...
<?php } ?>
<?php if ($app_tips_pkg_cat->tcat_id->Visible) { // tcat_id ?>
		<td<?php echo $app_tips_pkg_cat->tcat_id->CellAttributes() ?>>
<span id="el<?php echo $app_tips_pkg_cat_delete->RowCnt ?>_app_tips_pkg_cat_tcat_id" class="app_tips_pkg_cat_tcat_id">
...
<?php
	$app_tips_pkg_cat_delete->Recordset->MoveNext();
}
$app_tips_pkg_cat_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $app_tips_pkg_cat_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fapp_tips_pkg_catdelete.Init();
</script>
<?php
$app_tips_pkg_cat_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$app_tips_pkg_cat_delete->Page_Terminate();
?>
