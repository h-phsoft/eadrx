...
$app_book_page_edit = NULL; // Initialize page object first

class capp_book_page_edit extends capp_book_page {
	...
	// Page object name
	var $PageObjName = 'app_book_page_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			...
			// Load key from Form
			if ($objForm->HasValue("x_bpage_id")) {
		...
		if ($this->TotalRecs <= 0) { // No record found
			...
			$this->Page_Terminate("app_book_pagelist.php"); // Return to list page
			...
			if (!is_null($this->bpage_id->CurrentValue)) {
				while (!$this->Recordset->EOF) {
					if (strval($this->bpage_id->CurrentValue) == strval($this->Recordset->fields('bpage_id'))) {
	...
	function RenderRow() {
		...
		$this->page_text->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->page_image);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// page_text
			$this->page_text->SetDbValueDef($rsnew, $this->page_text->CurrentValue, "", $this->page_text->ReadOnly);
...
// Create page object
if (!isset($app_book_page_edit)) $app_book_page_edit = new capp_book_page_edit();

// Page init
$app_book_page_edit->Page_Init();

// Page main
$app_book_page_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_book_page_edit->Page_Render();
?>
...
var CurrentForm = fapp_book_pageedit = new ew_Form("fapp_book_pageedit", "edit");

// Validate form
fapp_book_pageedit.Validate = function() {
...
fapp_book_pageedit.Form_CustomValidate = 
...
fapp_book_pageedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_book_pageedit.Lists["x_book_id"] = {"LinkField":"x_book_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_book_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_book"};
fapp_book_pageedit.Lists["x_book_id"].Data = "<?php echo $app_book_page_edit->book_id->LookupFilterQuery(FALSE, "edit") ?>";
...
<?php $app_book_page_edit->ShowPageHeader(); ?>
<?php
$app_book_page_edit->ShowMessage();
?>
<?php if (!$app_book_page_edit->IsModal) { ?>
<form name="ewPagerForm" class="form-horizontal ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($app_book_page_edit->Pager)) $app_book_page_edit->Pager = new cPrevNextPager($app_book_page_edit->StartRec, $app_book_page_edit->DisplayRecs, $app_book_page_edit->TotalRecs, $app_book_page_edit->AutoHidePager) ?>
<?php if ($app_book_page_edit->Pager->RecordCount > 0 && $app_book_page_edit->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($app_book_page_edit->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $app_book_page_edit->PageUrl() ?>start=<?php echo $app_book_page_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($app_book_page_edit->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $app_book_page_edit->PageUrl() ?>start=<?php echo $app_book_page_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $app_book_page_edit->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($app_book_page_edit->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $app_book_page_edit->PageUrl() ?>start=<?php echo $app_book_page_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($app_book_page_edit->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $app_book_page_edit->PageUrl() ?>start=<?php echo $app_book_page_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $app_book_page_edit->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<form name="fapp_book_pageedit" id="fapp_book_pageedit" class="<?php echo $app_book_page_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_book_page_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_book_page_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_book_page">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($app_book_page_edit->IsModal) ?>">
...
<?php if ($app_book_page->book_id->Visible) { // book_id ?>
	<div id="r_book_id" class="form-group">
		<label id="elh_app_book_page_book_id" for="x_book_id" class="<?php echo $app_book_page_edit->LeftColumnClass ?>"><?php echo $app_book_page->book_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $app_book_page_edit->RightColumnClass ?>"><div<?php echo $app_book_page->book_id->CellAttributes() ?>>
...
<?php if ($app_book_page->page_num->Visible) { // page_num ?>
	<div id="r_page_num" class="form-group">
		<label id="elh_app_book_page_page_num" for="x_page_num" class="<?php echo $app_book_page_edit->LeftColumnClass ?>"><?php echo $app_book_page->page_num->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $app_book_page_edit->RightColumnClass ?>"><div<?php echo $app_book_page->page_num->CellAttributes() ?>>
...
<?php if ($app_book_page->page_image->Visible) { // page_image ?>
	<div id="r_page_image" class="form-group">
		<label id="elh_app_book_page_page_image" class="<?php echo $app_book_page_edit->LeftColumnClass ?>"><?php echo $app_book_page->page_image->FldCaption() ?></label>
		<div class="<?php echo $app_book_page_edit->RightColumnClass ?>"><div<?php echo $app_book_page->page_image->CellAttributes() ?>>
...
<?php if ($app_book_page->page_text->Visible) { // page_text ?>
	<div id="r_page_text" class="form-group">
		<label id="elh_app_book_page_page_text" class="<?php echo $app_book_page_edit->LeftColumnClass ?>"><?php echo $app_book_page->page_text->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $app_book_page_edit->RightColumnClass ?>"><div<?php echo $app_book_page->page_text->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateEditor("fapp_book_pageedit", "x_page_text", 35, 8, <?php echo ($app_book_page->page_text->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<input type="hidden" data-table="app_book_page" data-field="x_bpage_id" name="x_bpage_id" id="x_bpage_id" value="<?php echo ew_HtmlEncode($app_book_page->bpage_id->CurrentValue) ?>">
<?php if (!$app_book_page_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $app_book_page_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $app_book_page_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
...
<script type="text/javascript">
fapp_book_pageedit.Init();
</script>
<?php
$app_book_page_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$app_book_page_edit->Page_Terminate();
?>
