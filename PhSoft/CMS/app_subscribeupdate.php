...
$app_subscribe_update = NULL; // Initialize page object first

class capp_subscribe_update extends capp_subscribe {
	...
	// Page object name
	var $PageObjName = 'app_subscribe_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("app_subscribelist.php"); // No records selected, return to list
	...
	function LoadMultiUpdateValues() {
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					$this->serv_id->setDbValue($this->Recordset->fields('serv_id'));
					$this->user_id->setDbValue($this->Recordset->fields('user_id'));
					$this->cycle_id->setDbValue($this->Recordset->fields('cycle_id'));
					...
					$this->cons_id->setDbValue($this->Recordset->fields('cons_id'));
					$this->subs_start->setDbValue($this->Recordset->fields('subs_start'));
					$this->subs_end->setDbValue($this->Recordset->fields('subs_end'));
					$this->subs_qnt->setDbValue($this->Recordset->fields('subs_qnt'));
					$this->subs_price->setDbValue($this->Recordset->fields('subs_price'));
					$this->subs_amt->setDbValue($this->Recordset->fields('subs_amt'));
					$this->ins_datetime->setDbValue($this->Recordset->fields('ins_datetime'));
				} else {
					if (!ew_CompareValue($this->serv_id->DbValue, $this->Recordset->fields('serv_id')))
						$this->serv_id->CurrentValue = NULL;
					if (!ew_CompareValue($this->user_id->DbValue, $this->Recordset->fields('user_id')))
						$this->user_id->CurrentValue = NULL;
					if (!ew_CompareValue($this->cycle_id->DbValue, $this->Recordset->fields('cycle_id')))
					...
					if (!ew_CompareValue($this->cons_id->DbValue, $this->Recordset->fields('cons_id')))
						$this->cons_id->CurrentValue = NULL;
					if (!ew_CompareValue($this->subs_start->DbValue, $this->Recordset->fields('subs_start')))
						$this->subs_start->CurrentValue = NULL;
					if (!ew_CompareValue($this->subs_end->DbValue, $this->Recordset->fields('subs_end')))
						$this->subs_end->CurrentValue = NULL;
					if (!ew_CompareValue($this->subs_qnt->DbValue, $this->Recordset->fields('subs_qnt')))
						$this->subs_qnt->CurrentValue = NULL;
					if (!ew_CompareValue($this->subs_price->DbValue, $this->Recordset->fields('subs_price')))
						$this->subs_price->CurrentValue = NULL;
					if (!ew_CompareValue($this->subs_amt->DbValue, $this->Recordset->fields('subs_amt')))
						$this->subs_amt->CurrentValue = NULL;
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->subs_id->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->serv_id->MultiUpdate = $objForm->GetValue("u_serv_id");
		...
		$this->cycle_id->MultiUpdate = $objForm->GetValue("u_cycle_id");
		...
		$this->cons_id->MultiUpdate = $objForm->GetValue("u_cons_id");
		...
		$this->subs_start->MultiUpdate = $objForm->GetValue("u_subs_start");
	...
	function ValidateForm() {
		...
		$gsFormError = "";
		$lUpdateCnt = 0;
		if ($this->serv_id->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->user_id->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->cycle_id->MultiUpdate == "1") $lUpdateCnt++;
		...
		if ($this->cons_id->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->subs_start->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->subs_end->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->subs_qnt->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->subs_price->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->subs_amt->MultiUpdate == "1") $lUpdateCnt++;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// serv_id
			$this->serv_id->SetDbValueDef($rsnew, $this->serv_id->CurrentValue, 0, $this->serv_id->ReadOnly || $this->serv_id->MultiUpdate <> "1");

			// user_id
			$this->user_id->SetDbValueDef($rsnew, $this->user_id->CurrentValue, 0, $this->user_id->ReadOnly || $this->user_id->MultiUpdate <> "1");

			// cycle_id
			$this->cycle_id->SetDbValueDef($rsnew, $this->cycle_id->CurrentValue, 0, $this->cycle_id->ReadOnly || $this->cycle_id->MultiUpdate <> "1");
			...
			// cons_id
			$this->cons_id->SetDbValueDef($rsnew, $this->cons_id->CurrentValue, 0, $this->cons_id->ReadOnly || $this->cons_id->MultiUpdate <> "1");

			// subs_start
			$this->subs_start->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->subs_start->CurrentValue, 0), ew_CurrentDate(), $this->subs_start->ReadOnly || $this->subs_start->MultiUpdate <> "1");

			// subs_end
			$this->subs_end->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->subs_end->CurrentValue, 0), ew_CurrentDate(), $this->subs_end->ReadOnly || $this->subs_end->MultiUpdate <> "1");

			// subs_qnt
			$this->subs_qnt->SetDbValueDef($rsnew, $this->subs_qnt->CurrentValue, 0, $this->subs_qnt->ReadOnly || $this->subs_qnt->MultiUpdate <> "1");

			// subs_price
			$this->subs_price->SetDbValueDef($rsnew, $this->subs_price->CurrentValue, 0, $this->subs_price->ReadOnly || $this->subs_price->MultiUpdate <> "1");

			// subs_amt
			$this->subs_amt->SetDbValueDef($rsnew, $this->subs_amt->CurrentValue, 0, $this->subs_amt->ReadOnly || $this->subs_amt->MultiUpdate <> "1");

			// ins_datetime
			$this->ins_datetime->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->ins_datetime->CurrentValue, 0), ew_CurrentDate(), $this->ins_datetime->ReadOnly || $this->ins_datetime->MultiUpdate <> "1");
...
// Create page object
if (!isset($app_subscribe_update)) $app_subscribe_update = new capp_subscribe_update();

// Page init
$app_subscribe_update->Page_Init();

// Page main
$app_subscribe_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_subscribe_update->Page_Render();
?>
...
var CurrentForm = fapp_subscribeupdate = new ew_Form("fapp_subscribeupdate", "update");

// Validate form
fapp_subscribeupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_subs_start");
			...
			uelm = this.GetElements("u" + infix + "_subs_end");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
			...
			uelm = this.GetElements("u" + infix + "_subs_qnt");
...
fapp_subscribeupdate.Form_CustomValidate = 
...
fapp_subscribeupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_subscribeupdate.Lists["x_serv_id"] = {"LinkField":"x_serv_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_serv_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_service"};
fapp_subscribeupdate.Lists["x_serv_id"].Data = "<?php echo $app_subscribe_update->serv_id->LookupFilterQuery(FALSE, "update") ?>";
fapp_subscribeupdate.Lists["x_user_id"] = {"LinkField":"x_user_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_user_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cpy_user"};
fapp_subscribeupdate.Lists["x_user_id"].Data = "<?php echo $app_subscribe_update->user_id->LookupFilterQuery(FALSE, "update") ?>";
fapp_subscribeupdate.Lists["x_cycle_id"] = {"LinkField":"x_cycle_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_cycle_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_bill_cycle"};
fapp_subscribeupdate.Lists["x_cycle_id"].Data = "<?php echo $app_subscribe_update->cycle_id->LookupFilterQuery(FALSE, "update") ?>";
fapp_subscribeupdate.Lists["x_book_id"] = {"LinkField":"x_book_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_book_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_book"};
fapp_subscribeupdate.Lists["x_book_id"].Data = "<?php echo $app_subscribe_update->book_id->LookupFilterQuery(FALSE, "update") ?>";
fapp_subscribeupdate.Lists["x_pkg_id"] = {"LinkField":"x_pkg_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_pkg_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_tips_package"};
fapp_subscribeupdate.Lists["x_pkg_id"].Data = "<?php echo $app_subscribe_update->pkg_id->LookupFilterQuery(FALSE, "update") ?>";
fapp_subscribeupdate.Lists["x_tcat_id"] = {"LinkField":"x_tcat_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_tcat_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_tips_category"};
fapp_subscribeupdate.Lists["x_tcat_id"].Data = "<?php echo $app_subscribe_update->tcat_id->LookupFilterQuery(FALSE, "update") ?>";
fapp_subscribeupdate.Lists["x_test_id"] = {"LinkField":"x_test_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_test_iname","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_test"};
fapp_subscribeupdate.Lists["x_test_id"].Data = "<?php echo $app_subscribe_update->test_id->LookupFilterQuery(FALSE, "update") ?>";
fapp_subscribeupdate.Lists["x_cat_id"] = {"LinkField":"x_cat_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_cat_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_consultation_category"};
fapp_subscribeupdate.Lists["x_cat_id"].Data = "<?php echo $app_subscribe_update->cat_id->LookupFilterQuery(FALSE, "update") ?>";
fapp_subscribeupdate.Lists["x_cons_id"] = {"LinkField":"x_cons_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_cons_amount","x_cons_message","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_consultation"};
fapp_subscribeupdate.Lists["x_cons_id"].Data = "<?php echo $app_subscribe_update->cons_id->LookupFilterQuery(FALSE, "update") ?>";
...
<?php $app_subscribe_update->ShowPageHeader(); ?>
<?php
$app_subscribe_update->ShowMessage();
?>
<form name="fapp_subscribeupdate" id="fapp_subscribeupdate" class="<?php echo $app_subscribe_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_subscribe_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_subscribe_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_subscribe">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($app_subscribe_update->IsModal) ?>">
<?php foreach ($app_subscribe_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_app_subscribeupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($app_subscribe->serv_id->Visible) { // serv_id ?>
	<div id="r_serv_id" class="form-group">
		<label for="x_serv_id" class="<?php echo $app_subscribe_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_serv_id" id="u_serv_id" class="ewMultiSelect" value="1"<?php echo ($app_subscribe->serv_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_subscribe->serv_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_subscribe_update->RightColumnClass ?>"><div<?php echo $app_subscribe->serv_id->CellAttributes() ?>>
...
<?php if ($app_subscribe->user_id->Visible) { // user_id ?>
	<div id="r_user_id" class="form-group">
		<label for="x_user_id" class="<?php echo $app_subscribe_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_user_id" id="u_user_id" class="ewMultiSelect" value="1"<?php echo ($app_subscribe->user_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_subscribe->user_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_subscribe_update->RightColumnClass ?>"><div<?php echo $app_subscribe->user_id->CellAttributes() ?>>
...
<?php if ($app_subscribe->cycle_id->Visible) { // cycle_id ?>
	<div id="r_cycle_id" class="form-group">
		<label for="x_cycle_id" class="<?php echo $app_subscribe_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_cycle_id" id="u_cycle_id" class="ewMultiSelect" value="1"<?php echo ($app_subscribe->cycle_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_subscribe->cycle_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_subscribe_update->RightColumnClass ?>"><div<?php echo $app_subscribe->cycle_id->CellAttributes() ?>>
...
<?php if ($app_subscribe->book_id->Visible) { // book_id ?>
	<div id="r_book_id" class="form-group">
		<label for="x_book_id" class="<?php echo $app_subscribe_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_book_id" id="u_book_id" class="ewMultiSelect" value="1"<?php echo ($app_subscribe->book_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_subscribe->book_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_subscribe_update->RightColumnClass ?>"><div<?php echo $app_subscribe->book_id->CellAttributes() ?>>
...
<?php if ($app_subscribe->pkg_id->Visible) { // pkg_id ?>
	<div id="r_pkg_id" class="form-group">
		<label for="x_pkg_id" class="<?php echo $app_subscribe_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_pkg_id" id="u_pkg_id" class="ewMultiSelect" value="1"<?php echo ($app_subscribe->pkg_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_subscribe->pkg_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_subscribe_update->RightColumnClass ?>"><div<?php echo $app_subscribe->pkg_id->CellAttributes() ?>>
...
<?php if ($app_subscribe->tcat_id->Visible) { // tcat_id ?>
	<div id="r_tcat_id" class="form-group">
		<label for="x_tcat_id" class="<?php echo $app_subscribe_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_tcat_id" id="u_tcat_id" class="ewMultiSelect" value="1"<?php echo ($app_subscribe->tcat_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_subscribe->tcat_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_subscribe_update->RightColumnClass ?>"><div<?php echo $app_subscribe->tcat_id->CellAttributes() ?>>
...
<?php if ($app_subscribe->test_id->Visible) { // test_id ?>
	<div id="r_test_id" class="form-group">
		<label for="x_test_id" class="<?php echo $app_subscribe_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_test_id" id="u_test_id" class="ewMultiSelect" value="1"<?php echo ($app_subscribe->test_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_subscribe->test_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_subscribe_update->RightColumnClass ?>"><div<?php echo $app_subscribe->test_id->CellAttributes() ?>>
...
<?php if ($app_subscribe->cat_id->Visible) { // cat_id ?>
	<div id="r_cat_id" class="form-group">
		<label for="x_cat_id" class="<?php echo $app_subscribe_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_cat_id" id="u_cat_id" class="ewMultiSelect" value="1"<?php echo ($app_subscribe->cat_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_subscribe->cat_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_subscribe_update->RightColumnClass ?>"><div<?php echo $app_subscribe->cat_id->CellAttributes() ?>>
...
<?php if ($app_subscribe->cons_id->Visible) { // cons_id ?>
	<div id="r_cons_id" class="form-group">
		<label for="x_cons_id" class="<?php echo $app_subscribe_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_cons_id" id="u_cons_id" class="ewMultiSelect" value="1"<?php echo ($app_subscribe->cons_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_subscribe->cons_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_subscribe_update->RightColumnClass ?>"><div<?php echo $app_subscribe->cons_id->CellAttributes() ?>>
...
<?php if ($app_subscribe->subs_start->Visible) { // subs_start ?>
	<div id="r_subs_start" class="form-group">
		<label for="x_subs_start" class="<?php echo $app_subscribe_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_subs_start" id="u_subs_start" class="ewMultiSelect" value="1"<?php echo ($app_subscribe->subs_start->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_subscribe->subs_start->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_subscribe_update->RightColumnClass ?>"><div<?php echo $app_subscribe->subs_start->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("fapp_subscribeupdate", "x_subs_start", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php } ?>
</span>
...
<?php if ($app_subscribe->subs_end->Visible) { // subs_end ?>
	<div id="r_subs_end" class="form-group">
		<label for="x_subs_end" class="<?php echo $app_subscribe_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_subs_end" id="u_subs_end" class="ewMultiSelect" value="1"<?php echo ($app_subscribe->subs_end->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_subscribe->subs_end->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_subscribe_update->RightColumnClass ?>"><div<?php echo $app_subscribe->subs_end->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("fapp_subscribeupdate", "x_subs_end", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php } ?>
</span>
...
<?php if ($app_subscribe->subs_qnt->Visible) { // subs_qnt ?>
	<div id="r_subs_qnt" class="form-group">
		<label for="x_subs_qnt" class="<?php echo $app_subscribe_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_subs_qnt" id="u_subs_qnt" class="ewMultiSelect" value="1"<?php echo ($app_subscribe->subs_qnt->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_subscribe->subs_qnt->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_subscribe_update->RightColumnClass ?>"><div<?php echo $app_subscribe->subs_qnt->CellAttributes() ?>>
...
<?php if ($app_subscribe->subs_price->Visible) { // subs_price ?>
	<div id="r_subs_price" class="form-group">
		<label for="x_subs_price" class="<?php echo $app_subscribe_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_subs_price" id="u_subs_price" class="ewMultiSelect" value="1"<?php echo ($app_subscribe->subs_price->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_subscribe->subs_price->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_subscribe_update->RightColumnClass ?>"><div<?php echo $app_subscribe->subs_price->CellAttributes() ?>>
...
<?php if ($app_subscribe->subs_amt->Visible) { // subs_amt ?>
	<div id="r_subs_amt" class="form-group">
		<label for="x_subs_amt" class="<?php echo $app_subscribe_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_subs_amt" id="u_subs_amt" class="ewMultiSelect" value="1"<?php echo ($app_subscribe->subs_amt->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_subscribe->subs_amt->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_subscribe_update->RightColumnClass ?>"><div<?php echo $app_subscribe->subs_amt->CellAttributes() ?>>
...
<?php if ($app_subscribe->ins_datetime->Visible) { // ins_datetime ?>
	<div id="r_ins_datetime" class="form-group">
		<label for="x_ins_datetime" class="<?php echo $app_subscribe_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_ins_datetime" id="u_ins_datetime" class="ewMultiSelect" value="1"<?php echo ($app_subscribe->ins_datetime->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_subscribe->ins_datetime->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_subscribe_update->RightColumnClass ?>"><div<?php echo $app_subscribe->ins_datetime->CellAttributes() ?>>
...
<?php echo $app_subscribe->ins_datetime->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page -->
<?php if (!$app_subscribe_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $app_subscribe_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $app_subscribe_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
	</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fapp_subscribeupdate.Init();
</script>
<?php
$app_subscribe_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$app_subscribe_update->Page_Terminate();
?>
