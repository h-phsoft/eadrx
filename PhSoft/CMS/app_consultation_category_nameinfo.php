...
$app_consultation_category_name = NULL;

//
// Table class for app_consultation_category_name
//
class capp_consultation_category_name extends cTable {
	var $ncat_id;
	var $cat_id;
	var $lang_id;
	var $cat_name;
	...
	function __construct() {
		...
		$this->TableVar = 'app_consultation_category_name';
		$this->TableName = 'app_consultation_category_name';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`app_consultation_category_name`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// ncat_id
		$this->ncat_id = new cField('app_consultation_category_name', 'app_consultation_category_name', 'x_ncat_id', 'ncat_id', '`ncat_id`', '`ncat_id`', 3, -1, FALSE, '`ncat_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// cat_id
		$this->cat_id = new cField('app_consultation_category_name', 'app_consultation_category_name', 'x_cat_id', 'cat_id', '`cat_id`', '`cat_id`', 3, -1, FALSE, '`cat_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// lang_id
		$this->lang_id = new cField('app_consultation_category_name', 'app_consultation_category_name', 'x_lang_id', 'lang_id', '`lang_id`', '`lang_id`', 3, -1, FALSE, '`lang_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// cat_name
		$this->cat_name = new cField('app_consultation_category_name', 'app_consultation_category_name', 'x_cat_name', 'cat_name', '`cat_name`', '`cat_name`', 200, -1, FALSE, '`cat_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->cat_name->Sortable = TRUE; // Allow sort
		$this->fields['cat_name'] = &$this->cat_name;

		// cat_desc
		$this->cat_desc = new cField('app_consultation_category_name', 'app_consultation_category_name', 'x_cat_desc', 'cat_desc', '`cat_desc`', '`cat_desc`', 201, -1, FALSE, '`cat_desc`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "app_consultation_category") {
			if ($this->cat_id->getSessionValue() <> "")
				$sMasterFilter .= "`cat_id`=" . ew_QuotedValue($this->cat_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "app_consultation_category") {
			if ($this->cat_id->getSessionValue() <> "")
				$sDetailFilter .= "`cat_id`=" . ew_QuotedValue($this->cat_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_app_consultation_category() {
		return "`cat_id`=@cat_id@";
	}

	// Detail filter
	function SqlDetailFilter_app_consultation_category() {
		return "`cat_id`=@cat_id@";
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`app_consultation_category_name`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->ncat_id->setDbValue($conn->Insert_ID());
			$rs['ncat_id'] = $this->ncat_id->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('ncat_id', $rs))
				ew_AddFilter($where, ew_QuotedName('ncat_id', $this->DBID) . '=' . ew_QuotedValue($rs['ncat_id'], $this->ncat_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`ncat_id` = @ncat_id@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->ncat_id->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->ncat_id->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@ncat_id@", ew_AdjustSql($this->ncat_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "app_consultation_category_namelist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "app_consultation_category_nameedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "app_consultation_category_nameadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("app_consultation_category_nameview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("app_consultation_category_nameview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "app_consultation_category_nameadd.php?" . $this->UrlParm($parm);
		else
			$url = "app_consultation_category_nameadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("app_consultation_category_nameedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("app_consultation_category_nameadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("app_consultation_category_namedelete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "app_consultation_category" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
			$url .= (strpos($url, "?") !== FALSE ? "&" : "?") . EW_TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_cat_id=" . urlencode($this->cat_id->CurrentValue);
		}
		return $url;
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "ncat_id:" . ew_VarToJson($this->ncat_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->ncat_id->CurrentValue)) {
			$sUrl .= "ncat_id=" . urlencode($this->ncat_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["ncat_id"]))
				$arKeys[] = $_POST["ncat_id"];
			elseif (isset($_GET["ncat_id"]))
				$arKeys[] = $_GET["ncat_id"];
			else
