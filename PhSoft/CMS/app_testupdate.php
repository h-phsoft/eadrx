...
$app_test_update = NULL; // Initialize page object first

class capp_test_update extends capp_test {
	...
	// Page object name
	var $PageObjName = 'app_test_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("app_testlist.php"); // No records selected, return to list
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					...
					$this->test_num->setDbValue($this->Recordset->fields('test_num'));
					$this->test_iname->setDbValue($this->Recordset->fields('test_iname'));
					$this->test_price->setDbValue($this->Recordset->fields('test_price'));
				} else {
					if (!ew_CompareValue($this->status_id->DbValue, $this->Recordset->fields('status_id')))
						$this->status_id->CurrentValue = NULL;
					if (!ew_CompareValue($this->test_num->DbValue, $this->Recordset->fields('test_num')))
						$this->test_num->CurrentValue = NULL;
					if (!ew_CompareValue($this->test_iname->DbValue, $this->Recordset->fields('test_iname')))
						$this->test_iname->CurrentValue = NULL;
					if (!ew_CompareValue($this->test_price->DbValue, $this->Recordset->fields('test_price')))
						$this->test_price->CurrentValue = NULL;
				}
				$i++;
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->test_id->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function GetUploadFiles() {
		...
		$this->test_image->MultiUpdate = $objForm->GetValue("u_test_image");
	}
	...
	function ValidateForm() {
		...
		if ($this->test_num->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->test_iname->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->test_price->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->test_image->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
		...
		if ($this->test_num->MultiUpdate <> "" && !$this->test_num->FldIsDetailKey && !is_null($this->test_num->FormValue) && $this->test_num->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->test_num->FldCaption(), $this->test_num->ReqErrMsg));
		}
		if ($this->test_num->MultiUpdate <> "") {
			if (!ew_CheckInteger($this->test_num->FormValue)) {
				ew_AddMessage($gsFormError, $this->test_num->FldErrMsg());
			}
		}
		if ($this->test_iname->MultiUpdate <> "" && !$this->test_iname->FldIsDetailKey && !is_null($this->test_iname->FormValue) && $this->test_iname->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->test_iname->FldCaption(), $this->test_iname->ReqErrMsg));
		}
		if ($this->test_price->MultiUpdate <> "" && !$this->test_price->FldIsDetailKey && !is_null($this->test_price->FormValue) && $this->test_price->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->test_price->FldCaption(), $this->test_price->ReqErrMsg));
		}
		if ($this->test_price->MultiUpdate <> "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// test_num
			$this->test_num->SetDbValueDef($rsnew, $this->test_num->CurrentValue, 0, $this->test_num->ReadOnly || $this->test_num->MultiUpdate <> "1");

			// test_iname
			$this->test_iname->SetDbValueDef($rsnew, $this->test_iname->CurrentValue, "", $this->test_iname->ReadOnly || $this->test_iname->MultiUpdate <> "1");

			// test_price
			$this->test_price->SetDbValueDef($rsnew, $this->test_price->CurrentValue, 0, $this->test_price->ReadOnly || $this->test_price->MultiUpdate <> "1");

			// test_image
			if ($this->test_image->Visible && !$this->test_image->ReadOnly && strval($this->test_image->MultiUpdate) == "1" && !$this->test_image->Upload->KeepFile) {
			...
			if ($this->test_image->Visible && !$this->test_image->Upload->KeepFile) {
				...
				if (!ew_Empty($this->test_image->Upload->FileName) && $this->UpdateCount == 1) {
					...
					$this->test_image->SetDbValueDef($rsnew, $this->test_image->Upload->FileName, NULL, $this->test_image->ReadOnly || $this->test_image->MultiUpdate <> "1");
				}
			}
...
// Create page object
if (!isset($app_test_update)) $app_test_update = new capp_test_update();

// Page init
$app_test_update->Page_Init();

// Page main
$app_test_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_test_update->Page_Render();
?>
...
var CurrentForm = fapp_testupdate = new ew_Form("fapp_testupdate", "update");

// Validate form
fapp_testupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_test_num");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
			...
			uelm = this.GetElements("u" + infix + "_test_iname");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
			...
			uelm = this.GetElements("u" + infix + "_test_price");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
...
fapp_testupdate.Form_CustomValidate = 
...
fapp_testupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_testupdate.Lists["x_status_id"] = {"LinkField":"x_status_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_status_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_status"};
fapp_testupdate.Lists["x_status_id"].Data = "<?php echo $app_test_update->status_id->LookupFilterQuery(FALSE, "update") ?>";
...
<?php $app_test_update->ShowPageHeader(); ?>
<?php
$app_test_update->ShowMessage();
?>
<form name="fapp_testupdate" id="fapp_testupdate" class="<?php echo $app_test_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_test_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_test_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_test">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($app_test_update->IsModal) ?>">
<?php foreach ($app_test_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_app_testupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($app_test->status_id->Visible) { // status_id ?>
	<div id="r_status_id" class="form-group">
		<label for="x_status_id" class="<?php echo $app_test_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_status_id" id="u_status_id" class="ewMultiSelect" value="1"<?php echo ($app_test->status_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test->status_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_update->RightColumnClass ?>"><div<?php echo $app_test->status_id->CellAttributes() ?>>
...
<?php if ($app_test->test_num->Visible) { // test_num ?>
	<div id="r_test_num" class="form-group">
		<label for="x_test_num" class="<?php echo $app_test_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_test_num" id="u_test_num" class="ewMultiSelect" value="1"<?php echo ($app_test->test_num->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test->test_num->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_update->RightColumnClass ?>"><div<?php echo $app_test->test_num->CellAttributes() ?>>
...
<?php if ($app_test->test_iname->Visible) { // test_iname ?>
	<div id="r_test_iname" class="form-group">
		<label for="x_test_iname" class="<?php echo $app_test_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_test_iname" id="u_test_iname" class="ewMultiSelect" value="1"<?php echo ($app_test->test_iname->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test->test_iname->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_update->RightColumnClass ?>"><div<?php echo $app_test->test_iname->CellAttributes() ?>>
...
<?php if ($app_test->test_price->Visible) { // test_price ?>
	<div id="r_test_price" class="form-group">
		<label for="x_test_price" class="<?php echo $app_test_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_test_price" id="u_test_price" class="ewMultiSelect" value="1"<?php echo ($app_test->test_price->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test->test_price->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_update->RightColumnClass ?>"><div<?php echo $app_test->test_price->CellAttributes() ?>>
...
<?php if ($app_test->test_image->Visible) { // test_image ?>
	<div id="r_test_image" class="form-group">
		<label class="<?php echo $app_test_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_test_image" id="u_test_image" class="ewMultiSelect" value="1"<?php echo ($app_test->test_image->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test->test_image->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_update->RightColumnClass ?>"><div<?php echo $app_test->test_image->CellAttributes() ?>>
...
<?php if (!$app_test_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $app_test_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $app_test_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
	</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fapp_testupdate.Init();
</script>
<?php
$app_test_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$app_test_update->Page_Terminate();
?>
