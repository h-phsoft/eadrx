...
$cpy_faq_category_update = NULL; // Initialize page object first

class ccpy_faq_category_update extends ccpy_faq_category {
	...
	// Page object name
	var $PageObjName = 'cpy_faq_category_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("cpy_faq_categorylist.php"); // No records selected, return to list
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					...
					$this->fcat_order->setDbValue($this->Recordset->fields('fcat_order'));
					$this->fcat_name->setDbValue($this->Recordset->fields('fcat_name'));
				} else {
					if (!ew_CompareValue($this->status_id->DbValue, $this->Recordset->fields('status_id')))
						$this->status_id->CurrentValue = NULL;
					if (!ew_CompareValue($this->fcat_order->DbValue, $this->Recordset->fields('fcat_order')))
						$this->fcat_order->CurrentValue = NULL;
					if (!ew_CompareValue($this->fcat_name->DbValue, $this->Recordset->fields('fcat_name')))
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->fcat_id->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->fcat_order->MultiUpdate = $objForm->GetValue("u_fcat_order");
		if (!$this->fcat_name->FldIsDetailKey) {
			$this->fcat_name->setFormValue($objForm->GetValue("x_fcat_name"));
		}
		$this->fcat_name->MultiUpdate = $objForm->GetValue("u_fcat_name");
	...
	function ValidateForm() {
		...
		if ($this->fcat_order->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->fcat_name->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
		...
		if ($this->fcat_order->MultiUpdate <> "" && !$this->fcat_order->FldIsDetailKey && !is_null($this->fcat_order->FormValue) && $this->fcat_order->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->fcat_order->FldCaption(), $this->fcat_order->ReqErrMsg));
		}
		if ($this->fcat_order->MultiUpdate <> "") {
			if (!ew_CheckInteger($this->fcat_order->FormValue)) {
				ew_AddMessage($gsFormError, $this->fcat_order->FldErrMsg());
			}
		}
		if ($this->fcat_name->MultiUpdate <> "" && !$this->fcat_name->FldIsDetailKey && !is_null($this->fcat_name->FormValue) && $this->fcat_name->FormValue == "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// fcat_order
			$this->fcat_order->SetDbValueDef($rsnew, $this->fcat_order->CurrentValue, 0, $this->fcat_order->ReadOnly || $this->fcat_order->MultiUpdate <> "1");

			// fcat_name
			$this->fcat_name->SetDbValueDef($rsnew, $this->fcat_name->CurrentValue, "", $this->fcat_name->ReadOnly || $this->fcat_name->MultiUpdate <> "1");
...
// Create page object
if (!isset($cpy_faq_category_update)) $cpy_faq_category_update = new ccpy_faq_category_update();

// Page init
$cpy_faq_category_update->Page_Init();

// Page main
$cpy_faq_category_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cpy_faq_category_update->Page_Render();
?>
...
var CurrentForm = fcpy_faq_categoryupdate = new ew_Form("fcpy_faq_categoryupdate", "update");

// Validate form
fcpy_faq_categoryupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_fcat_order");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
			...
			uelm = this.GetElements("u" + infix + "_fcat_name");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
...
fcpy_faq_categoryupdate.Form_CustomValidate = 
...
fcpy_faq_categoryupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fcpy_faq_categoryupdate.Lists["x_status_id"] = {"LinkField":"x_status_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_status_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_status"};
fcpy_faq_categoryupdate.Lists["x_status_id"].Data = "<?php echo $cpy_faq_category_update->status_id->LookupFilterQuery(FALSE, "update") ?>";
...
<?php $cpy_faq_category_update->ShowPageHeader(); ?>
<?php
$cpy_faq_category_update->ShowMessage();
?>
<form name="fcpy_faq_categoryupdate" id="fcpy_faq_categoryupdate" class="<?php echo $cpy_faq_category_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cpy_faq_category_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cpy_faq_category_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="cpy_faq_category">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($cpy_faq_category_update->IsModal) ?>">
<?php foreach ($cpy_faq_category_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_cpy_faq_categoryupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($cpy_faq_category->status_id->Visible) { // status_id ?>
	<div id="r_status_id" class="form-group">
		<label for="x_status_id" class="<?php echo $cpy_faq_category_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_status_id" id="u_status_id" class="ewMultiSelect" value="1"<?php echo ($cpy_faq_category->status_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $cpy_faq_category->status_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $cpy_faq_category_update->RightColumnClass ?>"><div<?php echo $cpy_faq_category->status_id->CellAttributes() ?>>
...
<?php if ($cpy_faq_category->fcat_order->Visible) { // fcat_order ?>
	<div id="r_fcat_order" class="form-group">
		<label for="x_fcat_order" class="<?php echo $cpy_faq_category_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_fcat_order" id="u_fcat_order" class="ewMultiSelect" value="1"<?php echo ($cpy_faq_category->fcat_order->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $cpy_faq_category->fcat_order->FldCaption() ?></label></div></label>
		<div class="<?php echo $cpy_faq_category_update->RightColumnClass ?>"><div<?php echo $cpy_faq_category->fcat_order->CellAttributes() ?>>
...
<?php if ($cpy_faq_category->fcat_name->Visible) { // fcat_name ?>
	<div id="r_fcat_name" class="form-group">
		<label for="x_fcat_name" class="<?php echo $cpy_faq_category_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_fcat_name" id="u_fcat_name" class="ewMultiSelect" value="1"<?php echo ($cpy_faq_category->fcat_name->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $cpy_faq_category->fcat_name->FldCaption() ?></label></div></label>
		<div class="<?php echo $cpy_faq_category_update->RightColumnClass ?>"><div<?php echo $cpy_faq_category->fcat_name->CellAttributes() ?>>
...
<?php if (!$cpy_faq_category_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $cpy_faq_category_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cpy_faq_category_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
	</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fcpy_faq_categoryupdate.Init();
</script>
<?php
$cpy_faq_category_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cpy_faq_category_update->Page_Terminate();
?>
