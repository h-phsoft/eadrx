...
$app_consultation = NULL;

//
// Table class for app_consultation
//
class capp_consultation extends cTable {
	var $cons_id;
	var $user_id;
	var $cstatus_id;
	var $cat_id;
	...
	function __construct() {
		...
		$this->TableVar = 'app_consultation';
		$this->TableName = 'app_consultation';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`app_consultation`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// cons_id
		$this->cons_id = new cField('app_consultation', 'app_consultation', 'x_cons_id', 'cons_id', '`cons_id`', '`cons_id`', 3, -1, FALSE, '`cons_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->cons_id->Sortable = FALSE; // Allow sort
		$this->cons_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['cons_id'] = &$this->cons_id;

		// user_id
		$this->user_id = new cField('app_consultation', 'app_consultation', 'x_user_id', 'user_id', '`user_id`', '`user_id`', 3, -1, FALSE, '`user_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// cstatus_id
		$this->cstatus_id = new cField('app_consultation', 'app_consultation', 'x_cstatus_id', 'cstatus_id', '`cstatus_id`', '`cstatus_id`', 3, -1, FALSE, '`cstatus_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// cat_id
		$this->cat_id = new cField('app_consultation', 'app_consultation', 'x_cat_id', 'cat_id', '`cat_id`', '`cat_id`', 3, -1, FALSE, '`cat_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// cons_amount
		$this->cons_amount = new cField('app_consultation', 'app_consultation', 'x_cons_amount', 'cons_amount', '`cons_amount`', '`cons_amount`', 131, -1, FALSE, '`cons_amount`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->cons_amount->Sortable = TRUE; // Allow sort
		$this->cons_amount->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['cons_amount'] = &$this->cons_amount;

		// cons_file
		$this->cons_file = new cField('app_consultation', 'app_consultation', 'x_cons_file', 'cons_file', '`cons_file`', '`cons_file`', 200, -1, FALSE, '`cons_file`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->cons_file->Sortable = TRUE; // Allow sort
		$this->fields['cons_file'] = &$this->cons_file;

		// cons_audio
		$this->cons_audio = new cField('app_consultation', 'app_consultation', 'x_cons_audio', 'cons_audio', '`cons_audio`', '`cons_audio`', 200, -1, FALSE, '`cons_audio`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->cons_audio->Sortable = TRUE; // Allow sort
		$this->fields['cons_audio'] = &$this->cons_audio;

		// cons_message
		$this->cons_message = new cField('app_consultation', 'app_consultation', 'x_cons_message', 'cons_message', '`cons_message`', '`cons_message`', 201, -1, FALSE, '`cons_message`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->cons_message->Sortable = TRUE; // Allow sort
		$this->fields['cons_message'] = &$this->cons_message;

		// ins_datetime
		$this->ins_datetime = new cField('app_consultation', 'app_consultation', 'x_ins_datetime', 'ins_datetime', '`ins_datetime`', ew_CastDateFieldForLike('`ins_datetime`', 11, "DB"), 135, 11, FALSE, '`ins_datetime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "app_consult_assign") {
			$sDetailUrl = $GLOBALS["app_consult_assign"]->GetListUrl() . "?" . EW_TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$sDetailUrl .= "&fk_cons_id=" . urlencode($this->cons_id->CurrentValue);
		}
		if ($this->getCurrentDetailTable() == "app_consult_answer") {
			$sDetailUrl = $GLOBALS["app_consult_answer"]->GetListUrl() . "?" . EW_TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$sDetailUrl .= "&fk_cons_id=" . urlencode($this->cons_id->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "app_consultationlist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`app_consultation`";
	}
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "`ins_datetime` DESC";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->cons_id->setDbValue($conn->Insert_ID());
			$rs['cons_id'] = $this->cons_id->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('cons_id', $rs))
				ew_AddFilter($where, ew_QuotedName('cons_id', $this->DBID) . '=' . ew_QuotedValue($rs['cons_id'], $this->cons_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`cons_id` = @cons_id@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->cons_id->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->cons_id->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@cons_id@", ew_AdjustSql($this->cons_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "app_consultationlist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "app_consultationedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "app_consultationadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("app_consultationview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("app_consultationview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "app_consultationadd.php?" . $this->UrlParm($parm);
		else
			$url = "app_consultationadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("app_consultationedit.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("app_consultationedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("app_consultationadd.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("app_consultationadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("app_consultationdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "cons_id:" . ew_VarToJson($this->cons_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->cons_id->CurrentValue)) {
			$sUrl .= "cons_id=" . urlencode($this->cons_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["cons_id"]))
				$arKeys[] = $_POST["cons_id"];
			elseif (isset($_GET["cons_id"]))
				$arKeys[] = $_GET["cons_id"];
			else
	...
	function LoadListRowValues(&$rs) {
		...
		$this->cons_amount->setDbValue($rs->fields('cons_amount'));
		$this->cons_file->setDbValue($rs->fields('cons_file'));
		$this->cons_audio->setDbValue($rs->fields('cons_audio'));
		$this->cons_message->setDbValue($rs->fields('cons_message'));
	...
	function RenderEditRow() {
		...
		$this->cons_id->EditValue = $this->cons_id->CurrentValue;
		$this->cons_id->ViewCustomAttributes = "";
		...
		$this->cons_amount->EditValue = $this->cons_amount->CurrentValue;
		...
		$this->cons_file->EditValue = $this->cons_file->CurrentValue;
		...
		$this->cons_audio->EditValue = $this->cons_audio->CurrentValue;
		...
		$this->cons_message->EditValue = $this->cons_message->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->cons_amount->Exportable) $Doc->ExportCaption($this->cons_amount);
					if ($this->cons_file->Exportable) $Doc->ExportCaption($this->cons_file);
					if ($this->cons_audio->Exportable) $Doc->ExportCaption($this->cons_audio);
					if ($this->cons_message->Exportable) $Doc->ExportCaption($this->cons_message);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->cons_amount->Exportable) $Doc->ExportField($this->cons_amount);
						if ($this->cons_file->Exportable) $Doc->ExportField($this->cons_file);
						if ($this->cons_audio->Exportable) $Doc->ExportField($this->cons_audio);
						if ($this->cons_message->Exportable) $Doc->ExportField($this->cons_message);
