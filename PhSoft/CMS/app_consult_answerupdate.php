...
$app_consult_answer_update = NULL; // Initialize page object first

class capp_consult_answer_update extends capp_consult_answer {
	...
	// Page object name
	var $PageObjName = 'app_consult_answer_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("app_consult_answerlist.php"); // No records selected, return to list
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					...
					$this->answer_file->setDbValue($this->Recordset->fields('answer_file'));
					$this->answer_audio->setDbValue($this->Recordset->fields('answer_audio'));
					$this->answer_text->setDbValue($this->Recordset->fields('answer_text'));
					...
					if (!ew_CompareValue($this->answer_file->DbValue, $this->Recordset->fields('answer_file')))
						$this->answer_file->CurrentValue = NULL;
					if (!ew_CompareValue($this->answer_audio->DbValue, $this->Recordset->fields('answer_audio')))
						$this->answer_audio->CurrentValue = NULL;
					if (!ew_CompareValue($this->answer_text->DbValue, $this->Recordset->fields('answer_text')))
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->answer_id->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function ValidateForm() {
		...
		if ($this->answer_file->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->answer_audio->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->answer_text->MultiUpdate == "1") $lUpdateCnt++;
		...
		if ($this->answer_text->MultiUpdate <> "" && !$this->answer_text->FldIsDetailKey && !is_null($this->answer_text->FormValue) && $this->answer_text->FormValue == "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// answer_file
			$this->answer_file->SetDbValueDef($rsnew, $this->answer_file->CurrentValue, NULL, $this->answer_file->ReadOnly || $this->answer_file->MultiUpdate <> "1");

			// answer_audio
			$this->answer_audio->SetDbValueDef($rsnew, $this->answer_audio->CurrentValue, NULL, $this->answer_audio->ReadOnly || $this->answer_audio->MultiUpdate <> "1");

			// answer_text
			$this->answer_text->SetDbValueDef($rsnew, $this->answer_text->CurrentValue, "", $this->answer_text->ReadOnly || $this->answer_text->MultiUpdate <> "1");
...
// Create page object
if (!isset($app_consult_answer_update)) $app_consult_answer_update = new capp_consult_answer_update();

// Page init
$app_consult_answer_update->Page_Init();

// Page main
$app_consult_answer_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_consult_answer_update->Page_Render();
?>
...
var CurrentForm = fapp_consult_answerupdate = new ew_Form("fapp_consult_answerupdate", "update");

// Validate form
fapp_consult_answerupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_answer_text");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
...
fapp_consult_answerupdate.Form_CustomValidate = 
...
fapp_consult_answerupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_consult_answerupdate.Lists["x_user_id"] = {"LinkField":"x_user_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_user_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cpy_user"};
fapp_consult_answerupdate.Lists["x_user_id"].Data = "<?php echo $app_consult_answer_update->user_id->LookupFilterQuery(FALSE, "update") ?>";
fapp_consult_answerupdate.Lists["x_cons_id"] = {"LinkField":"x_cons_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_cons_message","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_consultation"};
fapp_consult_answerupdate.Lists["x_cons_id"].Data = "<?php echo $app_consult_answer_update->cons_id->LookupFilterQuery(FALSE, "update") ?>";
...
<?php $app_consult_answer_update->ShowPageHeader(); ?>
<?php
$app_consult_answer_update->ShowMessage();
?>
<form name="fapp_consult_answerupdate" id="fapp_consult_answerupdate" class="<?php echo $app_consult_answer_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_consult_answer_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_consult_answer_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_consult_answer">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($app_consult_answer_update->IsModal) ?>">
<?php foreach ($app_consult_answer_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_app_consult_answerupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($app_consult_answer->user_id->Visible) { // user_id ?>
	<div id="r_user_id" class="form-group">
		<label for="x_user_id" class="<?php echo $app_consult_answer_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_user_id" id="u_user_id" class="ewMultiSelect" value="1"<?php echo ($app_consult_answer->user_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_consult_answer->user_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_consult_answer_update->RightColumnClass ?>"><div<?php echo $app_consult_answer->user_id->CellAttributes() ?>>
...
<?php if ($app_consult_answer->cons_id->Visible) { // cons_id ?>
	<div id="r_cons_id" class="form-group">
		<label for="x_cons_id" class="<?php echo $app_consult_answer_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_cons_id" id="u_cons_id" class="ewMultiSelect" value="1"<?php echo ($app_consult_answer->cons_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_consult_answer->cons_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_consult_answer_update->RightColumnClass ?>"><div<?php echo $app_consult_answer->cons_id->CellAttributes() ?>>
...
<?php if ($app_consult_answer->answer_file->Visible) { // answer_file ?>
	<div id="r_answer_file" class="form-group">
		<label for="x_answer_file" class="<?php echo $app_consult_answer_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_answer_file" id="u_answer_file" class="ewMultiSelect" value="1"<?php echo ($app_consult_answer->answer_file->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_consult_answer->answer_file->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_consult_answer_update->RightColumnClass ?>"><div<?php echo $app_consult_answer->answer_file->CellAttributes() ?>>
...
<?php if ($app_consult_answer->answer_audio->Visible) { // answer_audio ?>
	<div id="r_answer_audio" class="form-group">
		<label for="x_answer_audio" class="<?php echo $app_consult_answer_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_answer_audio" id="u_answer_audio" class="ewMultiSelect" value="1"<?php echo ($app_consult_answer->answer_audio->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_consult_answer->answer_audio->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_consult_answer_update->RightColumnClass ?>"><div<?php echo $app_consult_answer->answer_audio->CellAttributes() ?>>
...
<?php if ($app_consult_answer->answer_text->Visible) { // answer_text ?>
	<div id="r_answer_text" class="form-group">
		<label class="<?php echo $app_consult_answer_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_answer_text" id="u_answer_text" class="ewMultiSelect" value="1"<?php echo ($app_consult_answer->answer_text->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_consult_answer->answer_text->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_consult_answer_update->RightColumnClass ?>"><div<?php echo $app_consult_answer->answer_text->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateEditor("fapp_consult_answerupdate", "x_answer_text", 35, 4, <?php echo ($app_consult_answer->answer_text->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<?php if ($app_consult_answer->ins_datetime->Visible) { // ins_datetime ?>
	<div id="r_ins_datetime" class="form-group">
		<label for="x_ins_datetime" class="<?php echo $app_consult_answer_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_ins_datetime" id="u_ins_datetime" class="ewMultiSelect" value="1"<?php echo ($app_consult_answer->ins_datetime->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_consult_answer->ins_datetime->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_consult_answer_update->RightColumnClass ?>"><div<?php echo $app_consult_answer->ins_datetime->CellAttributes() ?>>
...
<?php echo $app_consult_answer->ins_datetime->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page -->
<?php if (!$app_consult_answer_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $app_consult_answer_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $app_consult_answer_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
	</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fapp_consult_answerupdate.Init();
</script>
<?php
$app_consult_answer_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$app_consult_answer_update->Page_Terminate();
?>
