...
$app_tips_pkg_cat_grid = NULL; // Initialize page object first

class capp_tips_pkg_cat_grid extends capp_tips_pkg_cat {
	...
	// Page object name
	var $PageObjName = 'app_tips_pkg_cat_grid';

	// Grid form hidden field names
	var $FormName = 'fapp_tips_pkg_catgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["app_tips_pkg_cat"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->tpkg_id->CurrentValue = strval($arKeys[0]); // tpkg_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "app_tips_package") {
			$this->pkg_id->Visible = FALSE;
			if ($GLOBALS["app_tips_package"]->EventCancelled) $this->EventCancelled = TRUE;
		}
