...
$app_contactus_update = NULL; // Initialize page object first

class capp_contactus_update extends capp_contactus {
	...
	// Page object name
	var $PageObjName = 'app_contactus_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("app_contactuslist.php"); // No records selected, return to list
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					...
					$this->cont_subj->setDbValue($this->Recordset->fields('cont_subj'));
					$this->cont_msg->setDbValue($this->Recordset->fields('cont_msg'));
					$this->cont_datetime->setDbValue($this->Recordset->fields('cont_datetime'));
				} else {
					if (!ew_CompareValue($this->user_id->DbValue, $this->Recordset->fields('user_id')))
						$this->user_id->CurrentValue = NULL;
					if (!ew_CompareValue($this->cont_subj->DbValue, $this->Recordset->fields('cont_subj')))
						$this->cont_subj->CurrentValue = NULL;
					if (!ew_CompareValue($this->cont_msg->DbValue, $this->Recordset->fields('cont_msg')))
						$this->cont_msg->CurrentValue = NULL;
					if (!ew_CompareValue($this->cont_datetime->DbValue, $this->Recordset->fields('cont_datetime')))
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->cont_id->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->cont_subj->MultiUpdate = $objForm->GetValue("u_cont_subj");
		if (!$this->cont_msg->FldIsDetailKey) {
			$this->cont_msg->setFormValue($objForm->GetValue("x_cont_msg"));
		}
		$this->cont_msg->MultiUpdate = $objForm->GetValue("u_cont_msg");
		...
		$this->cont_datetime->MultiUpdate = $objForm->GetValue("u_cont_datetime");
	...
	function ValidateForm() {
		...
		if ($this->cont_subj->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->cont_msg->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->cont_datetime->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
		...
		if ($this->cont_subj->MultiUpdate <> "" && !$this->cont_subj->FldIsDetailKey && !is_null($this->cont_subj->FormValue) && $this->cont_subj->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->cont_subj->FldCaption(), $this->cont_subj->ReqErrMsg));
		}
		if ($this->cont_msg->MultiUpdate <> "" && !$this->cont_msg->FldIsDetailKey && !is_null($this->cont_msg->FormValue) && $this->cont_msg->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->cont_msg->FldCaption(), $this->cont_msg->ReqErrMsg));
		}
		if ($this->cont_datetime->MultiUpdate <> "" && !$this->cont_datetime->FldIsDetailKey && !is_null($this->cont_datetime->FormValue) && $this->cont_datetime->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->cont_datetime->FldCaption(), $this->cont_datetime->ReqErrMsg));
		}
		if ($this->cont_datetime->MultiUpdate <> "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// cont_subj
			$this->cont_subj->SetDbValueDef($rsnew, $this->cont_subj->CurrentValue, "", $this->cont_subj->ReadOnly || $this->cont_subj->MultiUpdate <> "1");

			// cont_msg
			$this->cont_msg->SetDbValueDef($rsnew, $this->cont_msg->CurrentValue, "", $this->cont_msg->ReadOnly || $this->cont_msg->MultiUpdate <> "1");

			// cont_datetime
			$this->cont_datetime->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->cont_datetime->CurrentValue, 0), ew_CurrentDate(), $this->cont_datetime->ReadOnly || $this->cont_datetime->MultiUpdate <> "1");
...
// Create page object
if (!isset($app_contactus_update)) $app_contactus_update = new capp_contactus_update();

// Page init
$app_contactus_update->Page_Init();

// Page main
$app_contactus_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_contactus_update->Page_Render();
?>
...
var CurrentForm = fapp_contactusupdate = new ew_Form("fapp_contactusupdate", "update");

// Validate form
fapp_contactusupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_cont_subj");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
			...
			uelm = this.GetElements("u" + infix + "_cont_msg");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
			...
			uelm = this.GetElements("u" + infix + "_cont_datetime");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
...
fapp_contactusupdate.Form_CustomValidate = 
...
fapp_contactusupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_contactusupdate.Lists["x_user_id"] = {"LinkField":"x_user_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_user_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cpy_user"};
fapp_contactusupdate.Lists["x_user_id"].Data = "<?php echo $app_contactus_update->user_id->LookupFilterQuery(FALSE, "update") ?>";
...
<?php $app_contactus_update->ShowPageHeader(); ?>
<?php
$app_contactus_update->ShowMessage();
?>
<form name="fapp_contactusupdate" id="fapp_contactusupdate" class="<?php echo $app_contactus_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_contactus_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_contactus_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_contactus">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($app_contactus_update->IsModal) ?>">
<?php foreach ($app_contactus_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_app_contactusupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($app_contactus->user_id->Visible) { // user_id ?>
	<div id="r_user_id" class="form-group">
		<label for="x_user_id" class="<?php echo $app_contactus_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_user_id" id="u_user_id" class="ewMultiSelect" value="1"<?php echo ($app_contactus->user_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_contactus->user_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_contactus_update->RightColumnClass ?>"><div<?php echo $app_contactus->user_id->CellAttributes() ?>>
...
<?php if ($app_contactus->cont_subj->Visible) { // cont_subj ?>
	<div id="r_cont_subj" class="form-group">
		<label for="x_cont_subj" class="<?php echo $app_contactus_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_cont_subj" id="u_cont_subj" class="ewMultiSelect" value="1"<?php echo ($app_contactus->cont_subj->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_contactus->cont_subj->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_contactus_update->RightColumnClass ?>"><div<?php echo $app_contactus->cont_subj->CellAttributes() ?>>
...
<?php if ($app_contactus->cont_msg->Visible) { // cont_msg ?>
	<div id="r_cont_msg" class="form-group">
		<label for="x_cont_msg" class="<?php echo $app_contactus_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_cont_msg" id="u_cont_msg" class="ewMultiSelect" value="1"<?php echo ($app_contactus->cont_msg->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_contactus->cont_msg->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_contactus_update->RightColumnClass ?>"><div<?php echo $app_contactus->cont_msg->CellAttributes() ?>>
...
<?php if ($app_contactus->cont_datetime->Visible) { // cont_datetime ?>
	<div id="r_cont_datetime" class="form-group">
		<label for="x_cont_datetime" class="<?php echo $app_contactus_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_cont_datetime" id="u_cont_datetime" class="ewMultiSelect" value="1"<?php echo ($app_contactus->cont_datetime->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_contactus->cont_datetime->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_contactus_update->RightColumnClass ?>"><div<?php echo $app_contactus->cont_datetime->CellAttributes() ?>>
...
<?php if (!$app_contactus_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $app_contactus_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $app_contactus_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
	</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fapp_contactusupdate.Init();
</script>
<?php
$app_contactus_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$app_contactus_update->Page_Terminate();
?>
