...
//
// Table class for app_book_page
//
class capp_book_page extends cTable {
	var $bpage_id;
	var $book_id;
	var $page_num;
	var $page_image;
	...
	function __construct() {
		...
		$this->TableVar = 'app_book_page';
		$this->TableName = 'app_book_page';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`app_book_page`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// bpage_id
		$this->bpage_id = new cField('app_book_page', 'app_book_page', 'x_bpage_id', 'bpage_id', '`bpage_id`', '`bpage_id`', 3, -1, FALSE, '`bpage_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// book_id
		$this->book_id = new cField('app_book_page', 'app_book_page', 'x_book_id', 'book_id', '`book_id`', '`book_id`', 3, -1, FALSE, '`book_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// page_num
		$this->page_num = new cField('app_book_page', 'app_book_page', 'x_page_num', 'page_num', '`page_num`', '`page_num`', 2, -1, FALSE, '`page_num`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// page_image
		$this->page_image = new cField('app_book_page', 'app_book_page', 'x_page_image', 'page_image', '`page_image`', '`page_image`', 200, -1, TRUE, '`page_image`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->page_image->Sortable = TRUE; // Allow sort
		$this->fields['page_image'] = &$this->page_image;

		// page_text
		$this->page_text = new cField('app_book_page', 'app_book_page', 'x_page_text', 'page_text', '`page_text`', '`page_text`', 201, -1, FALSE, '`page_text`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "app_book") {
			if ($this->book_id->getSessionValue() <> "")
				$sMasterFilter .= "`book_id`=" . ew_QuotedValue($this->book_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "app_book") {
			if ($this->book_id->getSessionValue() <> "")
				$sDetailFilter .= "`book_id`=" . ew_QuotedValue($this->book_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_app_book() {
		return "`book_id`=@book_id@";
	}

	// Detail filter
	function SqlDetailFilter_app_book() {
		return "`book_id`=@book_id@";
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`app_book_page`";
	}
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "`book_id` ASC,`page_num` ASC";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "app_book_pageedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "app_book_pageadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("app_book_pageview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("app_book_pageview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "app_book_pageadd.php?" . $this->UrlParm($parm);
		else
			$url = "app_book_pageadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("app_book_pageedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("app_book_pageadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("app_book_pagedelete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "app_book" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
			$url .= (strpos($url, "?") !== FALSE ? "&" : "?") . EW_TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_book_id=" . urlencode($this->book_id->CurrentValue);
		}
		return $url;
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["bpage_id"]))
				$arKeys[] = $_POST["bpage_id"];
			elseif (isset($_GET["bpage_id"]))
				$arKeys[] = $_GET["bpage_id"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->bpage_id->CurrentValue = $key;
	...
	function LoadListRowValues(&$rs) {
		...
		$this->page_image->Upload->DbValue = $rs->fields('page_image');
