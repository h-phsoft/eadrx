...
//
// Table class for phs_status
//
class cphs_status extends cTable {
	var $status_id;
	var $status_name;
	...
	function __construct() {
		...
		$this->TableVar = 'phs_status';
		$this->TableName = 'phs_status';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`phs_status`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// status_id
		$this->status_id = new cField('phs_status', 'phs_status', 'x_status_id', 'status_id', '`status_id`', '`status_id`', 16, -1, FALSE, '`status_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// status_name
		$this->status_name = new cField('phs_status', 'phs_status', 'x_status_name', 'status_name', '`status_name`', '`status_name`', 200, -1, FALSE, '`status_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`phs_status`";
	}
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "`status_id` ASC";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "phs_statusedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "phs_statusadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("phs_statusview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("phs_statusview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "phs_statusadd.php?" . $this->UrlParm($parm);
		else
			$url = "phs_statusadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("phs_statusedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("phs_statusadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("phs_statusdelete.php", $this->UrlParm());
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["status_id"]))
				$arKeys[] = $_POST["status_id"];
			elseif (isset($_GET["status_id"]))
				$arKeys[] = $_GET["status_id"];
			else
