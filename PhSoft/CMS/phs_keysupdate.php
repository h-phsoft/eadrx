...
$phs_keys_update = NULL; // Initialize page object first

class cphs_keys_update extends cphs_keys {
	...
	// Page object name
	var $PageObjName = 'phs_keys_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("phs_keyslist.php"); // No records selected, return to list
	...
	function LoadMultiUpdateValues() {
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					$this->key_name->setDbValue($this->Recordset->fields('key_name'));
					$this->key_defvalue->setDbValue($this->Recordset->fields('key_defvalue'));
				} else {
					if (!ew_CompareValue($this->key_name->DbValue, $this->Recordset->fields('key_name')))
						$this->key_name->CurrentValue = NULL;
					if (!ew_CompareValue($this->key_defvalue->DbValue, $this->Recordset->fields('key_defvalue')))
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->key_id->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->key_name->MultiUpdate = $objForm->GetValue("u_key_name");
		if (!$this->key_defvalue->FldIsDetailKey) {
			$this->key_defvalue->setFormValue($objForm->GetValue("x_key_defvalue"));
		}
		$this->key_defvalue->MultiUpdate = $objForm->GetValue("u_key_defvalue");
	...
	function ValidateForm() {
		...
		$gsFormError = "";
		$lUpdateCnt = 0;
		if ($this->key_name->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->key_defvalue->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if ($this->key_name->MultiUpdate <> "" && !$this->key_name->FldIsDetailKey && !is_null($this->key_name->FormValue) && $this->key_name->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->key_name->FldCaption(), $this->key_name->ReqErrMsg));
		}
		if ($this->key_defvalue->MultiUpdate <> "" && !$this->key_defvalue->FldIsDetailKey && !is_null($this->key_defvalue->FormValue) && $this->key_defvalue->FormValue == "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// key_name
			$this->key_name->SetDbValueDef($rsnew, $this->key_name->CurrentValue, "", $this->key_name->ReadOnly || $this->key_name->MultiUpdate <> "1");

			// key_defvalue
			$this->key_defvalue->SetDbValueDef($rsnew, $this->key_defvalue->CurrentValue, "", $this->key_defvalue->ReadOnly || $this->key_defvalue->MultiUpdate <> "1");
...
// Create page object
if (!isset($phs_keys_update)) $phs_keys_update = new cphs_keys_update();

// Page init
$phs_keys_update->Page_Init();

// Page main
$phs_keys_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$phs_keys_update->Page_Render();
?>
...
var CurrentForm = fphs_keysupdate = new ew_Form("fphs_keysupdate", "update");

// Validate form
fphs_keysupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_key_name");
			...
			uelm = this.GetElements("u" + infix + "_key_defvalue");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
...
fphs_keysupdate.Form_CustomValidate = 
...
fphs_keysupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $phs_keys_update->ShowPageHeader(); ?>
<?php
$phs_keys_update->ShowMessage();
?>
<form name="fphs_keysupdate" id="fphs_keysupdate" class="<?php echo $phs_keys_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($phs_keys_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $phs_keys_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="phs_keys">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($phs_keys_update->IsModal) ?>">
<?php foreach ($phs_keys_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_phs_keysupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($phs_keys->key_name->Visible) { // key_name ?>
	<div id="r_key_name" class="form-group">
		<label for="x_key_name" class="<?php echo $phs_keys_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_key_name" id="u_key_name" class="ewMultiSelect" value="1"<?php echo ($phs_keys->key_name->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_keys->key_name->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_keys_update->RightColumnClass ?>"><div<?php echo $phs_keys->key_name->CellAttributes() ?>>
...
<?php if ($phs_keys->key_defvalue->Visible) { // key_defvalue ?>
	<div id="r_key_defvalue" class="form-group">
		<label for="x_key_defvalue" class="<?php echo $phs_keys_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_key_defvalue" id="u_key_defvalue" class="ewMultiSelect" value="1"<?php echo ($phs_keys->key_defvalue->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_keys->key_defvalue->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_keys_update->RightColumnClass ?>"><div<?php echo $phs_keys->key_defvalue->CellAttributes() ?>>
...
<?php if (!$phs_keys_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $phs_keys_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $phs_keys_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
	</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fphs_keysupdate.Init();
</script>
<?php
$phs_keys_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$phs_keys_update->Page_Terminate();
?>
