...
$app_test_name_delete = NULL; // Initialize page object first

class capp_test_name_delete extends capp_test_name {
	...
	// Page object name
	var $PageObjName = 'app_test_name_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("app_test_namelist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in app_test_name class, app_test_nameinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("app_test_namelist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($app_test_name_delete)) $app_test_name_delete = new capp_test_name_delete();

// Page init
$app_test_name_delete->Page_Init();

// Page main
$app_test_name_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_test_name_delete->Page_Render();
?>
...
var CurrentForm = fapp_test_namedelete = new ew_Form("fapp_test_namedelete", "delete");

// Form_CustomValidate event
fapp_test_namedelete.Form_CustomValidate = 
...
fapp_test_namedelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_test_namedelete.Lists["x_test_id"] = {"LinkField":"x_test_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_test_iname","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_test"};
fapp_test_namedelete.Lists["x_test_id"].Data = "<?php echo $app_test_name_delete->test_id->LookupFilterQuery(FALSE, "delete") ?>";
fapp_test_namedelete.Lists["x_lang_id"] = {"LinkField":"x_lang_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_lang_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_language"};
fapp_test_namedelete.Lists["x_lang_id"].Data = "<?php echo $app_test_name_delete->lang_id->LookupFilterQuery(FALSE, "delete") ?>";
...
<?php $app_test_name_delete->ShowPageHeader(); ?>
<?php
$app_test_name_delete->ShowMessage();
?>
<form name="fapp_test_namedelete" id="fapp_test_namedelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_test_name_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_test_name_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_test_name">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($app_test_name_delete->RecKeys as $key) { ?>
...
<?php if ($app_test_name->test_id->Visible) { // test_id ?>
		<th class="<?php echo $app_test_name->test_id->HeaderCellClass() ?>"><span id="elh_app_test_name_test_id" class="app_test_name_test_id"><?php echo $app_test_name->test_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($app_test_name->lang_id->Visible) { // lang_id ?>
		<th class="<?php echo $app_test_name->lang_id->HeaderCellClass() ?>"><span id="elh_app_test_name_lang_id" class="app_test_name_lang_id"><?php echo $app_test_name->lang_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($app_test_name->test_name->Visible) { // test_name ?>
		<th class="<?php echo $app_test_name->test_name->HeaderCellClass() ?>"><span id="elh_app_test_name_test_name" class="app_test_name_test_name"><?php echo $app_test_name->test_name->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$app_test_name_delete->RecCnt = 0;
$i = 0;
while (!$app_test_name_delete->Recordset->EOF) {
	$app_test_name_delete->RecCnt++;
	$app_test_name_delete->RowCnt++;

	// Set row properties
	$app_test_name->ResetAttrs();
	$app_test_name->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$app_test_name_delete->LoadRowValues($app_test_name_delete->Recordset);

	// Render row
	$app_test_name_delete->RenderRow();
?>
	<tr<?php echo $app_test_name->RowAttributes() ?>>
<?php if ($app_test_name->test_id->Visible) { // test_id ?>
		<td<?php echo $app_test_name->test_id->CellAttributes() ?>>
<span id="el<?php echo $app_test_name_delete->RowCnt ?>_app_test_name_test_id" class="app_test_name_test_id">
...
<?php } ?>
<?php if ($app_test_name->lang_id->Visible) { // lang_id ?>
		<td<?php echo $app_test_name->lang_id->CellAttributes() ?>>
<span id="el<?php echo $app_test_name_delete->RowCnt ?>_app_test_name_lang_id" class="app_test_name_lang_id">
...
<?php } ?>
<?php if ($app_test_name->test_name->Visible) { // test_name ?>
		<td<?php echo $app_test_name->test_name->CellAttributes() ?>>
<span id="el<?php echo $app_test_name_delete->RowCnt ?>_app_test_name_test_name" class="app_test_name_test_name">
...
<?php
	$app_test_name_delete->Recordset->MoveNext();
}
$app_test_name_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $app_test_name_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fapp_test_namedelete.Init();
</script>
<?php
$app_test_name_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$app_test_name_delete->Page_Terminate();
?>
