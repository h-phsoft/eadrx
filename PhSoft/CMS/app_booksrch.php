...
$app_book_search = NULL; // Initialize page object first

class capp_book_search extends capp_book {
	...
	// Page object name
	var $PageObjName = 'app_book_search';
			...
			switch ($this->CurrentAction) {
				case "S": // Get search criteria
					...
					if ($sSrchStr <> "") {
						...
						$sSrchStr = "app_booklist.php" . "?" . $sSrchStr;
	...
	function BuildAdvancedSearch() {
		...
		$this->BuildSearchUrl($sSrchUrl, $this->book_title); // book_title
		$this->BuildSearchUrl($sSrchUrl, $this->book_price); // book_price
		$this->BuildSearchUrl($sSrchUrl, $this->book_image); // book_image
		$this->BuildSearchUrl($sSrchUrl, $this->book_desc); // book_desc
	...
	function LoadSearchValues() {
		...
		// book_title
		$this->book_title->AdvancedSearch->SearchValue = $objForm->GetValue("x_book_title");
		$this->book_title->AdvancedSearch->SearchOperator = $objForm->GetValue("z_book_title");

		// book_price
		$this->book_price->AdvancedSearch->SearchValue = $objForm->GetValue("x_book_price");
		$this->book_price->AdvancedSearch->SearchOperator = $objForm->GetValue("z_book_price");

		// book_image
		$this->book_image->AdvancedSearch->SearchValue = $objForm->GetValue("x_book_image");
		$this->book_image->AdvancedSearch->SearchOperator = $objForm->GetValue("z_book_image");

		// book_desc
		$this->book_desc->AdvancedSearch->SearchValue = $objForm->GetValue("x_book_desc");
		$this->book_desc->AdvancedSearch->SearchOperator = $objForm->GetValue("z_book_desc");
	...
	function RenderRow() {
		...
		$this->ins_datetime->ViewCustomAttributes = "";
			...
			$this->book_title->EditValue = ew_HtmlEncode($this->book_title->AdvancedSearch->SearchValue);
			...
			$this->book_price->EditValue = ew_HtmlEncode($this->book_price->AdvancedSearch->SearchValue);
			...
			$this->book_image->EditValue = ew_HtmlEncode($this->book_image->AdvancedSearch->SearchValue);
			...
			$this->book_desc->EditValue = ew_HtmlEncode($this->book_desc->AdvancedSearch->SearchValue);
	...
	function ValidateSearch() {
		...
		if (!EW_SERVER_VALIDATE)
			return TRUE;
		if (!ew_CheckNumber($this->book_price->AdvancedSearch->SearchValue)) {
			ew_AddMessage($gsSearchError, $this->book_price->FldErrMsg());
		}
...
// Create page object
if (!isset($app_book_search)) $app_book_search = new capp_book_search();

// Page init
$app_book_search->Page_Init();

// Page main
$app_book_search->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_book_search->Page_Render();
?>
...
<?php if ($app_book_search->IsModal) { ?>
var CurrentAdvancedSearchForm = fapp_booksearch = new ew_Form("fapp_booksearch", "search");
<?php } else { ?>
var CurrentForm = fapp_booksearch = new ew_Form("fapp_booksearch", "search");
<?php } ?>

// Form_CustomValidate event
fapp_booksearch.Form_CustomValidate = 
...
fapp_booksearch.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_booksearch.Lists["x_lang_id"] = {"LinkField":"x_lang_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_lang_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_language"};
fapp_booksearch.Lists["x_lang_id"].Data = "<?php echo $app_book_search->lang_id->LookupFilterQuery(FALSE, "search") ?>";
fapp_booksearch.Lists["x_status_id"] = {"LinkField":"x_status_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_status_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_status"};
fapp_booksearch.Lists["x_status_id"].Data = "<?php echo $app_book_search->status_id->LookupFilterQuery(FALSE, "search") ?>";

// Form object for search
// Validate function for search

fapp_booksearch.Validate = function(fobj) {
...
<?php $app_book_search->ShowPageHeader(); ?>
<?php
$app_book_search->ShowMessage();
?>
<form name="fapp_booksearch" id="fapp_booksearch" class="<?php echo $app_book_search->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_book_search->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_book_search->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_book">
<input type="hidden" name="a_search" id="a_search" value="S">
<input type="hidden" name="modal" value="<?php echo intval($app_book_search->IsModal) ?>">
<div class="ewSearchDiv"><!-- page* -->
<?php if ($app_book->lang_id->Visible) { // lang_id ?>
	<div id="r_lang_id" class="form-group">
		<label for="x_lang_id" class="<?php echo $app_book_search->LeftColumnClass ?>"><span id="elh_app_book_lang_id"><?php echo $app_book->lang_id->FldCaption() ?></span>
		...
		<div class="<?php echo $app_book_search->RightColumnClass ?>"><div<?php echo $app_book->lang_id->CellAttributes() ?>>
...
<?php if ($app_book->status_id->Visible) { // status_id ?>
	<div id="r_status_id" class="form-group">
		<label for="x_status_id" class="<?php echo $app_book_search->LeftColumnClass ?>"><span id="elh_app_book_status_id"><?php echo $app_book->status_id->FldCaption() ?></span>
		...
		<div class="<?php echo $app_book_search->RightColumnClass ?>"><div<?php echo $app_book->status_id->CellAttributes() ?>>
...
<?php if ($app_book->book_title->Visible) { // book_title ?>
	<div id="r_book_title" class="form-group">
		<label for="x_book_title" class="<?php echo $app_book_search->LeftColumnClass ?>"><span id="elh_app_book_book_title"><?php echo $app_book->book_title->FldCaption() ?></span>
		<p class="form-control-static ewSearchOperator"><?php echo $Language->Phrase("LIKE") ?><input type="hidden" name="z_book_title" id="z_book_title" value="LIKE"></p>
		</label>
		<div class="<?php echo $app_book_search->RightColumnClass ?>"><div<?php echo $app_book->book_title->CellAttributes() ?>>
...
<?php if ($app_book->book_price->Visible) { // book_price ?>
	<div id="r_book_price" class="form-group">
		<label for="x_book_price" class="<?php echo $app_book_search->LeftColumnClass ?>"><span id="elh_app_book_book_price"><?php echo $app_book->book_price->FldCaption() ?></span>
		<p class="form-control-static ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_book_price" id="z_book_price" value="="></p>
		</label>
		<div class="<?php echo $app_book_search->RightColumnClass ?>"><div<?php echo $app_book->book_price->CellAttributes() ?>>
...
<?php if ($app_book->book_image->Visible) { // book_image ?>
	<div id="r_book_image" class="form-group">
		<label class="<?php echo $app_book_search->LeftColumnClass ?>"><span id="elh_app_book_book_image"><?php echo $app_book->book_image->FldCaption() ?></span>
		<p class="form-control-static ewSearchOperator"><?php echo $Language->Phrase("LIKE") ?><input type="hidden" name="z_book_image" id="z_book_image" value="LIKE"></p>
		</label>
		<div class="<?php echo $app_book_search->RightColumnClass ?>"><div<?php echo $app_book->book_image->CellAttributes() ?>>
			<span id="el_app_book_book_image">
<input type="text" data-table="app_book" data-field="x_book_image" name="x_book_image" id="x_book_image" size="30" maxlength="150" placeholder="<?php echo ew_HtmlEncode($app_book->book_image->getPlaceHolder()) ?>" value="<?php echo $app_book->book_image->EditValue ?>"<?php echo $app_book->book_image->EditAttributes() ?>>
</span>
		</div></div>
	</div>
<?php } ?>
<?php if ($app_book->book_desc->Visible) { // book_desc ?>
	<div id="r_book_desc" class="form-group">
		<label class="<?php echo $app_book_search->LeftColumnClass ?>"><span id="elh_app_book_book_desc"><?php echo $app_book->book_desc->FldCaption() ?></span>
		<p class="form-control-static ewSearchOperator"><?php echo $Language->Phrase("LIKE") ?><input type="hidden" name="z_book_desc" id="z_book_desc" value="LIKE"></p>
		</label>
		<div class="<?php echo $app_book_search->RightColumnClass ?>"><div<?php echo $app_book->book_desc->CellAttributes() ?>>
			<span id="el_app_book_book_desc">
<input type="text" data-table="app_book" data-field="x_book_desc" name="x_book_desc" id="x_book_desc" size="35" placeholder="<?php echo ew_HtmlEncode($app_book->book_desc->getPlaceHolder()) ?>" value="<?php echo $app_book->book_desc->EditValue ?>"<?php echo $app_book->book_desc->EditAttributes() ?>>
</span>
		</div></div>
	</div>
<?php } ?>
<?php if ($app_book->ins_datetime->Visible) { // ins_datetime ?>
	<div id="r_ins_datetime" class="form-group">
		<label for="x_ins_datetime" class="<?php echo $app_book_search->LeftColumnClass ?>"><span id="elh_app_book_ins_datetime"><?php echo $app_book->ins_datetime->FldCaption() ?></span>
		...
		<div class="<?php echo $app_book_search->RightColumnClass ?>"><div<?php echo $app_book->ins_datetime->CellAttributes() ?>>
...
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $app_book_search->OffsetColumnClass ?>"><!-- buttons offset -->
...
<script type="text/javascript">
fapp_booksearch.Init();
</script>
<?php
$app_book_search->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$app_book_search->Page_Terminate();
?>
