...
$app_book_delete = NULL; // Initialize page object first

class capp_book_delete extends capp_book {
	...
	// Page object name
	var $PageObjName = 'app_book_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("app_booklist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in app_book class, app_bookinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("app_booklist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($app_book_delete)) $app_book_delete = new capp_book_delete();

// Page init
$app_book_delete->Page_Init();

// Page main
$app_book_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_book_delete->Page_Render();
?>
...
var CurrentForm = fapp_bookdelete = new ew_Form("fapp_bookdelete", "delete");

// Form_CustomValidate event
fapp_bookdelete.Form_CustomValidate = 
...
fapp_bookdelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_bookdelete.Lists["x_lang_id"] = {"LinkField":"x_lang_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_lang_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_language"};
fapp_bookdelete.Lists["x_lang_id"].Data = "<?php echo $app_book_delete->lang_id->LookupFilterQuery(FALSE, "delete") ?>";
fapp_bookdelete.Lists["x_status_id"] = {"LinkField":"x_status_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_status_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_status"};
fapp_bookdelete.Lists["x_status_id"].Data = "<?php echo $app_book_delete->status_id->LookupFilterQuery(FALSE, "delete") ?>";
...
<?php $app_book_delete->ShowPageHeader(); ?>
<?php
$app_book_delete->ShowMessage();
?>
<form name="fapp_bookdelete" id="fapp_bookdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_book_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_book_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_book">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($app_book_delete->RecKeys as $key) { ?>
...
<?php if ($app_book->lang_id->Visible) { // lang_id ?>
		<th class="<?php echo $app_book->lang_id->HeaderCellClass() ?>"><span id="elh_app_book_lang_id" class="app_book_lang_id"><?php echo $app_book->lang_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($app_book->status_id->Visible) { // status_id ?>
		<th class="<?php echo $app_book->status_id->HeaderCellClass() ?>"><span id="elh_app_book_status_id" class="app_book_status_id"><?php echo $app_book->status_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($app_book->book_title->Visible) { // book_title ?>
		<th class="<?php echo $app_book->book_title->HeaderCellClass() ?>"><span id="elh_app_book_book_title" class="app_book_book_title"><?php echo $app_book->book_title->FldCaption() ?></span></th>
<?php } ?>
<?php if ($app_book->book_price->Visible) { // book_price ?>
		<th class="<?php echo $app_book->book_price->HeaderCellClass() ?>"><span id="elh_app_book_book_price" class="app_book_book_price"><?php echo $app_book->book_price->FldCaption() ?></span></th>
<?php } ?>
<?php if ($app_book->book_image->Visible) { // book_image ?>
		<th class="<?php echo $app_book->book_image->HeaderCellClass() ?>"><span id="elh_app_book_book_image" class="app_book_book_image"><?php echo $app_book->book_image->FldCaption() ?></span></th>
<?php } ?>
<?php if ($app_book->ins_datetime->Visible) { // ins_datetime ?>
		<th class="<?php echo $app_book->ins_datetime->HeaderCellClass() ?>"><span id="elh_app_book_ins_datetime" class="app_book_ins_datetime"><?php echo $app_book->ins_datetime->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$app_book_delete->RecCnt = 0;
$i = 0;
while (!$app_book_delete->Recordset->EOF) {
	$app_book_delete->RecCnt++;
	$app_book_delete->RowCnt++;

	// Set row properties
	$app_book->ResetAttrs();
	$app_book->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$app_book_delete->LoadRowValues($app_book_delete->Recordset);

	// Render row
	$app_book_delete->RenderRow();
?>
	<tr<?php echo $app_book->RowAttributes() ?>>
<?php if ($app_book->lang_id->Visible) { // lang_id ?>
		<td<?php echo $app_book->lang_id->CellAttributes() ?>>
<span id="el<?php echo $app_book_delete->RowCnt ?>_app_book_lang_id" class="app_book_lang_id">
...
<span id="el<?php echo $app_book_delete->RowCnt ?>_app_book_status_id" class="app_book_status_id">
...
<span id="el<?php echo $app_book_delete->RowCnt ?>_app_book_book_title" class="app_book_book_title">
...
<span id="el<?php echo $app_book_delete->RowCnt ?>_app_book_book_price" class="app_book_book_price">
...
<span id="el<?php echo $app_book_delete->RowCnt ?>_app_book_book_image" class="app_book_book_image">
<span>
...
<span id="el<?php echo $app_book_delete->RowCnt ?>_app_book_ins_datetime" class="app_book_ins_datetime">
...
<?php
	$app_book_delete->Recordset->MoveNext();
}
$app_book_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $app_book_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fapp_bookdelete.Init();
</script>
<?php
$app_book_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$app_book_delete->Page_Terminate();
?>
