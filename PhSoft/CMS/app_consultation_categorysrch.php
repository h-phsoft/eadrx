...
$app_consultation_category_search = NULL; // Initialize page object first

class capp_consultation_category_search extends capp_consultation_category {
	...
	// Page object name
	var $PageObjName = 'app_consultation_category_search';
			...
			switch ($this->CurrentAction) {
				case "S": // Get search criteria
					...
					if ($sSrchStr <> "") {
						...
						$sSrchStr = "app_consultation_categorylist.php" . "?" . $sSrchStr;
	...
	function BuildAdvancedSearch() {
		...
		$this->BuildSearchUrl($sSrchUrl, $this->cat_order); // cat_order
		$this->BuildSearchUrl($sSrchUrl, $this->cat_name); // cat_name
		$this->BuildSearchUrl($sSrchUrl, $this->cat_Price); // cat_Price
	...
	function LoadSearchValues() {
		...
		// cat_order
		$this->cat_order->AdvancedSearch->SearchValue = $objForm->GetValue("x_cat_order");
		$this->cat_order->AdvancedSearch->SearchOperator = $objForm->GetValue("z_cat_order");
		...
		// cat_Price
		$this->cat_Price->AdvancedSearch->SearchValue = $objForm->GetValue("x_cat_Price");
		$this->cat_Price->AdvancedSearch->SearchOperator = $objForm->GetValue("z_cat_Price");
	}
	...
	function RenderRow() {
		...
		$this->cat_Price->ViewCustomAttributes = "";
			...
			$this->cat_order->EditValue = ew_HtmlEncode($this->cat_order->AdvancedSearch->SearchValue);
			...
			$this->cat_Price->EditValue = ew_HtmlEncode($this->cat_Price->AdvancedSearch->SearchValue);
	...
	function ValidateSearch() {
		...
		if (!EW_SERVER_VALIDATE)
			return TRUE;
		if (!ew_CheckInteger($this->cat_id->AdvancedSearch->SearchValue)) {
			ew_AddMessage($gsSearchError, $this->cat_id->FldErrMsg());
		}
		if (!ew_CheckInteger($this->cat_order->AdvancedSearch->SearchValue)) {
			ew_AddMessage($gsSearchError, $this->cat_order->FldErrMsg());
		}
		if (!ew_CheckNumber($this->cat_Price->AdvancedSearch->SearchValue)) {
			ew_AddMessage($gsSearchError, $this->cat_Price->FldErrMsg());
		}
...
// Create page object
if (!isset($app_consultation_category_search)) $app_consultation_category_search = new capp_consultation_category_search();

// Page init
$app_consultation_category_search->Page_Init();

// Page main
$app_consultation_category_search->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_consultation_category_search->Page_Render();
?>
...
<?php if ($app_consultation_category_search->IsModal) { ?>
var CurrentAdvancedSearchForm = fapp_consultation_categorysearch = new ew_Form("fapp_consultation_categorysearch", "search");
<?php } else { ?>
var CurrentForm = fapp_consultation_categorysearch = new ew_Form("fapp_consultation_categorysearch", "search");
<?php } ?>

// Form_CustomValidate event
fapp_consultation_categorysearch.Form_CustomValidate = 
...
fapp_consultation_categorysearch.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_consultation_categorysearch.Lists["x_status_id"] = {"LinkField":"x_status_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_status_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_status"};
fapp_consultation_categorysearch.Lists["x_status_id"].Data = "<?php echo $app_consultation_category_search->status_id->LookupFilterQuery(FALSE, "search") ?>";

// Form object for search
// Validate function for search

fapp_consultation_categorysearch.Validate = function(fobj) {
	...
	if (elm && !ew_CheckInteger(elm.value))
		return this.OnError(elm, "<?php echo ew_JsEncode2($app_consultation_category->cat_id->FldErrMsg()) ?>");
...
<?php $app_consultation_category_search->ShowPageHeader(); ?>
<?php
$app_consultation_category_search->ShowMessage();
?>
<form name="fapp_consultation_categorysearch" id="fapp_consultation_categorysearch" class="<?php echo $app_consultation_category_search->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_consultation_category_search->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_consultation_category_search->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_consultation_category">
<input type="hidden" name="a_search" id="a_search" value="S">
<input type="hidden" name="modal" value="<?php echo intval($app_consultation_category_search->IsModal) ?>">
<div class="ewSearchDiv"><!-- page* -->
<?php if ($app_consultation_category->cat_id->Visible) { // cat_id ?>
	<div id="r_cat_id" class="form-group">
		<label for="x_cat_id" class="<?php echo $app_consultation_category_search->LeftColumnClass ?>"><span id="elh_app_consultation_category_cat_id"><?php echo $app_consultation_category->cat_id->FldCaption() ?></span>
		...
		<div class="<?php echo $app_consultation_category_search->RightColumnClass ?>"><div<?php echo $app_consultation_category->cat_id->CellAttributes() ?>>
			<span id="el_app_consultation_category_cat_id">
<input type="text" data-table="app_consultation_category" data-field="x_cat_id" name="x_cat_id" id="x_cat_id" placeholder="<?php echo ew_HtmlEncode($app_consultation_category->cat_id->getPlaceHolder()) ?>" value="<?php echo $app_consultation_category->cat_id->EditValue ?>"<?php echo $app_consultation_category->cat_id->EditAttributes() ?>>
</span>
		</div></div>
	</div>
<?php } ?>
<?php if ($app_consultation_category->status_id->Visible) { // status_id ?>
	<div id="r_status_id" class="form-group">
		<label for="x_status_id" class="<?php echo $app_consultation_category_search->LeftColumnClass ?>"><span id="elh_app_consultation_category_status_id"><?php echo $app_consultation_category->status_id->FldCaption() ?></span>
		...
		<div class="<?php echo $app_consultation_category_search->RightColumnClass ?>"><div<?php echo $app_consultation_category->status_id->CellAttributes() ?>>
...
<?php if ($app_consultation_category->cat_order->Visible) { // cat_order ?>
	<div id="r_cat_order" class="form-group">
		<label for="x_cat_order" class="<?php echo $app_consultation_category_search->LeftColumnClass ?>"><span id="elh_app_consultation_category_cat_order"><?php echo $app_consultation_category->cat_order->FldCaption() ?></span>
		<p class="form-control-static ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_cat_order" id="z_cat_order" value="="></p>
		</label>
		<div class="<?php echo $app_consultation_category_search->RightColumnClass ?>"><div<?php echo $app_consultation_category->cat_order->CellAttributes() ?>>
...
<?php if ($app_consultation_category->cat_name->Visible) { // cat_name ?>
	<div id="r_cat_name" class="form-group">
		<label for="x_cat_name" class="<?php echo $app_consultation_category_search->LeftColumnClass ?>"><span id="elh_app_consultation_category_cat_name"><?php echo $app_consultation_category->cat_name->FldCaption() ?></span>
		...
		<div class="<?php echo $app_consultation_category_search->RightColumnClass ?>"><div<?php echo $app_consultation_category->cat_name->CellAttributes() ?>>
...
<?php if ($app_consultation_category->cat_Price->Visible) { // cat_Price ?>
	<div id="r_cat_Price" class="form-group">
		<label for="x_cat_Price" class="<?php echo $app_consultation_category_search->LeftColumnClass ?>"><span id="elh_app_consultation_category_cat_Price"><?php echo $app_consultation_category->cat_Price->FldCaption() ?></span>
		<p class="form-control-static ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_cat_Price" id="z_cat_Price" value="="></p>
		</label>
		<div class="<?php echo $app_consultation_category_search->RightColumnClass ?>"><div<?php echo $app_consultation_category->cat_Price->CellAttributes() ?>>
...
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $app_consultation_category_search->OffsetColumnClass ?>"><!-- buttons offset -->
...
<script type="text/javascript">
fapp_consultation_categorysearch.Init();
</script>
<?php
$app_consultation_category_search->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$app_consultation_category_search->Page_Terminate();
?>
