...
$phs_bill_cycle_add = NULL; // Initialize page object first

class cphs_bill_cycle_add extends cphs_bill_cycle {
	...
	// Page object name
	var $PageObjName = 'phs_bill_cycle_add';
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$loaded) { // Record not loaded
					...
					$this->Page_Terminate("phs_bill_cyclelist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "phs_bill_cycleview.php")
...
// Create page object
if (!isset($phs_bill_cycle_add)) $phs_bill_cycle_add = new cphs_bill_cycle_add();

// Page init
$phs_bill_cycle_add->Page_Init();

// Page main
$phs_bill_cycle_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$phs_bill_cycle_add->Page_Render();
?>
...
var CurrentForm = fphs_bill_cycleadd = new ew_Form("fphs_bill_cycleadd", "add");

// Validate form
fphs_bill_cycleadd.Validate = function() {
...
fphs_bill_cycleadd.Form_CustomValidate = 
...
fphs_bill_cycleadd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fphs_bill_cycleadd.Lists["x_status_id"] = {"LinkField":"x_status_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_status_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_status"};
fphs_bill_cycleadd.Lists["x_status_id"].Data = "<?php echo $phs_bill_cycle_add->status_id->LookupFilterQuery(FALSE, "add") ?>";
...
<?php $phs_bill_cycle_add->ShowPageHeader(); ?>
<?php
$phs_bill_cycle_add->ShowMessage();
?>
<form name="fphs_bill_cycleadd" id="fphs_bill_cycleadd" class="<?php echo $phs_bill_cycle_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($phs_bill_cycle_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $phs_bill_cycle_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="phs_bill_cycle">
<input type="hidden" name="a_add" id="a_add" value="A">
<input type="hidden" name="modal" value="<?php echo intval($phs_bill_cycle_add->IsModal) ?>">
<div class="ewAddDiv"><!-- page* -->
<?php if ($phs_bill_cycle->cycle_name->Visible) { // cycle_name ?>
	<div id="r_cycle_name" class="form-group">
		<label id="elh_phs_bill_cycle_cycle_name" for="x_cycle_name" class="<?php echo $phs_bill_cycle_add->LeftColumnClass ?>"><?php echo $phs_bill_cycle->cycle_name->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $phs_bill_cycle_add->RightColumnClass ?>"><div<?php echo $phs_bill_cycle->cycle_name->CellAttributes() ?>>
...
<?php if ($phs_bill_cycle->cycle_interval->Visible) { // cycle_interval ?>
	<div id="r_cycle_interval" class="form-group">
		<label id="elh_phs_bill_cycle_cycle_interval" for="x_cycle_interval" class="<?php echo $phs_bill_cycle_add->LeftColumnClass ?>"><?php echo $phs_bill_cycle->cycle_interval->FldCaption() ?></label>
		<div class="<?php echo $phs_bill_cycle_add->RightColumnClass ?>"><div<?php echo $phs_bill_cycle->cycle_interval->CellAttributes() ?>>
...
<?php if ($phs_bill_cycle->status_id->Visible) { // status_id ?>
	<div id="r_status_id" class="form-group">
		<label id="elh_phs_bill_cycle_status_id" for="x_status_id" class="<?php echo $phs_bill_cycle_add->LeftColumnClass ?>"><?php echo $phs_bill_cycle->status_id->FldCaption() ?></label>
		<div class="<?php echo $phs_bill_cycle_add->RightColumnClass ?>"><div<?php echo $phs_bill_cycle->status_id->CellAttributes() ?>>
...
<?php if ($phs_bill_cycle->cycle_desc->Visible) { // cycle_desc ?>
	<div id="r_cycle_desc" class="form-group">
		<label id="elh_phs_bill_cycle_cycle_desc" for="x_cycle_desc" class="<?php echo $phs_bill_cycle_add->LeftColumnClass ?>"><?php echo $phs_bill_cycle->cycle_desc->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $phs_bill_cycle_add->RightColumnClass ?>"><div<?php echo $phs_bill_cycle->cycle_desc->CellAttributes() ?>>
...
<?php if (!$phs_bill_cycle_add->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $phs_bill_cycle_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $phs_bill_cycle_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fphs_bill_cycleadd.Init();
</script>
<?php
$phs_bill_cycle_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$phs_bill_cycle_add->Page_Terminate();
?>
