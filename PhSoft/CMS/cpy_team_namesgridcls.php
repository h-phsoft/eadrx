...
$cpy_team_names_grid = NULL; // Initialize page object first

class ccpy_team_names_grid extends ccpy_team_names {
	...
	// Page object name
	var $PageObjName = 'cpy_team_names_grid';

	// Grid form hidden field names
	var $FormName = 'fcpy_team_namesgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["cpy_team_names"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->teamn_id->CurrentValue = strval($arKeys[0]); // teamn_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "cpy_team") {
			...
			if ($GLOBALS["cpy_team"]->EventCancelled) $this->EventCancelled = TRUE;
		}
