...
$app_notification_for = NULL;

//
// Table class for app_notification_for
//
class capp_notification_for extends cTable {
	var $nfor_id;
	var $notif_id;
	var $user_id;
	var $nstatus_id;
	...
	function __construct() {
		...
		$this->TableVar = 'app_notification_for';
		$this->TableName = 'app_notification_for';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`app_notification_for`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// nfor_id
		$this->nfor_id = new cField('app_notification_for', 'app_notification_for', 'x_nfor_id', 'nfor_id', '`nfor_id`', '`nfor_id`', 3, -1, FALSE, '`nfor_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->nfor_id->Sortable = FALSE; // Allow sort
		$this->nfor_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nfor_id'] = &$this->nfor_id;

		// notif_id
		$this->notif_id = new cField('app_notification_for', 'app_notification_for', 'x_notif_id', 'notif_id', '`notif_id`', '`notif_id`', 3, -1, FALSE, '`notif_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// user_id
		$this->user_id = new cField('app_notification_for', 'app_notification_for', 'x_user_id', 'user_id', '`user_id`', '`user_id`', 3, -1, FALSE, '`user_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// nstatus_id
		$this->nstatus_id = new cField('app_notification_for', 'app_notification_for', 'x_nstatus_id', 'nstatus_id', '`nstatus_id`', '`nstatus_id`', 16, -1, FALSE, '`nstatus_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`app_notification_for`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->nfor_id->setDbValue($conn->Insert_ID());
			$rs['nfor_id'] = $this->nfor_id->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('nfor_id', $rs))
				ew_AddFilter($where, ew_QuotedName('nfor_id', $this->DBID) . '=' . ew_QuotedValue($rs['nfor_id'], $this->nfor_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`nfor_id` = @nfor_id@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->nfor_id->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->nfor_id->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@nfor_id@", ew_AdjustSql($this->nfor_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "app_notification_forlist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "app_notification_foredit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "app_notification_foradd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("app_notification_forview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("app_notification_forview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "app_notification_foradd.php?" . $this->UrlParm($parm);
		else
			$url = "app_notification_foradd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("app_notification_foredit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("app_notification_foradd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("app_notification_fordelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "nfor_id:" . ew_VarToJson($this->nfor_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->nfor_id->CurrentValue)) {
			$sUrl .= "nfor_id=" . urlencode($this->nfor_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["nfor_id"]))
				$arKeys[] = $_POST["nfor_id"];
			elseif (isset($_GET["nfor_id"]))
				$arKeys[] = $_GET["nfor_id"];
			else
	...
	function LoadListRowValues(&$rs) {
		$this->nfor_id->setDbValue($rs->fields('nfor_id'));
	...
	// Common render codes
		...
		$this->nfor_id->ViewValue = $this->nfor_id->CurrentValue;
		$this->nfor_id->ViewCustomAttributes = "";
		...
		// nfor_id
		$this->nfor_id->LinkCustomAttributes = "";
		$this->nfor_id->HrefValue = "";
		$this->nfor_id->TooltipValue = "";
	...
	function RenderEditRow() {
		...
		// nfor_id
		$this->nfor_id->EditAttrs["class"] = "form-control";
		$this->nfor_id->EditCustomAttributes = "";
		$this->nfor_id->EditValue = $this->nfor_id->CurrentValue;
		$this->nfor_id->ViewCustomAttributes = "";
