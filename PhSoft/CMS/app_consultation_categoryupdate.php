...
$app_consultation_category_update = NULL; // Initialize page object first

class capp_consultation_category_update extends capp_consultation_category {
	...
	// Page object name
	var $PageObjName = 'app_consultation_category_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("app_consultation_categorylist.php"); // No records selected, return to list
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					...
					$this->cat_order->setDbValue($this->Recordset->fields('cat_order'));
					$this->cat_name->setDbValue($this->Recordset->fields('cat_name'));
					$this->cat_Price->setDbValue($this->Recordset->fields('cat_Price'));
				} else {
					if (!ew_CompareValue($this->status_id->DbValue, $this->Recordset->fields('status_id')))
						$this->status_id->CurrentValue = NULL;
					if (!ew_CompareValue($this->cat_order->DbValue, $this->Recordset->fields('cat_order')))
					...
					if (!ew_CompareValue($this->cat_Price->DbValue, $this->Recordset->fields('cat_Price')))
						$this->cat_Price->CurrentValue = NULL;
				}
				$i++;
	...
	function LoadFormValues() {
		...
		$this->cat_order->MultiUpdate = $objForm->GetValue("u_cat_order");
		...
		$this->cat_Price->MultiUpdate = $objForm->GetValue("u_cat_Price");
	...
	function ValidateForm() {
		...
		if ($this->cat_order->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->cat_name->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->cat_Price->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// cat_order
			$this->cat_order->SetDbValueDef($rsnew, $this->cat_order->CurrentValue, 0, $this->cat_order->ReadOnly || $this->cat_order->MultiUpdate <> "1");

			// cat_name
			$this->cat_name->SetDbValueDef($rsnew, $this->cat_name->CurrentValue, "", $this->cat_name->ReadOnly || $this->cat_name->MultiUpdate <> "1");

			// cat_Price
			$this->cat_Price->SetDbValueDef($rsnew, $this->cat_Price->CurrentValue, 0, $this->cat_Price->ReadOnly || $this->cat_Price->MultiUpdate <> "1");
...
// Create page object
if (!isset($app_consultation_category_update)) $app_consultation_category_update = new capp_consultation_category_update();

// Page init
$app_consultation_category_update->Page_Init();

// Page main
$app_consultation_category_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_consultation_category_update->Page_Render();
?>
...
var CurrentForm = fapp_consultation_categoryupdate = new ew_Form("fapp_consultation_categoryupdate", "update");

// Validate form
fapp_consultation_categoryupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_cat_order");
			...
			uelm = this.GetElements("u" + infix + "_cat_Price");
...
fapp_consultation_categoryupdate.Form_CustomValidate = 
...
fapp_consultation_categoryupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_consultation_categoryupdate.Lists["x_status_id"] = {"LinkField":"x_status_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_status_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_status"};
fapp_consultation_categoryupdate.Lists["x_status_id"].Data = "<?php echo $app_consultation_category_update->status_id->LookupFilterQuery(FALSE, "update") ?>";
...
<?php $app_consultation_category_update->ShowPageHeader(); ?>
<?php
$app_consultation_category_update->ShowMessage();
?>
<form name="fapp_consultation_categoryupdate" id="fapp_consultation_categoryupdate" class="<?php echo $app_consultation_category_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_consultation_category_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_consultation_category_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_consultation_category">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($app_consultation_category_update->IsModal) ?>">
<?php foreach ($app_consultation_category_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_app_consultation_categoryupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($app_consultation_category->status_id->Visible) { // status_id ?>
	<div id="r_status_id" class="form-group">
		<label for="x_status_id" class="<?php echo $app_consultation_category_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_status_id" id="u_status_id" class="ewMultiSelect" value="1"<?php echo ($app_consultation_category->status_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_consultation_category->status_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_consultation_category_update->RightColumnClass ?>"><div<?php echo $app_consultation_category->status_id->CellAttributes() ?>>
...
<?php if ($app_consultation_category->cat_order->Visible) { // cat_order ?>
	<div id="r_cat_order" class="form-group">
		<label for="x_cat_order" class="<?php echo $app_consultation_category_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_cat_order" id="u_cat_order" class="ewMultiSelect" value="1"<?php echo ($app_consultation_category->cat_order->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_consultation_category->cat_order->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_consultation_category_update->RightColumnClass ?>"><div<?php echo $app_consultation_category->cat_order->CellAttributes() ?>>
...
<?php if ($app_consultation_category->cat_name->Visible) { // cat_name ?>
	<div id="r_cat_name" class="form-group">
		<label for="x_cat_name" class="<?php echo $app_consultation_category_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_cat_name" id="u_cat_name" class="ewMultiSelect" value="1"<?php echo ($app_consultation_category->cat_name->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_consultation_category->cat_name->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_consultation_category_update->RightColumnClass ?>"><div<?php echo $app_consultation_category->cat_name->CellAttributes() ?>>
...
<?php if ($app_consultation_category->cat_Price->Visible) { // cat_Price ?>
	<div id="r_cat_Price" class="form-group">
		<label for="x_cat_Price" class="<?php echo $app_consultation_category_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_cat_Price" id="u_cat_Price" class="ewMultiSelect" value="1"<?php echo ($app_consultation_category->cat_Price->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_consultation_category->cat_Price->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_consultation_category_update->RightColumnClass ?>"><div<?php echo $app_consultation_category->cat_Price->CellAttributes() ?>>
...
<?php if (!$app_consultation_category_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $app_consultation_category_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $app_consultation_category_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
	</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fapp_consultation_categoryupdate.Init();
</script>
<?php
$app_consultation_category_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$app_consultation_category_update->Page_Terminate();
?>
