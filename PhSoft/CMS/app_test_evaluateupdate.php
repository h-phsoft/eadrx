...
$app_test_evaluate_update = NULL; // Initialize page object first

class capp_test_evaluate_update extends capp_test_evaluate {
	...
	// Page object name
	var $PageObjName = 'app_test_evaluate_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("app_test_evaluatelist.php"); // No records selected, return to list
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					...
					$this->eval_from->setDbValue($this->Recordset->fields('eval_from'));
					$this->eval_to->setDbValue($this->Recordset->fields('eval_to'));
					$this->eval_text->setDbValue($this->Recordset->fields('eval_text'));
				} else {
					...
					if (!ew_CompareValue($this->eval_from->DbValue, $this->Recordset->fields('eval_from')))
						$this->eval_from->CurrentValue = NULL;
					if (!ew_CompareValue($this->eval_to->DbValue, $this->Recordset->fields('eval_to')))
						$this->eval_to->CurrentValue = NULL;
					if (!ew_CompareValue($this->eval_text->DbValue, $this->Recordset->fields('eval_text')))
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->eval_id->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->eval_from->MultiUpdate = $objForm->GetValue("u_eval_from");
		if (!$this->eval_to->FldIsDetailKey) {
			$this->eval_to->setFormValue($objForm->GetValue("x_eval_to"));
		}
		$this->eval_to->MultiUpdate = $objForm->GetValue("u_eval_to");
		if (!$this->eval_text->FldIsDetailKey) {
			$this->eval_text->setFormValue($objForm->GetValue("x_eval_text"));
		}
		$this->eval_text->MultiUpdate = $objForm->GetValue("u_eval_text");
		if (!$this->eval_id->FldIsDetailKey)
	...
	function ValidateForm() {
		...
		if ($this->eval_from->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->eval_to->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->eval_text->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
		...
		if ($this->eval_from->MultiUpdate <> "" && !$this->eval_from->FldIsDetailKey && !is_null($this->eval_from->FormValue) && $this->eval_from->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->eval_from->FldCaption(), $this->eval_from->ReqErrMsg));
		}
		if ($this->eval_from->MultiUpdate <> "") {
			if (!ew_CheckNumber($this->eval_from->FormValue)) {
				ew_AddMessage($gsFormError, $this->eval_from->FldErrMsg());
			}
		}
		if ($this->eval_to->MultiUpdate <> "" && !$this->eval_to->FldIsDetailKey && !is_null($this->eval_to->FormValue) && $this->eval_to->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->eval_to->FldCaption(), $this->eval_to->ReqErrMsg));
		}
		if ($this->eval_to->MultiUpdate <> "") {
			if (!ew_CheckNumber($this->eval_to->FormValue)) {
				ew_AddMessage($gsFormError, $this->eval_to->FldErrMsg());
			}
		}
		if ($this->eval_text->MultiUpdate <> "" && !$this->eval_text->FldIsDetailKey && !is_null($this->eval_text->FormValue) && $this->eval_text->FormValue == "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// eval_from
			$this->eval_from->SetDbValueDef($rsnew, $this->eval_from->CurrentValue, 0, $this->eval_from->ReadOnly || $this->eval_from->MultiUpdate <> "1");

			// eval_to
			$this->eval_to->SetDbValueDef($rsnew, $this->eval_to->CurrentValue, 0, $this->eval_to->ReadOnly || $this->eval_to->MultiUpdate <> "1");

			// eval_text
			$this->eval_text->SetDbValueDef($rsnew, $this->eval_text->CurrentValue, "", $this->eval_text->ReadOnly || $this->eval_text->MultiUpdate <> "1");
...
// Create page object
if (!isset($app_test_evaluate_update)) $app_test_evaluate_update = new capp_test_evaluate_update();

// Page init
$app_test_evaluate_update->Page_Init();

// Page main
$app_test_evaluate_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_test_evaluate_update->Page_Render();
?>
...
var CurrentForm = fapp_test_evaluateupdate = new ew_Form("fapp_test_evaluateupdate", "update");

// Validate form
fapp_test_evaluateupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_eval_from");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
			...
			uelm = this.GetElements("u" + infix + "_eval_to");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
			...
			uelm = this.GetElements("u" + infix + "_eval_text");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
...
fapp_test_evaluateupdate.Form_CustomValidate = 
...
fapp_test_evaluateupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_test_evaluateupdate.Lists["x_test_id"] = {"LinkField":"x_test_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_test_iname","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_test"};
fapp_test_evaluateupdate.Lists["x_test_id"].Data = "<?php echo $app_test_evaluate_update->test_id->LookupFilterQuery(FALSE, "update") ?>";
fapp_test_evaluateupdate.Lists["x_lang_id"] = {"LinkField":"x_lang_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_lang_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_language"};
fapp_test_evaluateupdate.Lists["x_lang_id"].Data = "<?php echo $app_test_evaluate_update->lang_id->LookupFilterQuery(FALSE, "update") ?>";
fapp_test_evaluateupdate.Lists["x_gend_id"] = {"LinkField":"x_gend_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_gend_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_gender"};
fapp_test_evaluateupdate.Lists["x_gend_id"].Data = "<?php echo $app_test_evaluate_update->gend_id->LookupFilterQuery(FALSE, "update") ?>";
...
<?php $app_test_evaluate_update->ShowPageHeader(); ?>
<?php
$app_test_evaluate_update->ShowMessage();
?>
<form name="fapp_test_evaluateupdate" id="fapp_test_evaluateupdate" class="<?php echo $app_test_evaluate_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_test_evaluate_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_test_evaluate_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_test_evaluate">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($app_test_evaluate_update->IsModal) ?>">
<?php foreach ($app_test_evaluate_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_app_test_evaluateupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($app_test_evaluate->test_id->Visible) { // test_id ?>
	<div id="r_test_id" class="form-group">
		<label for="x_test_id" class="<?php echo $app_test_evaluate_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_test_id" id="u_test_id" class="ewMultiSelect" value="1"<?php echo ($app_test_evaluate->test_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test_evaluate->test_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_evaluate_update->RightColumnClass ?>"><div<?php echo $app_test_evaluate->test_id->CellAttributes() ?>>
...
<input type="hidden" id="x_test_id" name="x_test_id" value="<?php echo ew_HtmlEncode($app_test_evaluate->test_id->CurrentValue) ?>">
<?php } else { ?>
<span id="el_app_test_evaluate_test_id">
...
<?php } ?>
<?php echo $app_test_evaluate->test_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($app_test_evaluate->lang_id->Visible) { // lang_id ?>
	<div id="r_lang_id" class="form-group">
		<label for="x_lang_id" class="<?php echo $app_test_evaluate_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_lang_id" id="u_lang_id" class="ewMultiSelect" value="1"<?php echo ($app_test_evaluate->lang_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test_evaluate->lang_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_evaluate_update->RightColumnClass ?>"><div<?php echo $app_test_evaluate->lang_id->CellAttributes() ?>>
...
<?php echo $app_test_evaluate->lang_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($app_test_evaluate->gend_id->Visible) { // gend_id ?>
	<div id="r_gend_id" class="form-group">
		<label for="x_gend_id" class="<?php echo $app_test_evaluate_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_gend_id" id="u_gend_id" class="ewMultiSelect" value="1"<?php echo ($app_test_evaluate->gend_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test_evaluate->gend_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_evaluate_update->RightColumnClass ?>"><div<?php echo $app_test_evaluate->gend_id->CellAttributes() ?>>
...
<?php echo $app_test_evaluate->gend_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($app_test_evaluate->eval_from->Visible) { // eval_from ?>
	<div id="r_eval_from" class="form-group">
		<label for="x_eval_from" class="<?php echo $app_test_evaluate_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_eval_from" id="u_eval_from" class="ewMultiSelect" value="1"<?php echo ($app_test_evaluate->eval_from->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test_evaluate->eval_from->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_evaluate_update->RightColumnClass ?>"><div<?php echo $app_test_evaluate->eval_from->CellAttributes() ?>>
...
<?php echo $app_test_evaluate->eval_from->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($app_test_evaluate->eval_to->Visible) { // eval_to ?>
	<div id="r_eval_to" class="form-group">
		<label for="x_eval_to" class="<?php echo $app_test_evaluate_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_eval_to" id="u_eval_to" class="ewMultiSelect" value="1"<?php echo ($app_test_evaluate->eval_to->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test_evaluate->eval_to->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_evaluate_update->RightColumnClass ?>"><div<?php echo $app_test_evaluate->eval_to->CellAttributes() ?>>
...
<?php echo $app_test_evaluate->eval_to->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($app_test_evaluate->eval_text->Visible) { // eval_text ?>
	<div id="r_eval_text" class="form-group">
		<label class="<?php echo $app_test_evaluate_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_eval_text" id="u_eval_text" class="ewMultiSelect" value="1"<?php echo ($app_test_evaluate->eval_text->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test_evaluate->eval_text->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_evaluate_update->RightColumnClass ?>"><div<?php echo $app_test_evaluate->eval_text->CellAttributes() ?>>
<span id="el_app_test_evaluate_eval_text">
<?php ew_AppendClass($app_test_evaluate->eval_text->EditAttrs["class"], "editor"); ?>
<textarea data-table="app_test_evaluate" data-field="x_eval_text" name="x_eval_text" id="x_eval_text" cols="35" rows="8" placeholder="<?php echo ew_HtmlEncode($app_test_evaluate->eval_text->getPlaceHolder()) ?>"<?php echo $app_test_evaluate->eval_text->EditAttributes() ?>><?php echo $app_test_evaluate->eval_text->EditValue ?></textarea>
<script type="text/javascript">
ew_CreateEditor("fapp_test_evaluateupdate", "x_eval_text", 35, 8, <?php echo ($app_test_evaluate->eval_text->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
<?php echo $app_test_evaluate->eval_text->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page -->
<?php if (!$app_test_evaluate_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $app_test_evaluate_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $app_test_evaluate_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
	</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fapp_test_evaluateupdate.Init();
</script>
<?php
$app_test_evaluate_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$app_test_evaluate_update->Page_Terminate();
?>
