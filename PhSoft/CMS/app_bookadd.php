...
$app_book_add = NULL; // Initialize page object first

class capp_book_add extends capp_book {
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$loaded) { // Record not loaded
					...
					$this->Page_Terminate("app_booklist.php"); // No matching record, return to list
				}
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "app_bookview.php")
	...
	function LoadDefaultValues() {
		...
		$this->book_image->CurrentValue = NULL; // Clear file related field
	...
	function RenderRow() {
		...
		$this->ins_datetime->ViewCustomAttributes = "";
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->book_image);
	...
	function ValidateForm() {
		...
		if (in_array("app_book_page", $DetailTblVar) && $GLOBALS["app_book_page"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		// book_desc
		$this->book_desc->SetDbValueDef($rsnew, $this->book_desc->CurrentValue, "", FALSE);
		if ($this->book_image->Visible && !$this->book_image->Upload->KeepFile) {
			$this->book_image->UploadPath = '../../assets/img/bookImages';
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("app_book_page", $DetailTblVar) && $GLOBALS["app_book_page"]->DetailAdd) {
				$GLOBALS["app_book_page"]->book_id->setSessionValue($this->book_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["app_book_page_grid"])) $GLOBALS["app_book_page_grid"] = new capp_book_page_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "app_book_page"); // Load user level of detail table
				$AddRow = $GLOBALS["app_book_page_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["app_book_page"]->book_id->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("app_book_page", $DetailTblVar)) {
				...
				if ($GLOBALS["app_book_page_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["app_book_page_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["app_book_page_grid"]->CurrentMode = "add";
					$GLOBALS["app_book_page_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($app_book_add)) $app_book_add = new capp_book_add();

// Page init
$app_book_add->Page_Init();

// Page main
$app_book_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_book_add->Page_Render();
?>
...
var CurrentForm = fapp_bookadd = new ew_Form("fapp_bookadd", "add");

// Validate form
fapp_bookadd.Validate = function() {
...
fapp_bookadd.Form_CustomValidate = 
...
fapp_bookadd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_bookadd.Lists["x_lang_id"] = {"LinkField":"x_lang_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_lang_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_language"};
fapp_bookadd.Lists["x_lang_id"].Data = "<?php echo $app_book_add->lang_id->LookupFilterQuery(FALSE, "add") ?>";
fapp_bookadd.Lists["x_status_id"] = {"LinkField":"x_status_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_status_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_status"};
fapp_bookadd.Lists["x_status_id"].Data = "<?php echo $app_book_add->status_id->LookupFilterQuery(FALSE, "add") ?>";
...
<?php $app_book_add->ShowPageHeader(); ?>
<?php
$app_book_add->ShowMessage();
?>
<form name="fapp_bookadd" id="fapp_bookadd" class="<?php echo $app_book_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_book_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_book_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_book">
<input type="hidden" name="a_add" id="a_add" value="A">
<input type="hidden" name="modal" value="<?php echo intval($app_book_add->IsModal) ?>">
<div class="ewAddDiv"><!-- page* -->
<?php if ($app_book->lang_id->Visible) { // lang_id ?>
	<div id="r_lang_id" class="form-group">
		<label id="elh_app_book_lang_id" for="x_lang_id" class="<?php echo $app_book_add->LeftColumnClass ?>"><?php echo $app_book->lang_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $app_book_add->RightColumnClass ?>"><div<?php echo $app_book->lang_id->CellAttributes() ?>>
...
<?php if ($app_book->status_id->Visible) { // status_id ?>
	<div id="r_status_id" class="form-group">
		<label id="elh_app_book_status_id" for="x_status_id" class="<?php echo $app_book_add->LeftColumnClass ?>"><?php echo $app_book->status_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $app_book_add->RightColumnClass ?>"><div<?php echo $app_book->status_id->CellAttributes() ?>>
...
<?php if ($app_book->book_title->Visible) { // book_title ?>
	<div id="r_book_title" class="form-group">
		<label id="elh_app_book_book_title" for="x_book_title" class="<?php echo $app_book_add->LeftColumnClass ?>"><?php echo $app_book->book_title->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $app_book_add->RightColumnClass ?>"><div<?php echo $app_book->book_title->CellAttributes() ?>>
...
<?php if ($app_book->book_price->Visible) { // book_price ?>
	<div id="r_book_price" class="form-group">
		<label id="elh_app_book_book_price" for="x_book_price" class="<?php echo $app_book_add->LeftColumnClass ?>"><?php echo $app_book->book_price->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $app_book_add->RightColumnClass ?>"><div<?php echo $app_book->book_price->CellAttributes() ?>>
...
<?php if ($app_book->book_image->Visible) { // book_image ?>
	<div id="r_book_image" class="form-group">
		<label id="elh_app_book_book_image" class="<?php echo $app_book_add->LeftColumnClass ?>"><?php echo $app_book->book_image->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $app_book_add->RightColumnClass ?>"><div<?php echo $app_book->book_image->CellAttributes() ?>>
...
<?php if ($app_book->book_desc->Visible) { // book_desc ?>
	<div id="r_book_desc" class="form-group">
		<label id="elh_app_book_book_desc" class="<?php echo $app_book_add->LeftColumnClass ?>"><?php echo $app_book->book_desc->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $app_book_add->RightColumnClass ?>"><div<?php echo $app_book->book_desc->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateEditor("fapp_bookadd", "x_book_desc", 35, 8, <?php echo ($app_book->book_desc->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<?php
	if (in_array("app_book_page", explode(",", $app_book->getCurrentDetailTable())) && $app_book_page->DetailAdd) {
?>
...
<?php } ?>
<?php if (!$app_book_add->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $app_book_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $app_book_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
