...
$app_book_page_delete = NULL; // Initialize page object first

class capp_book_page_delete extends capp_book_page {
	...
	// Page object name
	var $PageObjName = 'app_book_page_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("app_book_pagelist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in app_book_page class, app_book_pageinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("app_book_pagelist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($app_book_page_delete)) $app_book_page_delete = new capp_book_page_delete();

// Page init
$app_book_page_delete->Page_Init();

// Page main
$app_book_page_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_book_page_delete->Page_Render();
?>
...
var CurrentForm = fapp_book_pagedelete = new ew_Form("fapp_book_pagedelete", "delete");

// Form_CustomValidate event
fapp_book_pagedelete.Form_CustomValidate = 
...
fapp_book_pagedelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_book_pagedelete.Lists["x_book_id"] = {"LinkField":"x_book_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_book_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_book"};
fapp_book_pagedelete.Lists["x_book_id"].Data = "<?php echo $app_book_page_delete->book_id->LookupFilterQuery(FALSE, "delete") ?>";
...
<?php $app_book_page_delete->ShowPageHeader(); ?>
<?php
$app_book_page_delete->ShowMessage();
?>
<form name="fapp_book_pagedelete" id="fapp_book_pagedelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_book_page_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_book_page_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_book_page">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($app_book_page_delete->RecKeys as $key) { ?>
...
<?php if ($app_book_page->book_id->Visible) { // book_id ?>
		<th class="<?php echo $app_book_page->book_id->HeaderCellClass() ?>"><span id="elh_app_book_page_book_id" class="app_book_page_book_id"><?php echo $app_book_page->book_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($app_book_page->page_num->Visible) { // page_num ?>
		<th class="<?php echo $app_book_page->page_num->HeaderCellClass() ?>"><span id="elh_app_book_page_page_num" class="app_book_page_page_num"><?php echo $app_book_page->page_num->FldCaption() ?></span></th>
<?php } ?>
<?php if ($app_book_page->page_image->Visible) { // page_image ?>
		<th class="<?php echo $app_book_page->page_image->HeaderCellClass() ?>"><span id="elh_app_book_page_page_image" class="app_book_page_page_image"><?php echo $app_book_page->page_image->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$app_book_page_delete->RecCnt = 0;
$i = 0;
while (!$app_book_page_delete->Recordset->EOF) {
	$app_book_page_delete->RecCnt++;
	$app_book_page_delete->RowCnt++;

	// Set row properties
	$app_book_page->ResetAttrs();
	$app_book_page->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$app_book_page_delete->LoadRowValues($app_book_page_delete->Recordset);

	// Render row
	$app_book_page_delete->RenderRow();
?>
	<tr<?php echo $app_book_page->RowAttributes() ?>>
<?php if ($app_book_page->book_id->Visible) { // book_id ?>
		<td<?php echo $app_book_page->book_id->CellAttributes() ?>>
<span id="el<?php echo $app_book_page_delete->RowCnt ?>_app_book_page_book_id" class="app_book_page_book_id">
...
<?php } ?>
<?php if ($app_book_page->page_num->Visible) { // page_num ?>
		<td<?php echo $app_book_page->page_num->CellAttributes() ?>>
<span id="el<?php echo $app_book_page_delete->RowCnt ?>_app_book_page_page_num" class="app_book_page_page_num">
...
<?php } ?>
<?php if ($app_book_page->page_image->Visible) { // page_image ?>
		<td<?php echo $app_book_page->page_image->CellAttributes() ?>>
<span id="el<?php echo $app_book_page_delete->RowCnt ?>_app_book_page_page_image" class="app_book_page_page_image">
<span>
...
<?php
	$app_book_page_delete->Recordset->MoveNext();
}
$app_book_page_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $app_book_page_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fapp_book_pagedelete.Init();
</script>
<?php
$app_book_page_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$app_book_page_delete->Page_Terminate();
?>
