...
function CurrentProjectID() {
	...
	return "{0D45E216-03EA-4B51-B419-4F1A34666877}";
}
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($valid) {
						...
						$this->setSessionParentUserID($rs->fields('pgrp_id')); // Load parent User ID
						if (is_null($rs->fields('pgrp_id'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('pgrp_id'))); // Load User Level
						}
						$this->SetupUserLevel();
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if (EW_USER_ID_IS_HIERARCHICAL) {
				...
				while ($sUserIDList <> $sCurUserIDList) {
					$sFilter = '`pgrp_id` IN (' . $sCurUserIDList . ')';
					$sSql = $UserTable->GetSQL($sFilter, '');
					if ($RsUser = $UserTableConn->Execute($sSql)) {
						while (!$RsUser->EOF) {
							$this->AddUserID($RsUser->fields('user_id'));
