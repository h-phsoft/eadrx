...
$phs_bill_cycle_update = NULL; // Initialize page object first

class cphs_bill_cycle_update extends cphs_bill_cycle {
	...
	// Page object name
	var $PageObjName = 'phs_bill_cycle_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("phs_bill_cyclelist.php"); // No records selected, return to list
	...
	function LoadMultiUpdateValues() {
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					$this->cycle_name->setDbValue($this->Recordset->fields('cycle_name'));
					$this->cycle_interval->setDbValue($this->Recordset->fields('cycle_interval'));
					$this->status_id->setDbValue($this->Recordset->fields('status_id'));
					$this->cycle_desc->setDbValue($this->Recordset->fields('cycle_desc'));
				} else {
					if (!ew_CompareValue($this->cycle_name->DbValue, $this->Recordset->fields('cycle_name')))
						$this->cycle_name->CurrentValue = NULL;
					if (!ew_CompareValue($this->cycle_interval->DbValue, $this->Recordset->fields('cycle_interval')))
						$this->cycle_interval->CurrentValue = NULL;
					if (!ew_CompareValue($this->status_id->DbValue, $this->Recordset->fields('status_id')))
						$this->status_id->CurrentValue = NULL;
					if (!ew_CompareValue($this->cycle_desc->DbValue, $this->Recordset->fields('cycle_desc')))
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->cycle_id->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->cycle_name->MultiUpdate = $objForm->GetValue("u_cycle_name");
		if (!$this->cycle_interval->FldIsDetailKey) {
			$this->cycle_interval->setFormValue($objForm->GetValue("x_cycle_interval"));
		}
		$this->cycle_interval->MultiUpdate = $objForm->GetValue("u_cycle_interval");
		...
		$this->cycle_desc->MultiUpdate = $objForm->GetValue("u_cycle_desc");
	...
	function ValidateForm() {
		...
		$gsFormError = "";
		$lUpdateCnt = 0;
		if ($this->cycle_name->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->cycle_interval->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->status_id->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->cycle_desc->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if ($this->cycle_name->MultiUpdate <> "" && !$this->cycle_name->FldIsDetailKey && !is_null($this->cycle_name->FormValue) && $this->cycle_name->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->cycle_name->FldCaption(), $this->cycle_name->ReqErrMsg));
		}
		if ($this->cycle_desc->MultiUpdate <> "" && !$this->cycle_desc->FldIsDetailKey && !is_null($this->cycle_desc->FormValue) && $this->cycle_desc->FormValue == "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// cycle_name
			$this->cycle_name->SetDbValueDef($rsnew, $this->cycle_name->CurrentValue, "", $this->cycle_name->ReadOnly || $this->cycle_name->MultiUpdate <> "1");

			// cycle_interval
			$this->cycle_interval->SetDbValueDef($rsnew, $this->cycle_interval->CurrentValue, "", $this->cycle_interval->ReadOnly || $this->cycle_interval->MultiUpdate <> "1");

			// status_id
			$this->status_id->SetDbValueDef($rsnew, $this->status_id->CurrentValue, 0, $this->status_id->ReadOnly || $this->status_id->MultiUpdate <> "1");

			// cycle_desc
			$this->cycle_desc->SetDbValueDef($rsnew, $this->cycle_desc->CurrentValue, "", $this->cycle_desc->ReadOnly || $this->cycle_desc->MultiUpdate <> "1");
...
// Create page object
if (!isset($phs_bill_cycle_update)) $phs_bill_cycle_update = new cphs_bill_cycle_update();

// Page init
$phs_bill_cycle_update->Page_Init();

// Page main
$phs_bill_cycle_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$phs_bill_cycle_update->Page_Render();
?>
...
var CurrentForm = fphs_bill_cycleupdate = new ew_Form("fphs_bill_cycleupdate", "update");

// Validate form
fphs_bill_cycleupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_cycle_name");
			...
			uelm = this.GetElements("u" + infix + "_cycle_desc");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
...
fphs_bill_cycleupdate.Form_CustomValidate = 
...
fphs_bill_cycleupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fphs_bill_cycleupdate.Lists["x_status_id"] = {"LinkField":"x_status_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_status_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_status"};
fphs_bill_cycleupdate.Lists["x_status_id"].Data = "<?php echo $phs_bill_cycle_update->status_id->LookupFilterQuery(FALSE, "update") ?>";
...
<?php $phs_bill_cycle_update->ShowPageHeader(); ?>
<?php
$phs_bill_cycle_update->ShowMessage();
?>
<form name="fphs_bill_cycleupdate" id="fphs_bill_cycleupdate" class="<?php echo $phs_bill_cycle_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($phs_bill_cycle_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $phs_bill_cycle_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="phs_bill_cycle">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($phs_bill_cycle_update->IsModal) ?>">
<?php foreach ($phs_bill_cycle_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_phs_bill_cycleupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($phs_bill_cycle->cycle_name->Visible) { // cycle_name ?>
	<div id="r_cycle_name" class="form-group">
		<label for="x_cycle_name" class="<?php echo $phs_bill_cycle_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_cycle_name" id="u_cycle_name" class="ewMultiSelect" value="1"<?php echo ($phs_bill_cycle->cycle_name->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_bill_cycle->cycle_name->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_bill_cycle_update->RightColumnClass ?>"><div<?php echo $phs_bill_cycle->cycle_name->CellAttributes() ?>>
...
<?php if ($phs_bill_cycle->cycle_interval->Visible) { // cycle_interval ?>
	<div id="r_cycle_interval" class="form-group">
		<label for="x_cycle_interval" class="<?php echo $phs_bill_cycle_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_cycle_interval" id="u_cycle_interval" class="ewMultiSelect" value="1"<?php echo ($phs_bill_cycle->cycle_interval->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_bill_cycle->cycle_interval->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_bill_cycle_update->RightColumnClass ?>"><div<?php echo $phs_bill_cycle->cycle_interval->CellAttributes() ?>>
...
<?php if ($phs_bill_cycle->status_id->Visible) { // status_id ?>
	<div id="r_status_id" class="form-group">
		<label for="x_status_id" class="<?php echo $phs_bill_cycle_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_status_id" id="u_status_id" class="ewMultiSelect" value="1"<?php echo ($phs_bill_cycle->status_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_bill_cycle->status_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_bill_cycle_update->RightColumnClass ?>"><div<?php echo $phs_bill_cycle->status_id->CellAttributes() ?>>
...
<?php if ($phs_bill_cycle->cycle_desc->Visible) { // cycle_desc ?>
	<div id="r_cycle_desc" class="form-group">
		<label for="x_cycle_desc" class="<?php echo $phs_bill_cycle_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_cycle_desc" id="u_cycle_desc" class="ewMultiSelect" value="1"<?php echo ($phs_bill_cycle->cycle_desc->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_bill_cycle->cycle_desc->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_bill_cycle_update->RightColumnClass ?>"><div<?php echo $phs_bill_cycle->cycle_desc->CellAttributes() ?>>
...
<?php if (!$phs_bill_cycle_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $phs_bill_cycle_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $phs_bill_cycle_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
	</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fphs_bill_cycleupdate.Init();
</script>
<?php
$phs_bill_cycle_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$phs_bill_cycle_update->Page_Terminate();
?>
