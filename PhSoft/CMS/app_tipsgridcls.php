...
$app_tips_grid = NULL; // Initialize page object first

class capp_tips_grid extends capp_tips {
	...
	// Page object name
	var $PageObjName = 'app_tips_grid';

	// Grid form hidden field names
	var $FormName = 'fapp_tipsgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["app_tips"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->tips_id->CurrentValue = strval($arKeys[0]); // tips_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "app_tips_category") {
			$this->tcat_id->Visible = FALSE;
			if ($GLOBALS["app_tips_category"]->EventCancelled) $this->EventCancelled = TRUE;
		}
