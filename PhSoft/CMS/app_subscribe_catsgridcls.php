...
$app_subscribe_cats_grid = NULL; // Initialize page object first

class capp_subscribe_cats_grid extends capp_subscribe_cats {
	...
	// Page object name
	var $PageObjName = 'app_subscribe_cats_grid';

	// Grid form hidden field names
	var $FormName = 'fapp_subscribe_catsgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["app_subscribe_cats"];

		}
		$this->AddUrl = "app_subscribe_catsadd.php";
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "app_subscribe") {
			global $app_subscribe;
			$rsmaster = $app_subscribe->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("app_subscribelist.php"); // Return to master page
			} else {
				$app_subscribe->LoadListRowValues($rsmaster);
				$app_subscribe->RowType = EW_ROWTYPE_MASTER; // Master row
				$app_subscribe->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->tsub_id->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->tsub_id->FormValue))
				return FALSE;
		}
		return TRUE;
	}
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->tsub_id->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_sub_id") && $objForm->HasValue("o_sub_id") && $this->sub_id->CurrentValue <> $this->sub_id->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_tpkg_id") && $objForm->HasValue("o_tpkg_id") && $this->tpkg_id->CurrentValue <> $this->tpkg_id->OldValue)
			return FALSE;
	...
	function LoadSortOrder() {
		...
		if ($sOrderBy == "") {
			if ($this->getSqlOrderBy() <> "") {
				...
				$this->sub_id->setSort("ASC");
				$this->tpkg_id->setSort("ASC");
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->tsub_id->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->sub_id->setOldValue($objForm->GetValue("o_sub_id"));
		if (!$this->tpkg_id->FldIsDetailKey) {
			$this->tpkg_id->setFormValue($objForm->GetValue("x_tpkg_id"));
		}
		$this->tpkg_id->setOldValue($objForm->GetValue("o_tpkg_id"));
		...
		if (!$this->tsub_id->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->tsub_id->CurrentValue = strval($arKeys[0]); // tsub_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "app_subscribe") {
			...
			if ($GLOBALS["app_subscribe"]->EventCancelled) $this->EventCancelled = TRUE;
		}
