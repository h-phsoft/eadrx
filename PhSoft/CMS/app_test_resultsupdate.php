...
$app_test_results_update = NULL; // Initialize page object first

class capp_test_results_update extends capp_test_results {
	...
	// Page object name
	var $PageObjName = 'app_test_results_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("app_test_resultslist.php"); // No records selected, return to list
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					...
					$this->res_request->setDbValue($this->Recordset->fields('res_request'));
					$this->res_result->setDbValue($this->Recordset->fields('res_result'));
					...
					if (!ew_CompareValue($this->res_request->DbValue, $this->Recordset->fields('res_request')))
						$this->res_request->CurrentValue = NULL;
					if (!ew_CompareValue($this->res_result->DbValue, $this->Recordset->fields('res_result')))
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->res_id->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->res_request->MultiUpdate = $objForm->GetValue("u_res_request");
		if (!$this->res_result->FldIsDetailKey) {
			$this->res_result->setFormValue($objForm->GetValue("x_res_result"));
		}
		$this->res_result->MultiUpdate = $objForm->GetValue("u_res_result");
	...
	function ValidateForm() {
		...
		if ($this->res_request->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->res_result->MultiUpdate == "1") $lUpdateCnt++;
		...
		if ($this->res_request->MultiUpdate <> "" && !$this->res_request->FldIsDetailKey && !is_null($this->res_request->FormValue) && $this->res_request->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->res_request->FldCaption(), $this->res_request->ReqErrMsg));
		}
		if ($this->res_result->MultiUpdate <> "" && !$this->res_result->FldIsDetailKey && !is_null($this->res_result->FormValue) && $this->res_result->FormValue == "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// res_request
			$this->res_request->SetDbValueDef($rsnew, $this->res_request->CurrentValue, "", $this->res_request->ReadOnly || $this->res_request->MultiUpdate <> "1");

			// res_result
			$this->res_result->SetDbValueDef($rsnew, $this->res_result->CurrentValue, "", $this->res_result->ReadOnly || $this->res_result->MultiUpdate <> "1");
...
// Create page object
if (!isset($app_test_results_update)) $app_test_results_update = new capp_test_results_update();

// Page init
$app_test_results_update->Page_Init();

// Page main
$app_test_results_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_test_results_update->Page_Render();
?>
...
var CurrentForm = fapp_test_resultsupdate = new ew_Form("fapp_test_resultsupdate", "update");

// Validate form
fapp_test_resultsupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_res_request");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
			...
			uelm = this.GetElements("u" + infix + "_res_result");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
...
fapp_test_resultsupdate.Form_CustomValidate = 
...
fapp_test_resultsupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_test_resultsupdate.Lists["x_user_id"] = {"LinkField":"x_user_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_user_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cpy_user"};
fapp_test_resultsupdate.Lists["x_user_id"].Data = "<?php echo $app_test_results_update->user_id->LookupFilterQuery(FALSE, "update") ?>";
fapp_test_resultsupdate.Lists["x_test_id"] = {"LinkField":"x_test_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_test_iname","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"app_test"};
fapp_test_resultsupdate.Lists["x_test_id"].Data = "<?php echo $app_test_results_update->test_id->LookupFilterQuery(FALSE, "update") ?>";
...
<?php $app_test_results_update->ShowPageHeader(); ?>
<?php
$app_test_results_update->ShowMessage();
?>
<form name="fapp_test_resultsupdate" id="fapp_test_resultsupdate" class="<?php echo $app_test_results_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_test_results_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_test_results_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_test_results">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($app_test_results_update->IsModal) ?>">
<?php foreach ($app_test_results_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_app_test_resultsupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($app_test_results->user_id->Visible) { // user_id ?>
	<div id="r_user_id" class="form-group">
		<label for="x_user_id" class="<?php echo $app_test_results_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_user_id" id="u_user_id" class="ewMultiSelect" value="1"<?php echo ($app_test_results->user_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test_results->user_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_results_update->RightColumnClass ?>"><div<?php echo $app_test_results->user_id->CellAttributes() ?>>
...
<?php if ($app_test_results->test_id->Visible) { // test_id ?>
	<div id="r_test_id" class="form-group">
		<label for="x_test_id" class="<?php echo $app_test_results_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_test_id" id="u_test_id" class="ewMultiSelect" value="1"<?php echo ($app_test_results->test_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test_results->test_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_results_update->RightColumnClass ?>"><div<?php echo $app_test_results->test_id->CellAttributes() ?>>
...
<?php if ($app_test_results->res_request->Visible) { // res_request ?>
	<div id="r_res_request" class="form-group">
		<label for="x_res_request" class="<?php echo $app_test_results_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_res_request" id="u_res_request" class="ewMultiSelect" value="1"<?php echo ($app_test_results->res_request->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test_results->res_request->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_results_update->RightColumnClass ?>"><div<?php echo $app_test_results->res_request->CellAttributes() ?>>
...
<?php if ($app_test_results->res_result->Visible) { // res_result ?>
	<div id="r_res_result" class="form-group">
		<label for="x_res_result" class="<?php echo $app_test_results_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_res_result" id="u_res_result" class="ewMultiSelect" value="1"<?php echo ($app_test_results->res_result->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test_results->res_result->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_results_update->RightColumnClass ?>"><div<?php echo $app_test_results->res_result->CellAttributes() ?>>
...
<?php if ($app_test_results->ins_datetime->Visible) { // ins_datetime ?>
	<div id="r_ins_datetime" class="form-group">
		<label for="x_ins_datetime" class="<?php echo $app_test_results_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_ins_datetime" id="u_ins_datetime" class="ewMultiSelect" value="1"<?php echo ($app_test_results->ins_datetime->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_test_results->ins_datetime->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_test_results_update->RightColumnClass ?>"><div<?php echo $app_test_results->ins_datetime->CellAttributes() ?>>
<span id="el_app_test_results_ins_datetime">
<input type="text" data-table="app_test_results" data-field="x_ins_datetime" data-format="11" name="x_ins_datetime" id="x_ins_datetime" placeholder="<?php echo ew_HtmlEncode($app_test_results->ins_datetime->getPlaceHolder()) ?>" value="<?php echo $app_test_results->ins_datetime->EditValue ?>"<?php echo $app_test_results->ins_datetime->EditAttributes() ?>>
</span>
<?php echo $app_test_results->ins_datetime->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page -->
<?php if (!$app_test_results_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $app_test_results_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $app_test_results_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
	</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fapp_test_resultsupdate.Init();
</script>
<?php
$app_test_results_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$app_test_results_update->Page_Terminate();
?>
