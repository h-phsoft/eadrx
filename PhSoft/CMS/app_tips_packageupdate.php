...
$app_tips_package_update = NULL; // Initialize page object first

class capp_tips_package_update extends capp_tips_package {
	...
	// Page object name
	var $PageObjName = 'app_tips_package_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("app_tips_packagelist.php"); // No records selected, return to list
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					...
					$this->pkg_order->setDbValue($this->Recordset->fields('pkg_order'));
					$this->pkg_name->setDbValue($this->Recordset->fields('pkg_name'));
					$this->pkg_price->setDbValue($this->Recordset->fields('pkg_price'));
				} else {
					...
					if (!ew_CompareValue($this->pkg_order->DbValue, $this->Recordset->fields('pkg_order')))
						$this->pkg_order->CurrentValue = NULL;
					if (!ew_CompareValue($this->pkg_name->DbValue, $this->Recordset->fields('pkg_name')))
						$this->pkg_name->CurrentValue = NULL;
					if (!ew_CompareValue($this->pkg_price->DbValue, $this->Recordset->fields('pkg_price')))
						$this->pkg_price->CurrentValue = NULL;
				}
				$i++;
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->pkg_id->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function ValidateForm() {
		...
		if ($this->pkg_order->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->pkg_name->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->pkg_price->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// pkg_order
			$this->pkg_order->SetDbValueDef($rsnew, $this->pkg_order->CurrentValue, 0, $this->pkg_order->ReadOnly || $this->pkg_order->MultiUpdate <> "1");

			// pkg_name
			$this->pkg_name->SetDbValueDef($rsnew, $this->pkg_name->CurrentValue, "", $this->pkg_name->ReadOnly || $this->pkg_name->MultiUpdate <> "1");

			// pkg_price
			$this->pkg_price->SetDbValueDef($rsnew, $this->pkg_price->CurrentValue, 0, $this->pkg_price->ReadOnly || $this->pkg_price->MultiUpdate <> "1");
...
// Create page object
if (!isset($app_tips_package_update)) $app_tips_package_update = new capp_tips_package_update();

// Page init
$app_tips_package_update->Page_Init();

// Page main
$app_tips_package_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_tips_package_update->Page_Render();
?>
...
var CurrentForm = fapp_tips_packageupdate = new ew_Form("fapp_tips_packageupdate", "update");

// Validate form
fapp_tips_packageupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_pkg_order");
			...
			uelm = this.GetElements("u" + infix + "_pkg_name");
			...
			uelm = this.GetElements("u" + infix + "_pkg_price");
...
fapp_tips_packageupdate.Form_CustomValidate = 
...
fapp_tips_packageupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapp_tips_packageupdate.Lists["x_cycle_id"] = {"LinkField":"x_cycle_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_cycle_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_bill_cycle"};
fapp_tips_packageupdate.Lists["x_cycle_id"].Data = "<?php echo $app_tips_package_update->cycle_id->LookupFilterQuery(FALSE, "update") ?>";
fapp_tips_packageupdate.Lists["x_status_id"] = {"LinkField":"x_status_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_status_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_status"};
fapp_tips_packageupdate.Lists["x_status_id"].Data = "<?php echo $app_tips_package_update->status_id->LookupFilterQuery(FALSE, "update") ?>";
...
<?php $app_tips_package_update->ShowPageHeader(); ?>
<?php
$app_tips_package_update->ShowMessage();
?>
<form name="fapp_tips_packageupdate" id="fapp_tips_packageupdate" class="<?php echo $app_tips_package_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_tips_package_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_tips_package_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_tips_package">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($app_tips_package_update->IsModal) ?>">
<?php foreach ($app_tips_package_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_app_tips_packageupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($app_tips_package->cycle_id->Visible) { // cycle_id ?>
	<div id="r_cycle_id" class="form-group">
		<label for="x_cycle_id" class="<?php echo $app_tips_package_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_cycle_id" id="u_cycle_id" class="ewMultiSelect" value="1"<?php echo ($app_tips_package->cycle_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_tips_package->cycle_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_tips_package_update->RightColumnClass ?>"><div<?php echo $app_tips_package->cycle_id->CellAttributes() ?>>
...
<?php if ($app_tips_package->status_id->Visible) { // status_id ?>
	<div id="r_status_id" class="form-group">
		<label for="x_status_id" class="<?php echo $app_tips_package_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_status_id" id="u_status_id" class="ewMultiSelect" value="1"<?php echo ($app_tips_package->status_id->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_tips_package->status_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_tips_package_update->RightColumnClass ?>"><div<?php echo $app_tips_package->status_id->CellAttributes() ?>>
...
<?php if ($app_tips_package->pkg_order->Visible) { // pkg_order ?>
	<div id="r_pkg_order" class="form-group">
		<label for="x_pkg_order" class="<?php echo $app_tips_package_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_pkg_order" id="u_pkg_order" class="ewMultiSelect" value="1"<?php echo ($app_tips_package->pkg_order->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_tips_package->pkg_order->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_tips_package_update->RightColumnClass ?>"><div<?php echo $app_tips_package->pkg_order->CellAttributes() ?>>
...
<?php if ($app_tips_package->pkg_name->Visible) { // pkg_name ?>
	<div id="r_pkg_name" class="form-group">
		<label for="x_pkg_name" class="<?php echo $app_tips_package_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_pkg_name" id="u_pkg_name" class="ewMultiSelect" value="1"<?php echo ($app_tips_package->pkg_name->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_tips_package->pkg_name->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_tips_package_update->RightColumnClass ?>"><div<?php echo $app_tips_package->pkg_name->CellAttributes() ?>>
...
<?php if ($app_tips_package->pkg_price->Visible) { // pkg_price ?>
	<div id="r_pkg_price" class="form-group">
		<label for="x_pkg_price" class="<?php echo $app_tips_package_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_pkg_price" id="u_pkg_price" class="ewMultiSelect" value="1"<?php echo ($app_tips_package->pkg_price->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $app_tips_package->pkg_price->FldCaption() ?></label></div></label>
		<div class="<?php echo $app_tips_package_update->RightColumnClass ?>"><div<?php echo $app_tips_package->pkg_price->CellAttributes() ?>>
...
<?php if (!$app_tips_package_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $app_tips_package_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $app_tips_package_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
	</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fapp_tips_packageupdate.Init();
</script>
<?php
$app_tips_package_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$app_tips_package_update->Page_Terminate();
?>
