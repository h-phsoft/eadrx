...
//
// Table class for phs_language
//
class cphs_language extends cTable {
	var $lang_id;
	var $status_id;
	var $lang_code;
	var $lang_dir;
	...
	function __construct() {
		...
		$this->TableVar = 'phs_language';
		$this->TableName = 'phs_language';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`phs_language`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// lang_id
		$this->lang_id = new cField('phs_language', 'phs_language', 'x_lang_id', 'lang_id', '`lang_id`', '`lang_id`', 3, -1, FALSE, '`lang_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// status_id
		$this->status_id = new cField('phs_language', 'phs_language', 'x_status_id', 'status_id', '`status_id`', '`status_id`', 16, -1, FALSE, '`status_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// lang_code
		$this->lang_code = new cField('phs_language', 'phs_language', 'x_lang_code', 'lang_code', '`lang_code`', '`lang_code`', 200, -1, FALSE, '`lang_code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->lang_code->Sortable = TRUE; // Allow sort
		$this->fields['lang_code'] = &$this->lang_code;

		// lang_dir
		$this->lang_dir = new cField('phs_language', 'phs_language', 'x_lang_dir', 'lang_dir', '`lang_dir`', '`lang_dir`', 200, -1, FALSE, '`lang_dir`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->lang_dir->Sortable = TRUE; // Allow sort
		$this->lang_dir->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->lang_dir->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->lang_dir->OptionCount = 2;
		$this->fields['lang_dir'] = &$this->lang_dir;

		// lang_name
		$this->lang_name = new cField('phs_language', 'phs_language', 'x_lang_name', 'lang_name', '`lang_name`', '`lang_name`', 200, -1, FALSE, '`lang_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->lang_name->Sortable = TRUE; // Allow sort
		$this->fields['lang_name'] = &$this->lang_name;

		// lang_dname
		$this->lang_dname = new cField('phs_language', 'phs_language', 'x_lang_dname', 'lang_dname', '`lang_dname`', '`lang_dname`', 200, -1, FALSE, '`lang_dname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->lang_dname->Sortable = TRUE; // Allow sort
		$this->fields['lang_dname'] = &$this->lang_dname;

		// lang_ccode
		$this->lang_ccode = new cField('phs_language', 'phs_language', 'x_lang_ccode', 'lang_ccode', '`lang_ccode`', '`lang_ccode`', 200, -1, FALSE, '`lang_ccode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->lang_ccode->Sortable = TRUE; // Allow sort
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`phs_language`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "phs_languageedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "phs_languageadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("phs_languageview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("phs_languageview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "phs_languageadd.php?" . $this->UrlParm($parm);
		else
			$url = "phs_languageadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("phs_languageedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("phs_languageadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("phs_languagedelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->lang_code->Exportable) $Doc->ExportCaption($this->lang_code);
					if ($this->lang_dir->Exportable) $Doc->ExportCaption($this->lang_dir);
					if ($this->lang_name->Exportable) $Doc->ExportCaption($this->lang_name);
					if ($this->lang_dname->Exportable) $Doc->ExportCaption($this->lang_dname);
					if ($this->lang_ccode->Exportable) $Doc->ExportCaption($this->lang_ccode);
				} else {
					if ($this->status_id->Exportable) $Doc->ExportCaption($this->status_id);
					if ($this->lang_code->Exportable) $Doc->ExportCaption($this->lang_code);
					if ($this->lang_dir->Exportable) $Doc->ExportCaption($this->lang_dir);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->lang_code->Exportable) $Doc->ExportField($this->lang_code);
						if ($this->lang_dir->Exportable) $Doc->ExportField($this->lang_dir);
						if ($this->lang_name->Exportable) $Doc->ExportField($this->lang_name);
						if ($this->lang_dname->Exportable) $Doc->ExportField($this->lang_dname);
						if ($this->lang_ccode->Exportable) $Doc->ExportField($this->lang_ccode);
					} else {
						if ($this->status_id->Exportable) $Doc->ExportField($this->status_id);
						if ($this->lang_code->Exportable) $Doc->ExportField($this->lang_code);
						if ($this->lang_dir->Exportable) $Doc->ExportField($this->lang_dir);
