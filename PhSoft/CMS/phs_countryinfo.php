...
//
// Table class for phs_country
//
class cphs_country extends cTable {
	var $cntry_id;
	var $status_id;
	var $cntry_code;
	var $cntry_name;
	...
	function __construct() {
		...
		$this->TableVar = 'phs_country';
		$this->TableName = 'phs_country';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`phs_country`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// cntry_id
		$this->cntry_id = new cField('phs_country', 'phs_country', 'x_cntry_id', 'cntry_id', '`cntry_id`', '`cntry_id`', 3, -1, FALSE, '`cntry_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// status_id
		$this->status_id = new cField('phs_country', 'phs_country', 'x_status_id', 'status_id', '`status_id`', '`status_id`', 16, -1, FALSE, '`status_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// cntry_code
		$this->cntry_code = new cField('phs_country', 'phs_country', 'x_cntry_code', 'cntry_code', '`cntry_code`', '`cntry_code`', 200, -1, FALSE, '`cntry_code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->cntry_code->Sortable = TRUE; // Allow sort
		$this->fields['cntry_code'] = &$this->cntry_code;

		// cntry_name
		$this->cntry_name = new cField('phs_country', 'phs_country', 'x_cntry_name', 'cntry_name', '`cntry_name`', '`cntry_name`', 200, -1, FALSE, '`cntry_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`phs_country`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "phs_countryedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "phs_countryadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("phs_countryview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("phs_countryview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "phs_countryadd.php?" . $this->UrlParm($parm);
		else
			$url = "phs_countryadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("phs_countryedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("phs_countryadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("phs_countrydelete.php", $this->UrlParm());
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["cntry_id"]))
				$arKeys[] = $_POST["cntry_id"];
			elseif (isset($_GET["cntry_id"]))
				$arKeys[] = $_GET["cntry_id"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->cntry_id->CurrentValue = $key;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->cntry_code->Exportable) $Doc->ExportCaption($this->cntry_code);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->cntry_code->Exportable) $Doc->ExportField($this->cntry_code);
