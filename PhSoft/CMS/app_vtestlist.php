...
<?php include_once "app_vtestinfo.php" ?>
...
$app_vtest_list = NULL; // Initialize page object first

class capp_vtest_list extends capp_vtest {
	...
	// Table name
	var $TableName = 'app_vtest';

	// Page object name
	var $PageObjName = 'app_vtest_list';

	// Grid form hidden field names
	var $FormName = 'fapp_vtestlist';
	...
	function __construct() {
		...
		// Table object (app_vtest)
		if (!isset($GLOBALS["app_vtest"]) || get_class($GLOBALS["app_vtest"]) == "capp_vtest") {
			$GLOBALS["app_vtest"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["app_vtest"];
		}
		...
		$this->AddUrl = "app_vtestadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "app_vtestdelete.php";
		$this->MultiUpdateUrl = "app_vtestupdate.php";
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'app_vtest', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fapp_vtestlistsrch";
	...
	function Page_Terminate($url = "") {
		...
		// Export
		global $EW_EXPORT, $app_vtest;
		if ($this->CustomExport <> "" && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, $EW_EXPORT)) {
				$sContent = ob_get_contents();
			if ($gsExportFile == "") $gsExportFile = $this->TableVar;
			$class = $EW_EXPORT[$this->CustomExport];
			if (class_exists($class)) {
				$doc = new $class($app_vtest);
				$doc->Text = $sContent;
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fapp_vtestlistsrch", $filters);
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" class=\"ewMultiSelect\" value=\"" . ew_HtmlEncode($this->test_id->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->ntest_id->CurrentValue) . "\" onclick=\"ew_ClickMultiCheckbox(event);\">";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fapp_vtestlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fapp_vtestlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fapp_vtestlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fapp_vtestlistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_app_vtest\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_app_vtest',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fapp_vtestlist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($app_vtest_list)) $app_vtest_list = new capp_vtest_list();

// Page init
$app_vtest_list->Page_Init();

// Page main
$app_vtest_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$app_vtest_list->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if ($app_vtest->Export == "") { ?>
<script type="text/javascript">

// Form object
var CurrentPageID = EW_PAGE_ID = "list";
var CurrentForm = fapp_vtestlist = new ew_Form("fapp_vtestlist", "list");
fapp_vtestlist.FormKeyCountName = '<?php echo $app_vtest_list->FormKeyCountName ?>';

// Form_CustomValidate event
fapp_vtestlist.Form_CustomValidate = 
...
<?php if ($app_vtest->Export == "") { ?>
<div class="ewToolbar">
<?php if ($app_vtest_list->TotalRecs > 0 && $app_vtest_list->ExportOptions->Visible()) { ?>
<?php $app_vtest_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($app_vtest_list->SearchOptions->Visible()) { ?>
<?php $app_vtest_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($app_vtest_list->FilterOptions->Visible()) { ?>
<?php $app_vtest_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $app_vtest_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($app_vtest_list->TotalRecs <= 0)
			$app_vtest_list->TotalRecs = $app_vtest->ListRecordCount();
	} else {
		if (!$app_vtest_list->Recordset && ($app_vtest_list->Recordset = $app_vtest_list->LoadRecordset()))
			$app_vtest_list->TotalRecs = $app_vtest_list->Recordset->RecordCount();
	}
	$app_vtest_list->StartRec = 1;
	if ($app_vtest_list->DisplayRecs <= 0 || ($app_vtest->Export <> "" && $app_vtest->ExportAll)) // Display all records
		$app_vtest_list->DisplayRecs = $app_vtest_list->TotalRecs;
	if (!($app_vtest->Export <> "" && $app_vtest->ExportAll))
		$app_vtest_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$app_vtest_list->Recordset = $app_vtest_list->LoadRecordset($app_vtest_list->StartRec-1, $app_vtest_list->DisplayRecs);

	// Set no record found message
	if ($app_vtest->CurrentAction == "" && $app_vtest_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$app_vtest_list->setWarningMessage(ew_DeniedMsg());
		if ($app_vtest_list->SearchWhere == "0=101")
			$app_vtest_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$app_vtest_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$app_vtest_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($app_vtest->Export == "" && $app_vtest->CurrentAction == "") { ?>
<form name="fapp_vtestlistsrch" id="fapp_vtestlistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($app_vtest_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fapp_vtestlistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="app_vtest">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($app_vtest_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($app_vtest_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $app_vtest_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($app_vtest_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($app_vtest_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($app_vtest_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($app_vtest_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $app_vtest_list->ShowPageHeader(); ?>
<?php
$app_vtest_list->ShowMessage();
?>
<?php if ($app_vtest_list->TotalRecs > 0 || $app_vtest->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($app_vtest_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> app_vtest">
<?php if ($app_vtest->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($app_vtest->CurrentAction <> "gridadd" && $app_vtest->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($app_vtest_list->Pager)) $app_vtest_list->Pager = new cPrevNextPager($app_vtest_list->StartRec, $app_vtest_list->DisplayRecs, $app_vtest_list->TotalRecs, $app_vtest_list->AutoHidePager) ?>
<?php if ($app_vtest_list->Pager->RecordCount > 0 && $app_vtest_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($app_vtest_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $app_vtest_list->PageUrl() ?>start=<?php echo $app_vtest_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($app_vtest_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $app_vtest_list->PageUrl() ?>start=<?php echo $app_vtest_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $app_vtest_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($app_vtest_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $app_vtest_list->PageUrl() ?>start=<?php echo $app_vtest_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($app_vtest_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $app_vtest_list->PageUrl() ?>start=<?php echo $app_vtest_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $app_vtest_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($app_vtest_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $app_vtest_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $app_vtest_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $app_vtest_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($app_vtest_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="fapp_vtestlist" id="fapp_vtestlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_vtest_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_vtest_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="app_vtest">
<div id="gmp_app_vtest" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($app_vtest_list->TotalRecs > 0 || $app_vtest->CurrentAction == "gridedit") { ?>
<table id="tbl_app_vtestlist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$app_vtest_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$app_vtest_list->RenderListOptions();
...
<?php if ($app_vtest->lang_id->Visible) { // lang_id ?>
	...
	<?php } ?>
<?php } ?>
<?php if ($app_vtest->test_num->Visible) { // test_num ?>
	<?php if ($app_vtest->SortUrl($app_vtest->test_num) == "") { ?>
		<th data-name="test_num" class="<?php echo $app_vtest->test_num->HeaderCellClass() ?>"><div id="elh_app_vtest_test_num" class="app_vtest_test_num"><div class="ewTableHeaderCaption"><?php echo $app_vtest->test_num->FldCaption() ?></div></div></th>
	<?php } else { ?>
...
<?php if ($app_vtest->test_name->Visible) { // test_name ?>
	...
	<?php } ?>
<?php } ?>
<?php if ($app_vtest->test_iname->Visible) { // test_iname ?>
	<?php if ($app_vtest->SortUrl($app_vtest->test_iname) == "") { ?>
		<th data-name="test_iname" class="<?php echo $app_vtest->test_iname->HeaderCellClass() ?>"><div id="elh_app_vtest_test_iname" class="app_vtest_test_iname"><div class="ewTableHeaderCaption"><?php echo $app_vtest->test_iname->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="test_iname" class="<?php echo $app_vtest->test_iname->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $app_vtest->SortUrl($app_vtest->test_iname) ?>',1);"><div id="elh_app_vtest_test_iname" class="app_vtest_test_iname">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $app_vtest->test_iname->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($app_vtest->test_iname->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($app_vtest->test_iname->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($app_vtest->status_id->Visible) { // status_id ?>
	<?php if ($app_vtest->SortUrl($app_vtest->status_id) == "") { ?>
		<th data-name="status_id" class="<?php echo $app_vtest->status_id->HeaderCellClass() ?>"><div id="elh_app_vtest_status_id" class="app_vtest_status_id"><div class="ewTableHeaderCaption"><?php echo $app_vtest->status_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="status_id" class="<?php echo $app_vtest->status_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $app_vtest->SortUrl($app_vtest->status_id) ?>',1);"><div id="elh_app_vtest_status_id" class="app_vtest_status_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $app_vtest->status_id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($app_vtest->status_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($app_vtest->status_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($app_vtest->test_image->Visible) { // test_image ?>
	...
	<?php } else { ?>
		<th data-name="test_image" class="<?php echo $app_vtest->test_image->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $app_vtest->SortUrl($app_vtest->test_image) ?>',1);"><div id="elh_app_vtest_test_image" class="app_vtest_test_image">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $app_vtest->test_image->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($app_vtest->test_image->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($app_vtest->test_image->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
...
$app_vtest_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($app_vtest->ExportAll && $app_vtest->Export <> "") {
	$app_vtest_list->StopRec = $app_vtest_list->TotalRecs;
} else {

	// Set the last record to display
	if ($app_vtest_list->TotalRecs > $app_vtest_list->StartRec + $app_vtest_list->DisplayRecs - 1)
		$app_vtest_list->StopRec = $app_vtest_list->StartRec + $app_vtest_list->DisplayRecs - 1;
	else
		$app_vtest_list->StopRec = $app_vtest_list->TotalRecs;
}
$app_vtest_list->RecCnt = $app_vtest_list->StartRec - 1;
if ($app_vtest_list->Recordset && !$app_vtest_list->Recordset->EOF) {
	$app_vtest_list->Recordset->MoveFirst();
	$bSelectLimit = $app_vtest_list->UseSelectLimit;
	if (!$bSelectLimit && $app_vtest_list->StartRec > 1)
		$app_vtest_list->Recordset->Move($app_vtest_list->StartRec - 1);
} elseif (!$app_vtest->AllowAddDeleteRow && $app_vtest_list->StopRec == 0) {
	$app_vtest_list->StopRec = $app_vtest->GridAddRowCount;
}

// Initialize aggregate
$app_vtest->RowType = EW_ROWTYPE_AGGREGATEINIT;
$app_vtest->ResetAttrs();
$app_vtest_list->RenderRow();
while ($app_vtest_list->RecCnt < $app_vtest_list->StopRec) {
	$app_vtest_list->RecCnt++;
	if (intval($app_vtest_list->RecCnt) >= intval($app_vtest_list->StartRec)) {
		$app_vtest_list->RowCnt++;

		// Set up key count
		$app_vtest_list->KeyCount = $app_vtest_list->RowIndex;

		// Init row class and style
		$app_vtest->ResetAttrs();
		$app_vtest->CssClass = "";
		if ($app_vtest->CurrentAction == "gridadd") {
		} else {
			$app_vtest_list->LoadRowValues($app_vtest_list->Recordset); // Load row values
		}
		$app_vtest->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$app_vtest->RowAttrs = array_merge($app_vtest->RowAttrs, array('data-rowindex'=>$app_vtest_list->RowCnt, 'id'=>'r' . $app_vtest_list->RowCnt . '_app_vtest', 'data-rowtype'=>$app_vtest->RowType));
		...
		$app_vtest_list->RenderListOptions();
?>
	<tr<?php echo $app_vtest->RowAttributes() ?>>
<?php
...
<?php echo $app_vtest->lang_id->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($app_vtest->test_num->Visible) { // test_num ?>
		<td data-name="test_num"<?php echo $app_vtest->test_num->CellAttributes() ?>>
...
<?php echo $app_vtest->test_name->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($app_vtest->test_iname->Visible) { // test_iname ?>
		<td data-name="test_iname"<?php echo $app_vtest->test_iname->CellAttributes() ?>>
<span id="el<?php echo $app_vtest_list->RowCnt ?>_app_vtest_test_iname" class="app_vtest_test_iname">
<span<?php echo $app_vtest->test_iname->ViewAttributes() ?>>
<?php echo $app_vtest->test_iname->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($app_vtest->status_id->Visible) { // status_id ?>
		<td data-name="status_id"<?php echo $app_vtest->status_id->CellAttributes() ?>>
<span id="el<?php echo $app_vtest_list->RowCnt ?>_app_vtest_status_id" class="app_vtest_status_id">
<span<?php echo $app_vtest->status_id->ViewAttributes() ?>>
<?php echo $app_vtest->status_id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($app_vtest->test_image->Visible) { // test_image ?>
...
<span<?php echo $app_vtest->test_price->ViewAttributes() ?>>
<?php echo $app_vtest->test_price->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($app_vtest->test_desc->Visible) { // test_desc ?>
...
$app_vtest_list->ListOptions->Render("body", "right", $app_vtest_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($app_vtest->CurrentAction <> "gridadd")
		$app_vtest_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($app_vtest->CurrentAction == "") { ?>
...
// Close recordset
if ($app_vtest_list->Recordset)
	$app_vtest_list->Recordset->Close();
?>
<?php if ($app_vtest->Export == "") { ?>
...
<?php } ?>
<?php if ($app_vtest_list->TotalRecs == 0 && $app_vtest->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($app_vtest_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($app_vtest->Export == "") { ?>
<script type="text/javascript">
fapp_vtestlistsrch.FilterList = <?php echo $app_vtest_list->GetFilterList() ?>;
fapp_vtestlistsrch.Init();
fapp_vtestlist.Init();
</script>
<?php } ?>
<?php
$app_vtest_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($app_vtest->Export == "") { ?>
