...
$cpy_page_row_list = NULL; // Initialize page object first

class ccpy_page_row_list extends ccpy_page_row {
	...
	// Page object name
	var $PageObjName = 'cpy_page_row_list';

	// Grid form hidden field names
	var $FormName = 'fcpy_page_rowlist';
	...
	function __construct() {
		...
		$this->AddUrl = "cpy_page_rowadd.php?" . EW_TABLE_SHOW_DETAIL . "=";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "cpy_page_rowdelete.php";
		$this->MultiUpdateUrl = "cpy_page_rowupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fcpy_page_rowlistsrch";
	...
	function ClearInlineMode() {
		$this->setKey("row_id", ""); // Clear inline edit key
	...
	function InlineEditMode() {
		...
		if ($bInlineEdit) {
			if ($this->LoadRow()) {
				$this->setKey("row_id", $this->row_id->CurrentValue); // Set up inline edit key
	...
	function CheckInlineEditKey() {
		...
		if (strval($this->getKey("row_id")) <> strval($this->row_id->CurrentValue))
			return FALSE;
		return TRUE;
	}
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_slid_id") && $objForm->HasValue("o_slid_id") && $this->slid_id->CurrentValue <> $this->slid_id->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_row_order") && $objForm->HasValue("o_row_order") && $this->row_order->CurrentValue <> $this->row_order->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_row_name") && $objForm->HasValue("o_row_name") && $this->row_name->CurrentValue <> $this->row_name->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_row_stext") && $objForm->HasValue("o_row_stext") && $this->row_stext->CurrentValue <> $this->row_stext->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function GetFilterList() {
		...
		$sFilterList = ew_Concat($sFilterList, $this->row_order->AdvancedSearch->ToJson(), ","); // Field row_order
		$sFilterList = ew_Concat($sFilterList, $this->row_name->AdvancedSearch->ToJson(), ","); // Field row_name
		$sFilterList = ew_Concat($sFilterList, $this->row_stext->AdvancedSearch->ToJson(), ","); // Field row_stext
		if ($this->BasicSearch->Keyword <> "") {
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fcpy_page_rowlistsrch", $filters);
	...
	function RestoreFilterList() {
		...
		// Field row_order
		$this->row_order->AdvancedSearch->SearchValue = @$filter["x_row_order"];
		$this->row_order->AdvancedSearch->SearchOperator = @$filter["z_row_order"];
		$this->row_order->AdvancedSearch->SearchCondition = @$filter["v_row_order"];
		$this->row_order->AdvancedSearch->SearchValue2 = @$filter["y_row_order"];
		$this->row_order->AdvancedSearch->SearchOperator2 = @$filter["w_row_order"];
		$this->row_order->AdvancedSearch->Save();

		// Field row_name
		$this->row_name->AdvancedSearch->SearchValue = @$filter["x_row_name"];
		$this->row_name->AdvancedSearch->SearchOperator = @$filter["z_row_name"];
		$this->row_name->AdvancedSearch->SearchCondition = @$filter["v_row_name"];
		$this->row_name->AdvancedSearch->SearchValue2 = @$filter["y_row_name"];
		$this->row_name->AdvancedSearch->SearchOperator2 = @$filter["w_row_name"];
		$this->row_name->AdvancedSearch->Save();

		// Field row_stext
		$this->row_stext->AdvancedSearch->SearchValue = @$filter["x_row_stext"];
		$this->row_stext->AdvancedSearch->SearchOperator = @$filter["z_row_stext"];
		$this->row_stext->AdvancedSearch->SearchCondition = @$filter["v_row_stext"];
		$this->row_stext->AdvancedSearch->SearchValue2 = @$filter["y_row_stext"];
		$this->row_stext->AdvancedSearch->SearchOperator2 = @$filter["w_row_stext"];
		$this->row_stext->AdvancedSearch->Save();
	...
	function AdvancedSearchWhere($Default = FALSE) {
		...
		$this->BuildSearchSql($sWhere, $this->row_order, $Default, FALSE); // row_order
		$this->BuildSearchSql($sWhere, $this->row_name, $Default, FALSE); // row_name
		$this->BuildSearchSql($sWhere, $this->row_stext, $Default, FALSE); // row_stext
		...
		if (!$Default && $this->Command == "search") {
			...
			$this->row_order->AdvancedSearch->Save(); // row_order
			$this->row_name->AdvancedSearch->Save(); // row_name
			$this->row_stext->AdvancedSearch->Save(); // row_stext
		}
		return $sWhere;
	}
	...
	function BasicSearchSQL($arKeywords, $type) {
		$sWhere = "";
		$this->BuildBasicSearchSQL($sWhere, $this->row_name, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->row_stext, $arKeywords, $type);
		return $sWhere;
	}
	...
	function CheckSearchParms() {
		...
		if ($this->slid_id->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->row_order->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->row_name->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->row_stext->AdvancedSearch->IssetSession())
			return TRUE;
		return FALSE;
	}
	...
	function ResetAdvancedSearchParms() {
		...
		$this->row_order->AdvancedSearch->UnsetSession();
		$this->row_name->AdvancedSearch->UnsetSession();
		$this->row_stext->AdvancedSearch->UnsetSession();
	}
	...
	function SetupSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->row_order); // row_order
			$this->UpdateSort($this->row_name); // row_name
			$this->UpdateSort($this->row_stext); // row_stext
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetsort") {
				...
				$this->row_order->setSort("");
				$this->row_name->setSort("");
				$this->row_stext->setSort("");
			}
	...
	function SetupListOptions() {
		...
		// "detail_cpy_page_row_block"
		$item = &$this->ListOptions->Add("detail_cpy_page_row_block");
		$item->CssClass = "text-nowrap";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'cpy_page_row_block') && !$this->ShowMultipleDetails;
		$item->OnLeft = TRUE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentAction == "edit" && $this->RowType == EW_ROWTYPE_EDIT) { // Inline-Edit
			...
			$oListOpt->Body .= "<input type=\"hidden\" name=\"k" . $this->RowIndex . "_key\" id=\"k" . $this->RowIndex . "_key\" value=\"" . ew_HtmlEncode($this->row_id->CurrentValue) . "\">";
			return;
		}
		...
		// "detail_cpy_page_row_block"
		$oListOpt = &$this->ListOptions->Items["detail_cpy_page_row_block"];
		if ($Security->AllowList(CurrentProjectID() . 'cpy_page_row_block')) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("cpy_page_row_block", "TblCaption");
			$body .= "&nbsp;" . str_replace("%c", $this->cpy_page_row_block_Count, $Language->Phrase("DetailCount"));
			$body = "<a class=\"btn btn-default btn-sm ewRowLink ewDetail\" data-action=\"list\" href=\"" . ew_HtmlEncode("cpy_page_row_blocklist.php?" . EW_TABLE_SHOW_MASTER . "=cpy_page_row&fk_row_id=" . urlencode(strval($this->row_id->CurrentValue)) . "") . "\">" . $body . "</a>";
			$links = "";
			if ($GLOBALS["cpy_page_row_block_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'cpy_page_row_block')) {
				$caption = $Language->Phrase("MasterDetailViewLink");
				$url = $this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=cpy_page_row_block");
				...
				$DetailViewTblVar .= "cpy_page_row_block";
			}
			if ($GLOBALS["cpy_page_row_block_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'cpy_page_row_block')) {
				$caption = $Language->Phrase("MasterDetailEditLink");
				$url = $this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=cpy_page_row_block");
				...
				$DetailEditTblVar .= "cpy_page_row_block";
			}
			if ($GLOBALS["cpy_page_row_block_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'cpy_page_row_block')) {
				$caption = $Language->Phrase("MasterDetailCopyLink");
				$url = $this->GetCopyUrl(EW_TABLE_SHOW_DETAIL . "=cpy_page_row_block");
				...
				$DetailCopyTblVar .= "cpy_page_row_block";
			}
			if ($links <> "") {
	...
	function SetupOtherOptions() {
		...
		$DetailTableLink = "";
		$item = &$option->Add("detailadd_cpy_page_row_block");
		$url = $this->GetAddUrl(EW_TABLE_SHOW_DETAIL . "=cpy_page_row_block");
		$caption = $Language->Phrase("Add") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["cpy_page_row_block"]->TableCaption();
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" title=\"" . ew_HtmlTitle($caption) . "\" data-caption=\"" . ew_HtmlTitle($caption) . "\" href=\"" . ew_HtmlEncode($url) . "\">" . $caption . "</a>";
		$item->Visible = ($GLOBALS["cpy_page_row_block"]->DetailAdd && $Security->AllowAdd(CurrentProjectID() . 'cpy_page_row_block') && $Security->CanAdd());
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "cpy_page_row_block";
		}
		...
		$item->Body = "<a class=\"ewAction ewMultiDelete\" title=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" href=\"\" onclick=\"ew_SubmitAction(event,{f:document.fcpy_page_rowlist,url:'" . $this->MultiDeleteUrl . "'});return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = ($Security->CanDelete());

		// Add multi update
		$item = &$option->Add("multiupdate");
		$item->Body = "<a class=\"ewAction ewMultiUpdate\" title=\"" . ew_HtmlTitle($Language->Phrase("UpdateSelectedLink")) . "\" data-table=\"cpy_page_row\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("UpdateSelectedLink")) . "\" href=\"\" onclick=\"ew_SubmitAction(event,{f:document.fcpy_page_rowlist,url:'" . $this->MultiUpdateUrl . "'});return false;\">" . $Language->Phrase("UpdateSelectedLink") . "</a>";
		$item->Visible = ($Security->CanEdit());
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fcpy_page_rowlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fcpy_page_rowlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		if ($this->CurrentAction <> "gridadd" && $this->CurrentAction <> "gridedit") { // Not grid add/edit mode
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fcpy_page_rowlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fcpy_page_rowlistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"btn btn-default ewAdvancedSearch\" title=\"" . $Language->Phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->Phrase("AdvancedSearch") . "\" href=\"cpy_page_rowsrch.php\">" . $Language->Phrase("AdvancedSearchBtn") . "</a>";
		$item->Visible = TRUE;

		// Search highlight button
		$item = &$this->SearchOptions->Add("searchhighlight");
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewHighlight active\" title=\"" . $Language->Phrase("Highlight") . "\" data-caption=\"" . $Language->Phrase("Highlight") . "\" data-toggle=\"button\" data-form=\"fcpy_page_rowlistsrch\" data-name=\"" . $this->HighlightName() . "\">" . $Language->Phrase("HighlightBtn") . "</button>";
	...
	function LoadDefaultValues() {
		...
		$this->row_order->OldValue = $this->row_order->CurrentValue;
	...
	function LoadSearchValues() {
		...
		// row_order
		$this->row_order->AdvancedSearch->SearchValue = @$_GET["x_row_order"];
		if ($this->row_order->AdvancedSearch->SearchValue <> "" && $this->Command == "") $this->Command = "search";
		$this->row_order->AdvancedSearch->SearchOperator = @$_GET["z_row_order"];

		// row_name
		$this->row_name->AdvancedSearch->SearchValue = @$_GET["x_row_name"];
		if ($this->row_name->AdvancedSearch->SearchValue <> "" && $this->Command == "") $this->Command = "search";
		$this->row_name->AdvancedSearch->SearchOperator = @$_GET["z_row_name"];

		// row_stext
		$this->row_stext->AdvancedSearch->SearchValue = @$_GET["x_row_stext"];
		if ($this->row_stext->AdvancedSearch->SearchValue <> "" && $this->Command == "") $this->Command = "search";
		$this->row_stext->AdvancedSearch->SearchOperator = @$_GET["z_row_stext"];
	}
	...
	function LoadRowValues($rs = NULL) {
		...
		if (!isset($GLOBALS["cpy_page_row_block_grid"])) $GLOBALS["cpy_page_row_block_grid"] = new ccpy_page_row_block_grid;
		$sDetailFilter = $GLOBALS["cpy_page_row_block"]->SqlDetailFilter_cpy_page_row();
		$sDetailFilter = str_replace("@row_id@", ew_AdjustSql($this->row_id->DbValue, "DB"), $sDetailFilter);
		$GLOBALS["cpy_page_row_block"]->setCurrentMasterTable("cpy_page_row");
		$sDetailFilter = $GLOBALS["cpy_page_row_block"]->ApplyUserIDFilters($sDetailFilter);
		$this->cpy_page_row_block_Count = $GLOBALS["cpy_page_row_block"]->LoadRecordCount($sDetailFilter);
	}
	...
	function RenderRow() {
		...
		$this->row_stext->ViewCustomAttributes = "";
			...
			if ($this->Export == "")
				$this->row_order->ViewValue = $this->HighlightValue($this->row_order);
			...
			if ($this->Export == "")
				$this->row_name->ViewValue = $this->HighlightValue($this->row_name);
			...
			if ($this->Export == "")
				$this->row_stext->ViewValue = $this->HighlightValue($this->row_stext);
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_cpy_page_row\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_cpy_page_row',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fcpy_page_rowlist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
	...
	function ExportData() {
		...
		if (EW_EXPORT_MASTER_RECORD && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "cpy_page") {
			global $cpy_page;
			if (!isset($cpy_page)) $cpy_page = new ccpy_page;
			$rsmaster = $cpy_page->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("v"); // Change to vertical
				if ($this->Export <> "csv" || EW_EXPORT_MASTER_RECORD_FOR_CSV) {
					$Doc->Table = &$cpy_page;
					$cpy_page->ExportDocument($Doc, $rsmaster, 1, 1);
					$Doc->ExportEmptyRow();
					$Doc->Table = &$this;
				}
...
// Create page object
if (!isset($cpy_page_row_list)) $cpy_page_row_list = new ccpy_page_row_list();

// Page init
$cpy_page_row_list->Page_Init();

// Page main
$cpy_page_row_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cpy_page_row_list->Page_Render();
?>
...
var CurrentForm = fcpy_page_rowlist = new ew_Form("fcpy_page_rowlist", "list");
fcpy_page_rowlist.FormKeyCountName = '<?php echo $cpy_page_row_list->FormKeyCountName ?>';

// Validate form
fcpy_page_rowlist.Validate = function() {
...
// Check empty row
fcpy_page_rowlist.EmptyRow = function(infix) {
	var fobj = this.Form;
...
fcpy_page_rowlist.Form_CustomValidate = 
...
fcpy_page_rowlist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fcpy_page_rowlist.Lists["x_page_id"] = {"LinkField":"x_page_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_page_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cpy_page"};
fcpy_page_rowlist.Lists["x_page_id"].Data = "<?php echo $cpy_page_row_list->page_id->LookupFilterQuery(FALSE, "list") ?>";
fcpy_page_rowlist.Lists["x_lang_id"] = {"LinkField":"x_lang_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_lang_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_language"};
fcpy_page_rowlist.Lists["x_lang_id"].Data = "<?php echo $cpy_page_row_list->lang_id->LookupFilterQuery(FALSE, "list") ?>";
fcpy_page_rowlist.Lists["x_status_id"] = {"LinkField":"x_status_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_status_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_status"};
fcpy_page_rowlist.Lists["x_status_id"].Data = "<?php echo $cpy_page_row_list->status_id->LookupFilterQuery(FALSE, "list") ?>";
fcpy_page_rowlist.Lists["x_color_id"] = {"LinkField":"x_color_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_color_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_color"};
fcpy_page_rowlist.Lists["x_color_id"].Data = "<?php echo $cpy_page_row_list->color_id->LookupFilterQuery(FALSE, "list") ?>";
fcpy_page_rowlist.Lists["x_slid_id"] = {"LinkField":"x_slid_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_slid_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cpy_slider_mst"};
fcpy_page_rowlist.Lists["x_slid_id"].Data = "<?php echo $cpy_page_row_list->slid_id->LookupFilterQuery(FALSE, "list") ?>";

// Form object for search
var CurrentSearchForm = fcpy_page_rowlistsrch = new ew_Form("fcpy_page_rowlistsrch");
</script>
<script type="text/javascript">
...
<?php if ($cpy_page_row->Export == "") { ?>
<div class="ewToolbar">
<?php if ($cpy_page_row_list->TotalRecs > 0 && $cpy_page_row_list->ExportOptions->Visible()) { ?>
<?php $cpy_page_row_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($cpy_page_row_list->SearchOptions->Visible()) { ?>
<?php $cpy_page_row_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($cpy_page_row_list->FilterOptions->Visible()) { ?>
<?php $cpy_page_row_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php if (($cpy_page_row->Export == "") || (EW_EXPORT_MASTER_RECORD && $cpy_page_row->Export == "print")) { ?>
<?php
if ($cpy_page_row_list->DbMasterFilter <> "" && $cpy_page_row->getCurrentMasterTable() == "cpy_page") {
	if ($cpy_page_row_list->MasterRecordExists) {
?>
...
if ($cpy_page_row->CurrentAction == "gridadd") {
	...
	$cpy_page_row_list->StartRec = 1;
	$cpy_page_row_list->DisplayRecs = $cpy_page_row->GridAddRowCount;
	$cpy_page_row_list->TotalRecs = $cpy_page_row_list->DisplayRecs;
	$cpy_page_row_list->StopRec = $cpy_page_row_list->DisplayRecs;
} else {
	$bSelectLimit = $cpy_page_row_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($cpy_page_row_list->TotalRecs <= 0)
			$cpy_page_row_list->TotalRecs = $cpy_page_row->ListRecordCount();
	} else {
		if (!$cpy_page_row_list->Recordset && ($cpy_page_row_list->Recordset = $cpy_page_row_list->LoadRecordset()))
			$cpy_page_row_list->TotalRecs = $cpy_page_row_list->Recordset->RecordCount();
	}
	$cpy_page_row_list->StartRec = 1;
	if ($cpy_page_row_list->DisplayRecs <= 0 || ($cpy_page_row->Export <> "" && $cpy_page_row->ExportAll)) // Display all records
		$cpy_page_row_list->DisplayRecs = $cpy_page_row_list->TotalRecs;
	if (!($cpy_page_row->Export <> "" && $cpy_page_row->ExportAll))
		$cpy_page_row_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$cpy_page_row_list->Recordset = $cpy_page_row_list->LoadRecordset($cpy_page_row_list->StartRec-1, $cpy_page_row_list->DisplayRecs);

	// Set no record found message
	if ($cpy_page_row->CurrentAction == "" && $cpy_page_row_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$cpy_page_row_list->setWarningMessage(ew_DeniedMsg());
		if ($cpy_page_row_list->SearchWhere == "0=101")
			$cpy_page_row_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$cpy_page_row_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
}
$cpy_page_row_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($cpy_page_row->Export == "" && $cpy_page_row->CurrentAction == "") { ?>
<form name="fcpy_page_rowlistsrch" id="fcpy_page_rowlistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($cpy_page_row_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fcpy_page_rowlistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="cpy_page_row">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($cpy_page_row_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($cpy_page_row_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $cpy_page_row_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($cpy_page_row_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($cpy_page_row_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($cpy_page_row_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($cpy_page_row_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $cpy_page_row_list->ShowPageHeader(); ?>
<?php
$cpy_page_row_list->ShowMessage();
?>
<?php if ($cpy_page_row_list->TotalRecs > 0 || $cpy_page_row->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($cpy_page_row_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> cpy_page_row">
<?php if ($cpy_page_row->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($cpy_page_row->CurrentAction <> "gridadd" && $cpy_page_row->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($cpy_page_row_list->Pager)) $cpy_page_row_list->Pager = new cPrevNextPager($cpy_page_row_list->StartRec, $cpy_page_row_list->DisplayRecs, $cpy_page_row_list->TotalRecs, $cpy_page_row_list->AutoHidePager) ?>
<?php if ($cpy_page_row_list->Pager->RecordCount > 0 && $cpy_page_row_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($cpy_page_row_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $cpy_page_row_list->PageUrl() ?>start=<?php echo $cpy_page_row_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($cpy_page_row_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $cpy_page_row_list->PageUrl() ?>start=<?php echo $cpy_page_row_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $cpy_page_row_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($cpy_page_row_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $cpy_page_row_list->PageUrl() ?>start=<?php echo $cpy_page_row_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($cpy_page_row_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $cpy_page_row_list->PageUrl() ?>start=<?php echo $cpy_page_row_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $cpy_page_row_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($cpy_page_row_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $cpy_page_row_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $cpy_page_row_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $cpy_page_row_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($cpy_page_row_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="fcpy_page_rowlist" id="fcpy_page_rowlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cpy_page_row_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cpy_page_row_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="cpy_page_row">
<?php if ($cpy_page_row->getCurrentMasterTable() == "cpy_page" && $cpy_page_row->CurrentAction <> "") { ?>
...
<?php if ($cpy_page_row_list->TotalRecs > 0 || $cpy_page_row->CurrentAction == "add" || $cpy_page_row->CurrentAction == "copy" || $cpy_page_row->CurrentAction == "gridedit") { ?>
<table id="tbl_cpy_page_rowlist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$cpy_page_row_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$cpy_page_row_list->RenderListOptions();

// Render list options (header, left)
$cpy_page_row_list->ListOptions->Render("header", "left");
?>
<?php if ($cpy_page_row->page_id->Visible) { // page_id ?>
	...
	<?php } else { ?>
		<th data-name="page_id" class="<?php echo $cpy_page_row->page_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $cpy_page_row->SortUrl($cpy_page_row->page_id) ?>',1);"><div id="elh_cpy_page_row_page_id" class="cpy_page_row_page_id">
...
<?php if ($cpy_page_row->lang_id->Visible) { // lang_id ?>
	...
	<?php } else { ?>
		<th data-name="lang_id" class="<?php echo $cpy_page_row->lang_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $cpy_page_row->SortUrl($cpy_page_row->lang_id) ?>',1);"><div id="elh_cpy_page_row_lang_id" class="cpy_page_row_lang_id">
...
<?php if ($cpy_page_row->status_id->Visible) { // status_id ?>
	...
	<?php } else { ?>
		<th data-name="status_id" class="<?php echo $cpy_page_row->status_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $cpy_page_row->SortUrl($cpy_page_row->status_id) ?>',1);"><div id="elh_cpy_page_row_status_id" class="cpy_page_row_status_id">
...
<?php if ($cpy_page_row->color_id->Visible) { // color_id ?>
	...
	<?php } else { ?>
		<th data-name="color_id" class="<?php echo $cpy_page_row->color_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $cpy_page_row->SortUrl($cpy_page_row->color_id) ?>',1);"><div id="elh_cpy_page_row_color_id" class="cpy_page_row_color_id">
...
<?php if ($cpy_page_row->slid_id->Visible) { // slid_id ?>
	...
	<?php } else { ?>
		<th data-name="slid_id" class="<?php echo $cpy_page_row->slid_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $cpy_page_row->SortUrl($cpy_page_row->slid_id) ?>',1);"><div id="elh_cpy_page_row_slid_id" class="cpy_page_row_slid_id">
...
<?php if ($cpy_page_row->row_order->Visible) { // row_order ?>
	...
	<?php } else { ?>
		<th data-name="row_order" class="<?php echo $cpy_page_row->row_order->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $cpy_page_row->SortUrl($cpy_page_row->row_order) ?>',1);"><div id="elh_cpy_page_row_row_order" class="cpy_page_row_row_order">
...
<?php if ($cpy_page_row->row_name->Visible) { // row_name ?>
	...
	<?php } else { ?>
		<th data-name="row_name" class="<?php echo $cpy_page_row->row_name->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $cpy_page_row->SortUrl($cpy_page_row->row_name) ?>',1);"><div id="elh_cpy_page_row_row_name" class="cpy_page_row_row_name">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $cpy_page_row->row_name->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($cpy_page_row->row_name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($cpy_page_row->row_name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($cpy_page_row->row_stext->Visible) { // row_stext ?>
	...
	<?php } else { ?>
		<th data-name="row_stext" class="<?php echo $cpy_page_row->row_stext->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $cpy_page_row->SortUrl($cpy_page_row->row_stext) ?>',1);"><div id="elh_cpy_page_row_row_stext" class="cpy_page_row_row_stext">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $cpy_page_row->row_stext->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($cpy_page_row->row_stext->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($cpy_page_row->row_stext->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
$cpy_page_row_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
	if ($cpy_page_row->CurrentAction == "add" || $cpy_page_row->CurrentAction == "copy") {
		$cpy_page_row_list->RowIndex = 0;
		$cpy_page_row_list->KeyCount = $cpy_page_row_list->RowIndex;
		if ($cpy_page_row->CurrentAction == "copy" && !$cpy_page_row_list->LoadRow())
			$cpy_page_row->CurrentAction = "add";
		if ($cpy_page_row->CurrentAction == "add")
			$cpy_page_row_list->LoadRowValues();
		if ($cpy_page_row->EventCancelled) // Insert failed
			$cpy_page_row_list->RestoreFormValues(); // Restore form values

		// Set row properties
		$cpy_page_row->ResetAttrs();
		$cpy_page_row->RowAttrs = array_merge($cpy_page_row->RowAttrs, array('data-rowindex'=>0, 'id'=>'r0_cpy_page_row', 'data-rowtype'=>EW_ROWTYPE_ADD));
		$cpy_page_row->RowType = EW_ROWTYPE_ADD;

		// Render row
		$cpy_page_row_list->RenderRow();

		// Render list options
		$cpy_page_row_list->RenderListOptions();
		$cpy_page_row_list->StartRowCnt = 0;
?>
	<tr<?php echo $cpy_page_row->RowAttributes() ?>>
<?php

// Render list options (body, left)
$cpy_page_row_list->ListOptions->Render("body", "left", $cpy_page_row_list->RowCnt);
?>
	<?php if ($cpy_page_row->page_id->Visible) { // page_id ?>
		<td data-name="page_id">
<?php if ($cpy_page_row->page_id->getSessionValue() <> "") { ?>
<span id="el<?php echo $cpy_page_row_list->RowCnt ?>_cpy_page_row_page_id" class="form-group cpy_page_row_page_id">
...
<input type="hidden" id="x<?php echo $cpy_page_row_list->RowIndex ?>_page_id" name="x<?php echo $cpy_page_row_list->RowIndex ?>_page_id" value="<?php echo ew_HtmlEncode($cpy_page_row->page_id->CurrentValue) ?>">
<?php } else { ?>
<span id="el<?php echo $cpy_page_row_list->RowCnt ?>_cpy_page_row_page_id" class="form-group cpy_page_row_page_id">
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next(":not([disabled])").click();" tabindex="-1" class="form-control ewLookupText" id="lu_x<?php echo $cpy_page_row_list->RowIndex ?>_page_id"><?php echo (strval($cpy_page_row->page_id->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $cpy_page_row->page_id->ViewValue); ?></span>
</span>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($cpy_page_row->page_id->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x<?php echo $cpy_page_row_list->RowIndex ?>_page_id',m:0,n:10});" class="ewLookupBtn btn btn-default btn-sm"<?php echo (($cpy_page_row->page_id->ReadOnly || $cpy_page_row->page_id->Disabled) ? " disabled" : "")?>><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="cpy_page_row" data-field="x_page_id" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $cpy_page_row->page_id->DisplayValueSeparatorAttribute() ?>" name="x<?php echo $cpy_page_row_list->RowIndex ?>_page_id" id="x<?php echo $cpy_page_row_list->RowIndex ?>_page_id" value="<?php echo $cpy_page_row->page_id->CurrentValue ?>"<?php echo $cpy_page_row->page_id->EditAttributes() ?>>
</span>
<?php } ?>
<input type="hidden" data-table="cpy_page_row" data-field="x_page_id" name="o<?php echo $cpy_page_row_list->RowIndex ?>_page_id" id="o<?php echo $cpy_page_row_list->RowIndex ?>_page_id" value="<?php echo ew_HtmlEncode($cpy_page_row->page_id->OldValue) ?>">
</td>
	<?php } ?>
	<?php if ($cpy_page_row->lang_id->Visible) { // lang_id ?>
		<td data-name="lang_id">
<span id="el<?php echo $cpy_page_row_list->RowCnt ?>_cpy_page_row_lang_id" class="form-group cpy_page_row_lang_id">
<select data-table="cpy_page_row" data-field="x_lang_id" data-value-separator="<?php echo $cpy_page_row->lang_id->DisplayValueSeparatorAttribute() ?>" id="x<?php echo $cpy_page_row_list->RowIndex ?>_lang_id" name="x<?php echo $cpy_page_row_list->RowIndex ?>_lang_id"<?php echo $cpy_page_row->lang_id->EditAttributes() ?>>
<?php echo $cpy_page_row->lang_id->SelectOptionListHtml("x<?php echo $cpy_page_row_list->RowIndex ?>_lang_id") ?>
</select>
</span>
<input type="hidden" data-table="cpy_page_row" data-field="x_lang_id" name="o<?php echo $cpy_page_row_list->RowIndex ?>_lang_id" id="o<?php echo $cpy_page_row_list->RowIndex ?>_lang_id" value="<?php echo ew_HtmlEncode($cpy_page_row->lang_id->OldValue) ?>">
</td>
	<?php } ?>
	<?php if ($cpy_page_row->status_id->Visible) { // status_id ?>
		<td data-name="status_id">
<span id="el<?php echo $cpy_page_row_list->RowCnt ?>_cpy_page_row_status_id" class="form-group cpy_page_row_status_id">
<select data-table="cpy_page_row" data-field="x_status_id" data-value-separator="<?php echo $cpy_page_row->status_id->DisplayValueSeparatorAttribute() ?>" id="x<?php echo $cpy_page_row_list->RowIndex ?>_status_id" name="x<?php echo $cpy_page_row_list->RowIndex ?>_status_id"<?php echo $cpy_page_row->status_id->EditAttributes() ?>>
<?php echo $cpy_page_row->status_id->SelectOptionListHtml("x<?php echo $cpy_page_row_list->RowIndex ?>_status_id") ?>
</select>
</span>
<input type="hidden" data-table="cpy_page_row" data-field="x_status_id" name="o<?php echo $cpy_page_row_list->RowIndex ?>_status_id" id="o<?php echo $cpy_page_row_list->RowIndex ?>_status_id" value="<?php echo ew_HtmlEncode($cpy_page_row->status_id->OldValue) ?>">
</td>
	<?php } ?>
	<?php if ($cpy_page_row->color_id->Visible) { // color_id ?>
		<td data-name="color_id">
<span id="el<?php echo $cpy_page_row_list->RowCnt ?>_cpy_page_row_color_id" class="form-group cpy_page_row_color_id">
<select data-table="cpy_page_row" data-field="x_color_id" data-value-separator="<?php echo $cpy_page_row->color_id->DisplayValueSeparatorAttribute() ?>" id="x<?php echo $cpy_page_row_list->RowIndex ?>_color_id" name="x<?php echo $cpy_page_row_list->RowIndex ?>_color_id"<?php echo $cpy_page_row->color_id->EditAttributes() ?>>
<?php echo $cpy_page_row->color_id->SelectOptionListHtml("x<?php echo $cpy_page_row_list->RowIndex ?>_color_id") ?>
</select>
</span>
<input type="hidden" data-table="cpy_page_row" data-field="x_color_id" name="o<?php echo $cpy_page_row_list->RowIndex ?>_color_id" id="o<?php echo $cpy_page_row_list->RowIndex ?>_color_id" value="<?php echo ew_HtmlEncode($cpy_page_row->color_id->OldValue) ?>">
</td>
	<?php } ?>
	<?php if ($cpy_page_row->slid_id->Visible) { // slid_id ?>
		<td data-name="slid_id">
<span id="el<?php echo $cpy_page_row_list->RowCnt ?>_cpy_page_row_slid_id" class="form-group cpy_page_row_slid_id">
<select data-table="cpy_page_row" data-field="x_slid_id" data-value-separator="<?php echo $cpy_page_row->slid_id->DisplayValueSeparatorAttribute() ?>" id="x<?php echo $cpy_page_row_list->RowIndex ?>_slid_id" name="x<?php echo $cpy_page_row_list->RowIndex ?>_slid_id"<?php echo $cpy_page_row->slid_id->EditAttributes() ?>>
<?php echo $cpy_page_row->slid_id->SelectOptionListHtml("x<?php echo $cpy_page_row_list->RowIndex ?>_slid_id") ?>
</select>
</span>
<input type="hidden" data-table="cpy_page_row" data-field="x_slid_id" name="o<?php echo $cpy_page_row_list->RowIndex ?>_slid_id" id="o<?php echo $cpy_page_row_list->RowIndex ?>_slid_id" value="<?php echo ew_HtmlEncode($cpy_page_row->slid_id->OldValue) ?>">
</td>
	<?php } ?>
	<?php if ($cpy_page_row->row_order->Visible) { // row_order ?>
		<td data-name="row_order">
<span id="el<?php echo $cpy_page_row_list->RowCnt ?>_cpy_page_row_row_order" class="form-group cpy_page_row_row_order">
<input type="text" data-table="cpy_page_row" data-field="x_row_order" name="x<?php echo $cpy_page_row_list->RowIndex ?>_row_order" id="x<?php echo $cpy_page_row_list->RowIndex ?>_row_order" size="30" placeholder="<?php echo ew_HtmlEncode($cpy_page_row->row_order->getPlaceHolder()) ?>" value="<?php echo $cpy_page_row->row_order->EditValue ?>"<?php echo $cpy_page_row->row_order->EditAttributes() ?>>
</span>
<input type="hidden" data-table="cpy_page_row" data-field="x_row_order" name="o<?php echo $cpy_page_row_list->RowIndex ?>_row_order" id="o<?php echo $cpy_page_row_list->RowIndex ?>_row_order" value="<?php echo ew_HtmlEncode($cpy_page_row->row_order->OldValue) ?>">
</td>
	<?php } ?>
	<?php if ($cpy_page_row->row_name->Visible) { // row_name ?>
		<td data-name="row_name">
<span id="el<?php echo $cpy_page_row_list->RowCnt ?>_cpy_page_row_row_name" class="form-group cpy_page_row_row_name">
<input type="text" data-table="cpy_page_row" data-field="x_row_name" name="x<?php echo $cpy_page_row_list->RowIndex ?>_row_name" id="x<?php echo $cpy_page_row_list->RowIndex ?>_row_name" size="30" maxlength="200" placeholder="<?php echo ew_HtmlEncode($cpy_page_row->row_name->getPlaceHolder()) ?>" value="<?php echo $cpy_page_row->row_name->EditValue ?>"<?php echo $cpy_page_row->row_name->EditAttributes() ?>>
</span>
<input type="hidden" data-table="cpy_page_row" data-field="x_row_name" name="o<?php echo $cpy_page_row_list->RowIndex ?>_row_name" id="o<?php echo $cpy_page_row_list->RowIndex ?>_row_name" value="<?php echo ew_HtmlEncode($cpy_page_row->row_name->OldValue) ?>">
</td>
	<?php } ?>
	<?php if ($cpy_page_row->row_stext->Visible) { // row_stext ?>
		<td data-name="row_stext">
<span id="el<?php echo $cpy_page_row_list->RowCnt ?>_cpy_page_row_row_stext" class="form-group cpy_page_row_row_stext">
<textarea data-table="cpy_page_row" data-field="x_row_stext" name="x<?php echo $cpy_page_row_list->RowIndex ?>_row_stext" id="x<?php echo $cpy_page_row_list->RowIndex ?>_row_stext" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($cpy_page_row->row_stext->getPlaceHolder()) ?>"<?php echo $cpy_page_row->row_stext->EditAttributes() ?>><?php echo $cpy_page_row->row_stext->EditValue ?></textarea>
</span>
<input type="hidden" data-table="cpy_page_row" data-field="x_row_stext" name="o<?php echo $cpy_page_row_list->RowIndex ?>_row_stext" id="o<?php echo $cpy_page_row_list->RowIndex ?>_row_stext" value="<?php echo ew_HtmlEncode($cpy_page_row->row_stext->OldValue) ?>">
</td>
	<?php } ?>
<?php

// Render list options (body, right)
$cpy_page_row_list->ListOptions->Render("body", "right", $cpy_page_row_list->RowCnt);
?>
<script type="text/javascript">
fcpy_page_rowlist.UpdateOpts(<?php echo $cpy_page_row_list->RowIndex ?>);
</script>
	</tr>
<?php
}
?>
<?php
if ($cpy_page_row->ExportAll && $cpy_page_row->Export <> "") {
	$cpy_page_row_list->StopRec = $cpy_page_row_list->TotalRecs;
} else {

	// Set the last record to display
	if ($cpy_page_row_list->TotalRecs > $cpy_page_row_list->StartRec + $cpy_page_row_list->DisplayRecs - 1)
		$cpy_page_row_list->StopRec = $cpy_page_row_list->StartRec + $cpy_page_row_list->DisplayRecs - 1;
	else
		$cpy_page_row_list->StopRec = $cpy_page_row_list->TotalRecs;
}
...
if ($objForm) {
	$objForm->Index = -1;
	if ($objForm->HasValue($cpy_page_row_list->FormKeyCountName) && ($cpy_page_row->CurrentAction == "gridadd" || $cpy_page_row->CurrentAction == "gridedit" || $cpy_page_row->CurrentAction == "F")) {
		$cpy_page_row_list->KeyCount = $objForm->GetValue($cpy_page_row_list->FormKeyCountName);
		$cpy_page_row_list->StopRec = $cpy_page_row_list->StartRec + $cpy_page_row_list->KeyCount - 1;
	}
}
$cpy_page_row_list->RecCnt = $cpy_page_row_list->StartRec - 1;
if ($cpy_page_row_list->Recordset && !$cpy_page_row_list->Recordset->EOF) {
	$cpy_page_row_list->Recordset->MoveFirst();
	$bSelectLimit = $cpy_page_row_list->UseSelectLimit;
	if (!$bSelectLimit && $cpy_page_row_list->StartRec > 1)
		$cpy_page_row_list->Recordset->Move($cpy_page_row_list->StartRec - 1);
} elseif (!$cpy_page_row->AllowAddDeleteRow && $cpy_page_row_list->StopRec == 0) {
	$cpy_page_row_list->StopRec = $cpy_page_row->GridAddRowCount;
}
...
$cpy_page_row->ResetAttrs();
$cpy_page_row_list->RenderRow();
$cpy_page_row_list->EditRowCnt = 0;
if ($cpy_page_row->CurrentAction == "edit")
	$cpy_page_row_list->RowIndex = 1;
if ($cpy_page_row->CurrentAction == "gridadd")
	$cpy_page_row_list->RowIndex = 0;
if ($cpy_page_row->CurrentAction == "gridedit")
	$cpy_page_row_list->RowIndex = 0;
while ($cpy_page_row_list->RecCnt < $cpy_page_row_list->StopRec) {
	$cpy_page_row_list->RecCnt++;
	if (intval($cpy_page_row_list->RecCnt) >= intval($cpy_page_row_list->StartRec)) {
		$cpy_page_row_list->RowCnt++;
		if ($cpy_page_row->CurrentAction == "gridadd" || $cpy_page_row->CurrentAction == "gridedit" || $cpy_page_row->CurrentAction == "F") {
			$cpy_page_row_list->RowIndex++;
			$objForm->Index = $cpy_page_row_list->RowIndex;
			if ($objForm->HasValue($cpy_page_row_list->FormActionName))
				$cpy_page_row_list->RowAction = strval($objForm->GetValue($cpy_page_row_list->FormActionName));
			elseif ($cpy_page_row->CurrentAction == "gridadd")
				$cpy_page_row_list->RowAction = "insert";
			else
				$cpy_page_row_list->RowAction = "";
		}
		...
		if ($cpy_page_row->CurrentAction == "gridadd") {
			$cpy_page_row_list->LoadRowValues(); // Load default values
		} else {
			$cpy_page_row_list->LoadRowValues($cpy_page_row_list->Recordset); // Load row values
		}
		...
		if ($cpy_page_row->CurrentAction == "gridadd" && $cpy_page_row->EventCancelled && !$objForm->HasValue("k_blankrow")) // Insert failed
			$cpy_page_row_list->RestoreCurrentRowFormValues($cpy_page_row_list->RowIndex); // Restore form values
		if ($cpy_page_row->CurrentAction == "edit") {
			if ($cpy_page_row_list->CheckInlineEditKey() && $cpy_page_row_list->EditRowCnt == 0) { // Inline edit
		...
		if ($cpy_page_row->CurrentAction == "gridedit") { // Grid edit
			...
			if ($cpy_page_row_list->RowAction == "insert")
				$cpy_page_row->RowType = EW_ROWTYPE_ADD; // Render add
			else
				$cpy_page_row->RowType = EW_ROWTYPE_EDIT; // Render edit
		}
		if ($cpy_page_row->CurrentAction == "edit" && $cpy_page_row->RowType == EW_ROWTYPE_EDIT && $cpy_page_row->EventCancelled) { // Update failed
			$objForm->Index = 1;
			$cpy_page_row_list->RestoreFormValues(); // Restore form values
		}
		...
		if ($cpy_page_row->RowType == EW_ROWTYPE_EDIT) // Edit row
			$cpy_page_row_list->EditRowCnt++;

		// Set up row id / data-rowindex
		$cpy_page_row->RowAttrs = array_merge($cpy_page_row->RowAttrs, array('data-rowindex'=>$cpy_page_row_list->RowCnt, 'id'=>'r' . $cpy_page_row_list->RowCnt . '_cpy_page_row', 'data-rowtype'=>$cpy_page_row->RowType));
		...
		if ($cpy_page_row_list->RowAction <> "delete" && $cpy_page_row_list->RowAction <> "insertdelete" && !($cpy_page_row_list->RowAction == "insert" && $cpy_page_row->CurrentAction == "F" && $cpy_page_row_list->EmptyRow())) {
?>
	<tr<?php echo $cpy_page_row->RowAttributes() ?>>
<?php
...
<span id="el<?php echo $cpy_page_row_list->RowCnt ?>_cpy_page_row_page_id" class="cpy_page_row_page_id">
<span<?php echo $cpy_page_row->page_id->ViewAttributes() ?>>
...
<?php } ?>
	...
	<?php } ?>
<?php if ($cpy_page_row->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<input type="hidden" data-table="cpy_page_row" data-field="x_row_id" name="x<?php echo $cpy_page_row_list->RowIndex ?>_row_id" id="x<?php echo $cpy_page_row_list->RowIndex ?>_row_id" value="<?php echo ew_HtmlEncode($cpy_page_row->row_id->CurrentValue) ?>">
<input type="hidden" data-table="cpy_page_row" data-field="x_row_id" name="o<?php echo $cpy_page_row_list->RowIndex ?>_row_id" id="o<?php echo $cpy_page_row_list->RowIndex ?>_row_id" value="<?php echo ew_HtmlEncode($cpy_page_row->row_id->OldValue) ?>">
<?php } ?>
...
<?php } ?>
<?php if ($cpy_page_row->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $cpy_page_row_list->RowCnt ?>_cpy_page_row_lang_id" class="cpy_page_row_lang_id">
...
<?php } ?>
<?php if ($cpy_page_row->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $cpy_page_row_list->RowCnt ?>_cpy_page_row_status_id" class="cpy_page_row_status_id">
...
<?php } ?>
<?php if ($cpy_page_row->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $cpy_page_row_list->RowCnt ?>_cpy_page_row_color_id" class="cpy_page_row_color_id">
...
<?php } ?>
<?php if ($cpy_page_row->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $cpy_page_row_list->RowCnt ?>_cpy_page_row_slid_id" class="cpy_page_row_slid_id">
...
<?php } ?>
<?php if ($cpy_page_row->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $cpy_page_row_list->RowCnt ?>_cpy_page_row_row_order" class="cpy_page_row_row_order">
...
<?php } ?>
<?php if ($cpy_page_row->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $cpy_page_row_list->RowCnt ?>_cpy_page_row_row_name" class="cpy_page_row_row_name">
...
<?php } ?>
<?php if ($cpy_page_row->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $cpy_page_row_list->RowCnt ?>_cpy_page_row_row_stext" class="cpy_page_row_row_stext">
...
<?php
	...
	if ($cpy_page_row->CurrentAction <> "gridadd")
		if (!$cpy_page_row_list->Recordset->EOF) $cpy_page_row_list->Recordset->MoveNext();
}
?>
<?php
	if ($cpy_page_row->CurrentAction == "gridadd" || $cpy_page_row->CurrentAction == "gridedit") {
		$cpy_page_row_list->RowIndex = '$rowindex$';
		$cpy_page_row_list->LoadRowValues();

		// Set row properties
		$cpy_page_row->ResetAttrs();
		$cpy_page_row->RowAttrs = array_merge($cpy_page_row->RowAttrs, array('data-rowindex'=>$cpy_page_row_list->RowIndex, 'id'=>'r0_cpy_page_row', 'data-rowtype'=>EW_ROWTYPE_ADD));
...
// Render list options (body, left)
$cpy_page_row_list->ListOptions->Render("body", "left", $cpy_page_row_list->RowIndex);
?>
	<?php if ($cpy_page_row->page_id->Visible) { // page_id ?>
		<td data-name="page_id">
<?php if ($cpy_page_row->page_id->getSessionValue() <> "") { ?>
...
// Render list options (body, right)
$cpy_page_row_list->ListOptions->Render("body", "right", $cpy_page_row_list->RowIndex);
?>
<script type="text/javascript">
fcpy_page_rowlist.UpdateOpts(<?php echo $cpy_page_row_list->RowIndex ?>);
</script>
...
<?php } ?>
<?php if ($cpy_page_row->CurrentAction == "add" || $cpy_page_row->CurrentAction == "copy") { ?>
<input type="hidden" name="<?php echo $cpy_page_row_list->FormKeyCountName ?>" id="<?php echo $cpy_page_row_list->FormKeyCountName ?>" value="<?php echo $cpy_page_row_list->KeyCount ?>">
<?php } ?>
<?php if ($cpy_page_row->CurrentAction == "gridadd") { ?>
<input type="hidden" name="a_list" id="a_list" value="gridinsert">
<input type="hidden" name="<?php echo $cpy_page_row_list->FormKeyCountName ?>" id="<?php echo $cpy_page_row_list->FormKeyCountName ?>" value="<?php echo $cpy_page_row_list->KeyCount ?>">
<?php echo $cpy_page_row_list->MultiSelectKey ?>
<?php } ?>
<?php if ($cpy_page_row->CurrentAction == "edit") { ?>
<input type="hidden" name="<?php echo $cpy_page_row_list->FormKeyCountName ?>" id="<?php echo $cpy_page_row_list->FormKeyCountName ?>" value="<?php echo $cpy_page_row_list->KeyCount ?>">
<?php } ?>
<?php if ($cpy_page_row->CurrentAction == "gridedit") { ?>
<input type="hidden" name="a_list" id="a_list" value="gridupdate">
<input type="hidden" name="<?php echo $cpy_page_row_list->FormKeyCountName ?>" id="<?php echo $cpy_page_row_list->FormKeyCountName ?>" value="<?php echo $cpy_page_row_list->KeyCount ?>">
<?php echo $cpy_page_row_list->MultiSelectKey ?>
<?php } ?>
<?php if ($cpy_page_row->CurrentAction == "") { ?>
...
// Close recordset
if ($cpy_page_row_list->Recordset)
	$cpy_page_row_list->Recordset->Close();
?>
<?php if ($cpy_page_row->Export == "") { ?>
...
<?php } ?>
<?php if ($cpy_page_row_list->TotalRecs == 0 && $cpy_page_row->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($cpy_page_row_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($cpy_page_row->Export == "") { ?>
<script type="text/javascript">
fcpy_page_rowlistsrch.FilterList = <?php echo $cpy_page_row_list->GetFilterList() ?>;
fcpy_page_rowlistsrch.Init();
fcpy_page_rowlist.Init();
</script>
<?php } ?>
<?php
$cpy_page_row_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($cpy_page_row->Export == "") { ?>
...
<?php
$cpy_page_row_list->Page_Terminate();
?>
