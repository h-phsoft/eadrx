...
$phs_metta_update = NULL; // Initialize page object first

class cphs_metta_update extends cphs_metta {
	...
	// Page object name
	var $PageObjName = 'phs_metta_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("phs_mettalist.php"); // No records selected, return to list
	...
	function LoadMultiUpdateValues() {
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					$this->metta_type->setDbValue($this->Recordset->fields('metta_type'));
					$this->metta_name->setDbValue($this->Recordset->fields('metta_name'));
					$this->metta_value->setDbValue($this->Recordset->fields('metta_value'));
				} else {
					if (!ew_CompareValue($this->metta_type->DbValue, $this->Recordset->fields('metta_type')))
						$this->metta_type->CurrentValue = NULL;
					if (!ew_CompareValue($this->metta_name->DbValue, $this->Recordset->fields('metta_name')))
						$this->metta_name->CurrentValue = NULL;
					if (!ew_CompareValue($this->metta_value->DbValue, $this->Recordset->fields('metta_value')))
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->mtta_id->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function ValidateForm() {
		...
		$gsFormError = "";
		$lUpdateCnt = 0;
		if ($this->metta_type->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->metta_name->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->metta_value->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if ($this->metta_type->MultiUpdate <> "" && !$this->metta_type->FldIsDetailKey && !is_null($this->metta_type->FormValue) && $this->metta_type->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->metta_type->FldCaption(), $this->metta_type->ReqErrMsg));
		}
		if ($this->metta_name->MultiUpdate <> "" && !$this->metta_name->FldIsDetailKey && !is_null($this->metta_name->FormValue) && $this->metta_name->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->metta_name->FldCaption(), $this->metta_name->ReqErrMsg));
		}
		if ($this->metta_value->MultiUpdate <> "" && !$this->metta_value->FldIsDetailKey && !is_null($this->metta_value->FormValue) && $this->metta_value->FormValue == "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// metta_type
			$this->metta_type->SetDbValueDef($rsnew, $this->metta_type->CurrentValue, "", $this->metta_type->ReadOnly || $this->metta_type->MultiUpdate <> "1");

			// metta_name
			$this->metta_name->SetDbValueDef($rsnew, $this->metta_name->CurrentValue, "", $this->metta_name->ReadOnly || $this->metta_name->MultiUpdate <> "1");

			// metta_value
			$this->metta_value->SetDbValueDef($rsnew, $this->metta_value->CurrentValue, "", $this->metta_value->ReadOnly || $this->metta_value->MultiUpdate <> "1");
...
// Create page object
if (!isset($phs_metta_update)) $phs_metta_update = new cphs_metta_update();

// Page init
$phs_metta_update->Page_Init();

// Page main
$phs_metta_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$phs_metta_update->Page_Render();
?>
...
var CurrentForm = fphs_mettaupdate = new ew_Form("fphs_mettaupdate", "update");

// Validate form
fphs_mettaupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_metta_type");
			...
			uelm = this.GetElements("u" + infix + "_metta_name");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
			...
			uelm = this.GetElements("u" + infix + "_metta_value");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
...
fphs_mettaupdate.Form_CustomValidate = 
...
fphs_mettaupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fphs_mettaupdate.Lists["x_metta_type"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fphs_mettaupdate.Lists["x_metta_type"].Options = <?php echo json_encode($phs_metta_update->metta_type->Options()) ?>;
...
<?php $phs_metta_update->ShowPageHeader(); ?>
<?php
$phs_metta_update->ShowMessage();
?>
<form name="fphs_mettaupdate" id="fphs_mettaupdate" class="<?php echo $phs_metta_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($phs_metta_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $phs_metta_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="phs_metta">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($phs_metta_update->IsModal) ?>">
<?php foreach ($phs_metta_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_phs_mettaupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($phs_metta->metta_type->Visible) { // metta_type ?>
	<div id="r_metta_type" class="form-group">
		<label for="x_metta_type" class="<?php echo $phs_metta_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_metta_type" id="u_metta_type" class="ewMultiSelect" value="1"<?php echo ($phs_metta->metta_type->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_metta->metta_type->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_metta_update->RightColumnClass ?>"><div<?php echo $phs_metta->metta_type->CellAttributes() ?>>
...
<?php if ($phs_metta->metta_name->Visible) { // metta_name ?>
	<div id="r_metta_name" class="form-group">
		<label for="x_metta_name" class="<?php echo $phs_metta_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_metta_name" id="u_metta_name" class="ewMultiSelect" value="1"<?php echo ($phs_metta->metta_name->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_metta->metta_name->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_metta_update->RightColumnClass ?>"><div<?php echo $phs_metta->metta_name->CellAttributes() ?>>
...
<?php if ($phs_metta->metta_value->Visible) { // metta_value ?>
	<div id="r_metta_value" class="form-group">
		<label for="x_metta_value" class="<?php echo $phs_metta_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_metta_value" id="u_metta_value" class="ewMultiSelect" value="1"<?php echo ($phs_metta->metta_value->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $phs_metta->metta_value->FldCaption() ?></label></div></label>
		<div class="<?php echo $phs_metta_update->RightColumnClass ?>"><div<?php echo $phs_metta->metta_value->CellAttributes() ?>>
...
<?php if (!$phs_metta_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $phs_metta_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $phs_metta_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
	</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fphs_mettaupdate.Init();
</script>
<?php
$phs_metta_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$phs_metta_update->Page_Terminate();
?>
