...
//
// Table class for phs_bill_cycle
//
class cphs_bill_cycle extends cTable {
	var $cycle_id;
	var $cycle_name;
	var $cycle_interval;
	var $status_id;
	...
	function __construct() {
		...
		$this->TableVar = 'phs_bill_cycle';
		$this->TableName = 'phs_bill_cycle';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`phs_bill_cycle`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// cycle_id
		$this->cycle_id = new cField('phs_bill_cycle', 'phs_bill_cycle', 'x_cycle_id', 'cycle_id', '`cycle_id`', '`cycle_id`', 2, -1, FALSE, '`cycle_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// cycle_name
		$this->cycle_name = new cField('phs_bill_cycle', 'phs_bill_cycle', 'x_cycle_name', 'cycle_name', '`cycle_name`', '`cycle_name`', 200, -1, FALSE, '`cycle_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->cycle_name->Sortable = TRUE; // Allow sort
		$this->fields['cycle_name'] = &$this->cycle_name;

		// cycle_interval
		$this->cycle_interval = new cField('phs_bill_cycle', 'phs_bill_cycle', 'x_cycle_interval', 'cycle_interval', '`cycle_interval`', '`cycle_interval`', 200, -1, FALSE, '`cycle_interval`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->cycle_interval->Sortable = TRUE; // Allow sort
		$this->fields['cycle_interval'] = &$this->cycle_interval;

		// status_id
		$this->status_id = new cField('phs_bill_cycle', 'phs_bill_cycle', 'x_status_id', 'status_id', '`status_id`', '`status_id`', 16, -1, FALSE, '`status_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// cycle_desc
		$this->cycle_desc = new cField('phs_bill_cycle', 'phs_bill_cycle', 'x_cycle_desc', 'cycle_desc', '`cycle_desc`', '`cycle_desc`', 200, -1, FALSE, '`cycle_desc`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->cycle_desc->Sortable = TRUE; // Allow sort
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`phs_bill_cycle`";
	}
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "`cycle_id` ASC";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "phs_bill_cycleedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "phs_bill_cycleadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("phs_bill_cycleview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("phs_bill_cycleview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "phs_bill_cycleadd.php?" . $this->UrlParm($parm);
		else
			$url = "phs_bill_cycleadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("phs_bill_cycleedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("phs_bill_cycleadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("phs_bill_cycledelete.php", $this->UrlParm());
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["cycle_id"]))
				$arKeys[] = $_POST["cycle_id"];
			elseif (isset($_GET["cycle_id"]))
				$arKeys[] = $_GET["cycle_id"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->cycle_id->CurrentValue = $key;
	...
	function LoadListRowValues(&$rs) {
		...
		$this->cycle_interval->setDbValue($rs->fields('cycle_interval'));
	...
	function RenderEditRow() {
		...
		$this->cycle_interval->EditValue = $this->cycle_interval->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->cycle_interval->Exportable) $Doc->ExportCaption($this->cycle_interval);
					if ($this->status_id->Exportable) $Doc->ExportCaption($this->status_id);
					if ($this->cycle_desc->Exportable) $Doc->ExportCaption($this->cycle_desc);
				} else {
					if ($this->cycle_id->Exportable) $Doc->ExportCaption($this->cycle_id);
					if ($this->cycle_name->Exportable) $Doc->ExportCaption($this->cycle_name);
					if ($this->cycle_interval->Exportable) $Doc->ExportCaption($this->cycle_interval);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->cycle_interval->Exportable) $Doc->ExportField($this->cycle_interval);
						if ($this->status_id->Exportable) $Doc->ExportField($this->status_id);
						if ($this->cycle_desc->Exportable) $Doc->ExportField($this->cycle_desc);
					} else {
						if ($this->cycle_id->Exportable) $Doc->ExportField($this->cycle_id);
						if ($this->cycle_name->Exportable) $Doc->ExportField($this->cycle_name);
						if ($this->cycle_interval->Exportable) $Doc->ExportField($this->cycle_interval);
