...
class capp_tips_package_add extends capp_tips_package {
	...
	function ValidateForm() {
		...
		if (in_array("app_tips_pkg_cat", $DetailTblVar) && $GLOBALS["app_tips_pkg_cat"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("app_tips_pkg_cat", $DetailTblVar) && $GLOBALS["app_tips_pkg_cat"]->DetailAdd) {
				$GLOBALS["app_tips_pkg_cat"]->pkg_id->setSessionValue($this->pkg_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["app_tips_pkg_cat_grid"])) $GLOBALS["app_tips_pkg_cat_grid"] = new capp_tips_pkg_cat_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "app_tips_pkg_cat"); // Load user level of detail table
				$AddRow = $GLOBALS["app_tips_pkg_cat_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["app_tips_pkg_cat"]->pkg_id->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("app_tips_pkg_cat", $DetailTblVar)) {
				...
				if ($GLOBALS["app_tips_pkg_cat_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["app_tips_pkg_cat_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["app_tips_pkg_cat_grid"]->CurrentMode = "add";
					$GLOBALS["app_tips_pkg_cat_grid"]->CurrentAction = "gridadd";
...
<?php if ($app_tips_package->cycle_id->Visible) { // cycle_id ?>
	<div id="r_cycle_id" class="form-group">
		<label id="elh_app_tips_package_cycle_id" for="x_cycle_id" class="<?php echo $app_tips_package_add->LeftColumnClass ?>"><?php echo $app_tips_package->cycle_id->FldCaption() ?></label>
...
<?php
	if (in_array("app_tips_pkg_cat", explode(",", $app_tips_package->getCurrentDetailTable())) && $app_tips_pkg_cat->DetailAdd) {
?>
